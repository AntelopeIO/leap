...
private:
   ...
   static constexpr uint32_t space_threshold_warning_diff =
      5; // Warning issued when space used reached (threshold - space_threshold_warning_diff).
         // space_threshold_warning_diff must be smaller than space_threshold_min

   static constexpr uint32_t def_monitor_warning_interval =
      30; // After this number of monitor intervals, warning is output if the threshold is hit
   static constexpr uint32_t warning_interval_min = 1;
   static constexpr uint32_t warning_interval_max =
      450; // e.g. if the monitor interval is 2 sec, the warning interval is at most 15 minutes
