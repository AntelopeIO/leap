...
public:
   ...
   void set_threshold(uint32_t new_threshold, uint32_t new_warning_threshold) {
      EOS_ASSERT(new_warning_threshold < new_threshold,
                 ...
                 ("new_warning_threshold", new_warning_threshold)("new_threshold", new_threshold));
      ...
      for (auto& fs : filesystems) {
         ...
         if (ec) {
            ...
            wlog("Unable to get space info for ${path_name}: [code: ${ec}] ${message}. Ignore this failure.",
                 ("path_name", fs.path_name.string())("ec", ec.value())("message", ec.message()));
   ...
   void add_file_system(const bfs::path& path_name) {
      ...
      EOS_ASSERT(status == 0,
                 chain::plugin_config_exception,
                 "Failed to run stat on ${path} with status ${status}",
                 ("path", path_name.string())("status", status));
      ...
      for (auto& fs : filesystems) {
         if (statbuf.st_dev ==
             fs.st_dev) { // Two files belong to the same file system if their device IDs are the same.
...
private:
   ...
   struct filesystem_info {
      ...
      bfs::path path_name;
      uintmax_t warning_available{
         0
      }; // warning is issued when availabla number of bytese drops below warning_available
