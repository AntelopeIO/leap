...
struct txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string&                                   init_name,
      ...
      try {
         ...
         fc::crypto::private_key txn_test_receiver_A_priv_key =
            fc::crypto::private_key::regenerate(fc::sha256(std::string(64, 'a')));
         fc::crypto::private_key txn_test_receiver_B_priv_key =
            fc::crypto::private_key::regenerate(fc::sha256(std::string(64, 'b')));
         fc::crypto::private_key txn_test_receiver_C_priv_key =
            fc::crypto::private_key::regenerate(fc::sha256(std::string(64, 'c')));
         ...
         // set newaccountT contract to eosio.token & initialize it
         {
...

            {
               ...
               act.authorization = vector<permission_level>{
                  {newaccountT, config::active_name}
               };
            ...
            }
            {
               ...
               act.data = eosio_token_serializer.variant_to_binary(
                  "issue",
                  fc::json::from_string(
                     fc::format_string("{\"to\":\"${to}\",\"quantity\":\"60000.0000 CUR\",\"memo\":\"\"}",
   ...
   string start_generation(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
      ...
      act_a_to_b.authorization = vector<permission_level>{
         {newaccountA, config::active_name}
      };
      act_a_to_b.data = eosio_token_serializer.variant_to_binary(
         "transfer",
         fc::json::from_string(fc::format_string(
      ...
      act_b_to_a.authorization = vector<permission_level>{
         {newaccountB, config::active_name}
      };
      act_b_to_a.data = eosio_token_serializer.variant_to_binary(
         "transfer",
         fc::json::from_string(fc::format_string(
            "{\"from\":\"${from}\",\"to\":\"${to}\",\"quantity\":\"1.0000 CUR\",\"memo\":\"${l}\"}",
            fc::mutable_variant_object()("from",
   ...
   void send_transaction(std::function<void(const fc::exception_ptr&)> next, uint64_t nonce_prefix) {
      ...
      try {
         ...
         static fc::crypto::private_key a_priv_key =
            fc::crypto::private_key::regenerate(fc::sha256(std::string(64, 'a')));
         static fc::crypto::private_key b_priv_key =
            fc::crypto::private_key::regenerate(fc::sha256(std::string(64, 'b')));
...
void txn_test_gen_plugin::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()("txn-reference-block-lag",
      ...
      "txn-test-gen-expiration-seconds",
      bpo::value<uint16_t>()->default_value(30),
      "expiration in seconds for transactions generated by this plugin")(
      "txn-test-gen-stop-on-push-failed",
...
void txn_test_gen_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      EOS_ASSERT(my->thread_pool_size > 0,
                 chain::plugin_config_exception,
                 "txn-test-gen-threads ${num} must be greater than 0",
...
void txn_test_gen_plugin::plugin_startup() {
   app().get_plugin<http_plugin>().add_api(
      { CALL_ASYNC(txn_test_gen,
                   ...
                   INVOKE_ASYNC_R_R(my, create_test_accounts, std::string, std::string),
                   200),
        CALL(txn_test_gen, my, stop_generation, INVOKE_V_V(my, stop_generation), 200),
        CALL(txn_test_gen,
             my,
             start_generation,
             INVOKE_V_R_R_R(my, start_generation, std::string, uint64_t, uint64_t),
             200) });
}
