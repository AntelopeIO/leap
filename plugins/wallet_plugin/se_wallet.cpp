...
#define STR(A) #A
   ...
   void populate_existing_keys()
   {
      ...
      CFDictionaryRef keyAttrDic = CFDictionaryCreate(nullptr,
                                                      keyAttrKeys,
                                                      keyAttrValues,
   ...
   public_key_type create()
   {
      ...
      const void* keyAttrKeys[]   = { kSecAttrIsPermanent, kSecAttrAccessControl, kSecAttrAccessGroup };
      const void* keyAttrValues[] = { kCFBooleanTrue,
                                      accessControlRef,
#ifdef MAS_KEYCHAIN_GROUP
                                      CFSTR(XSTR(MAS_KEYCHAIN_GROUP))
#endif
      };
      CFDictionaryRef keyAttrDic = CFDictionaryCreate(NULL,
                                                      keyAttrKeys,
                                                      keyAttrValues,
                                                      sizeof(keyAttrValues) / sizeof(keyAttrValues[0]),
                                                      &kCFTypeDictionaryKeyCallBacks,
      ...
      CFDictionaryRef attributesDic = CFDictionaryCreate(NULL,
                                                         attrKeys,
                                                         atrrValues,
      ...
      if (error_string.size())
         FC_THROW_EXCEPTION(
            chain::wallet_exception, "Failed to create key in Secure Enclave: ${m}", ("m", error_string));
   ...
   std::optional<signature_type> try_sign_digest(const digest_type d, const public_key_type public_key)
   {
      ...
      if (error) {
         ...
         FC_THROW_EXCEPTION(
            chain::wallet_exception, "Failed to sign digest in Secure Enclave: ${m}", ("m", error_string));
      }
   ...
   bool remove_key(string public_key)
   {
      ...
      CFDictionaryRef deleteDic = CFDictionaryCreate(nullptr,
