   ...
   virtual bool verify_max_requests_in_flight() override {
      ...
      if (requests_in_flight_num > plugin_state_->max_requests_in_flight) {
         fc_dlog(plugin_state_->logger,
                 "429 - too many requests in flight: ${requests}",
                 ("requests", requests_in_flight_num));
         error_results::error_info ei;
         ei.code = static_cast<int64_t>(http::status::too_many_requests);
         ei.name = "Busy";
...
public:
   ...
   virtual void handle_exception() override {
      ...
      try {
         ...
         fc_elog(plugin_state_->logger,
                 "Timeout exception ${te} attempting to handle exception: ${e}",
                 ("te", e.to_detail_string())("e", err_str));
...
public:
   ...
   static constexpr auto name() { return "unix_socket_session"; }
