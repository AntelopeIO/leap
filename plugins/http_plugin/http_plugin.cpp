   ...
   void create_beast_server(bool useSSL, bool isUnix = false)
   {
      if (useSSL) {
         try {
            ...
            fc::ec_key ecdh =
               EC_KEY_new_by_curve_name(https_ecdh_curve == SECP384R1 ? NID_secp384r1 : NID_X9_62_prime256v1);
            if (!ecdh)
            ...
            if (SSL_CTX_set_cipher_list(
                   plugin_state->ctx->native_handle(),
       ...
       "Filename with the certificate chain to present on https connections. PEM format. Required for https.")
         ...
         ("https-private-key-file",
          ...
          "Filename with https private key in PEM format. Required for https")
            ...
            ("https-ecdh-curve",
             bpo::value<https_ecdh_curve_t>()
                ->notifier([this](https_ecdh_curve_t c) { my->https_ecdh_curve = c; })
...
void http_plugin::plugin_initialize(const variables_map& options)
{
   try {
      ...
      EOS_ASSERT(my->plugin_state->thread_pool_size > 0,
                 chain::plugin_config_exception,
                 "http-threads ${num} must be greater than 0",
      ...
      EOS_ASSERT(max_reponse_time_ms == -1 || max_reponse_time_ms >= 0,
                 chain::plugin_config_exception,
