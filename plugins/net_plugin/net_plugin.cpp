...
namespace eosio {
   ...
   public:
      ...
      void bcast_msg( const std::optional<uint32_t>& connection_id, send_buffer_type msg );
...
#pragma GCC diagnostic pop
   ...
   void dispatch_manager::bcast_msg( const std::optional<uint32_t>& connection_id, send_buffer_type msg ) {
      my_impl->connections.for_each_block_connection( [connection_id, msg{std::move(msg)}]( auto& cp ) {
         if( !cp->current() ) return true;
         if( connection_id.has_value() && cp->connection_id == connection_id.value() ) return true;
