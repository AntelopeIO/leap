...
typedef multi_index_container<
   ...
   indexed_by<
      ...
      ordered_non_unique<tag<by_block_num>,
                         member<eosio::peer_block_state, uint32_t, &eosio::peer_block_state::block_num>>>>
   peer_block_state_index;

class sync_manager {
private:
   enum stages { lib_catchup, head_catchup, in_sync };

   static constexpr int64_t block_interval_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                   std::chrono::milliseconds(config::block_interval_ms))
                                                   .count();
...
public:
   ...
   void                                sync_recv_block(const connection_ptr& c,
                                                       const block_id_type&  blk_id,
                                                       uint32_t              blk_num,
                                                       bool                  blk_applied);
   void                                sync_update_expected(const connection_ptr& c,
                                                            const block_id_type&  blk_id,
                                                            uint32_t              blk_num,
                                                            bool                  blk_applied);
   ...
   inline std::unique_lock<std::mutex> locked_sync_mutex() { return std::unique_lock<std::mutex>(sync_mtx); }
...
public:
   ...
   bool add_peer_txn(const transaction_id_type id,
...
constexpr uint32_t signed_block_which =
   fc::get_index<net_message, signed_block>(); // see protocol net_message
constexpr uint32_t packed_transaction_which =
   fc::get_index<net_message, packed_transaction>(); // see protocol net_message
 ...
 *  rm -f gap.txt; prev=0; for a in $(cat sorted.txt); do echo $prev $((0x$a - 0x$prev)) $a >> gap.txt;
 * prev=$a; done; sort -k2 -n gap.txt | tail
 *
...
constexpr uint16_t proto_dup_goaway_resolution =
   5; // eosio 2.1: support peer address based duplicate connection resolution
constexpr uint16_t proto_dup_node_id_goaway =
   6; // eosio 2.1: support peer node_id based duplicate connection resolution
constexpr uint16_t proto_leap_initial =
   7; // leap client, needed because none of the 2.1 versions are supported
...
private:
   struct queued_write;
   void fill_out_buffer(std::vector<boost::asio::const_buffer>& bufs, deque<queued_write>& w_queue) {
...
private:
   ...
   enum connection_types : char { both, transactions_only, blocks_only };
...
public:
    ...
    */
   static tstamp get_time() { return std::chrono::system_clock::now().time_since_epoch().count(); }
   /** @} */
   ...
   void handle_message(const packed_transaction& msg) =
      delete; // packed_transaction_ptr overload used instead
   ...
   fc::variant_object get_logger_variant() const {
      ...
      mvo("_name", log_p2p_address)("_cid", connection_id)("_id", conn_node_id)("_sid", short_conn_node_id)(
         "_ip", log_remote_endpoint_ip)("_port", log_remote_endpoint_port)("_lip", local_endpoint_ip)(
...
struct msg_handler : public fc::visitor<void> {
   connection_ptr c;
   explicit msg_handler(const connection_ptr& conn)
      : c(conn) {}
...
void connection::set_connection_type(const string& peer_add) {
   ...
   if (type.empty()) {
      ...
      fc_dlog(logger,
              "Setting connection ${c} type for: ${peer} to transactions only",
              ("c", connection_id)("peer", peer_add));
      connection_type = transactions_only;
   } else if (type == "blk") {
      fc_dlog(logger,
              "Setting connection ${c} type for: ${peer} to blocks only",
              ("c", connection_id)("peer", peer_add));
      connection_type = blocks_only;
   } else {
      fc_wlog(logger,
              "Unknown connection ${c} type: ${t}, for ${peer}",
...
void connection::_close(connection* self, bool reconnect, bool shutdown) {
   ...
   if (!shutdown)
      my_impl->sync_master->sync_reset_lib_num(self->shared_from_this(), true);
...
void connection::blk_send_branch(const block_id_type& msg_head_id) {
   uint32_t head_num = 0;
   std::tie(std::ignore, std::ignore, head_num, std::ignore, std::ignore, std::ignore) =
   ...
   app().post(priority::medium,
              [chain_plug = my_impl->chain_plug, c = shared_from_this(), lib_num, head_num, msg_head_id]() {
...
void connection::blk_send_branch_impl(uint32_t msg_head_num, uint32_t lib_num, uint32_t head_num) {
   ...
   if (peer_requested->start_block <= peer_requested->end_block) {
      peer_ilog(
         this, "enqueue ${s} - ${e}", ("s", peer_requested->start_block)("e", peer_requested->end_block));
      enqueue_sync_block();
   } else {
   ...
   app().post(priority::medium, [blkid, weak{ std::move(weak) }]() {
      ...
      try {
         ...
         if (b) {
            ...
            c->strand.post([c, b{ std::move(b) }]() { c->enqueue_block(b); });
         } else {
            fc_ilog(logger,
...
void connection::send_handshake() {
   strand.post([c = shared_from_this()]() {
      ...
      if (c->populate_handshake(c->last_handshake_sent)) {
         static_assert(std::is_same_v<decltype(c->sent_handshake_count), int16_t>,
         ...
         peer_ilog(
            ...
            ("g", last_handshake_sent.generation)("lib", last_handshake_sent.last_irreversible_block_num)(
...
void connection::check_heartbeat(tstamp current_time) {
   if (latest_msg_time > 0) {
      if (current_time > latest_msg_time + hb_timeout) {
         ...
         const tstamp timeout =
            std::max(hb_timeout / 2, 2 * std::chrono::milliseconds(config::block_interval_ms).count());
...
void connection::queue_write(const std::shared_ptr<vector<char>>&                        buff,
   ...
   if (!buffer_queue.add_write_queue(buff, callback, to_sync_queue)) {
      peer_wlog(this,
                "write_queue full ${s} bytes, giving up on connection",
      ...
      boost::asio::async_write(
         *c->socket,
         ...
         boost::asio::bind_executor(
            c->strand, [c, socket = c->socket](boost::system::error_code ec, std::size_t w) {
               try {
                  ...
                  if (!c->socket_is_open() || socket != c->socket) {
                     peer_ilog(c,
                               ...
                               ("r", c->socket_is_open() ? "changed" : "closed"));
                     c->close();
                     return;
                  }
...
bool connection::enqueue_sync_block() {
   ...
   app().post(priority::medium, [num, weak{ std::move(weak) }]() {
      ...
      FC_LOG_AND_DROP();
      if (sb) {
         c->strand.post([c, sb{ std::move(sb) }]() { c->enqueue_block(sb, true); });
      } else {
         c->strand.post([c, num]() {
            peer_ilog(
               c, "enqueue sync, unable to fetch block ${num}, sending benign_other go away", ("num", num));
...
protected:
   static send_buffer_type create_send_buffer(const net_message& m) {
      ...
      const char* const header =
         reinterpret_cast<const char* const>(&payload_size); // avoid variable size encoding of uint32_t
...
struct trx_buffer_factory : public buffer_factory {
   ...
   /// caches result for subsequent calls, only provide same packed_transaction_ptr instance for each
   /// invocation.
...
void connection::sync_wait() {
   ...
   response_expected_timer.async_wait(
      boost::asio::bind_executor(c->strand, [c](boost::system::error_code ec) { c->sync_timeout(ec); }));
}
...
void connection::fetch_wait() {
   ...
   response_expected_timer.expires_from_now(my_impl->resp_expected_period);
   response_expected_timer.async_wait(
      boost::asio::bind_executor(c->strand, [c](boost::system::error_code ec) { c->fetch_timeout(ec); }));
}
...
void connection::sync_timeout(boost::system::error_code ec) {
   if (!ec) {
      ...
      close(true);
              ...
              boost::asio::error::operation_aborted) { // don't log on operation_aborted, called on destroy
...
bool sync_manager::set_state(stages newstate) {
   ...
   fc_ilog(
      logger, "old state ${os} becoming ${ns}", ("os", stage_str(sync_state))("ns", stage_str(newstate)));
   sync_state = newstate;
   return true;
}
...
void sync_manager::sync_reset_lib_num(const connection_ptr& c, bool closing) {
   ...
   if (!closing) {
      ...
      // Closing connection, therefore its view of LIB can no longer be considered as we will no longer be
      // connected. Determine current LIB of remaining peers as our sync_known_lib_num.
      ...
      // if closing the connection we are currently syncing from, then reset our last requested and next
      // expected.
      if (c == sync_source) {
         reset_last_requested_num(g);
         // if starting to sync need to always start from lib as we might be on our own fork
         uint32_t lib_num = 0;
         std::tie(lib_num, std::ignore, std::ignore, std::ignore, std::ignore, std::ignore) =
            my_impl->get_chain_info();
...
void sync_manager::request_next_chunk(std::unique_lock<std::mutex> g_sync, const connection_ptr& conn) {
   ...
   std::tie(lib_block_num, std::ignore, fork_head_block_num, std::ignore, std::ignore, std::ignore) =
      my_impl->get_chain_info();

   fc_dlog(logger,
           "sync_last_requested_num: ${r}, sync_next_expected_num: ${e}, sync_known_lib_num: ${k}, "
           "sync_req_span: ${s}",
           ("r", sync_last_requested_num)("e", sync_next_expected_num)("k", sync_known_lib_num)(
              "s", sync_req_span));

   if (fork_head_block_num < sync_last_requested_num && sync_source && sync_source->current()) {
      fc_ilog(logger,
              "ignoring request, head is ${h} last req = ${r}, sync_next_expected_num: ${e}, "
              "sync_known_lib_num: ${k}, sync_req_span: ${s}, source connection ${c}",
   ...
   if (conn && conn->current()) {
      ...
      if (my_impl->connections.size() == 0) {
         ...
         // no need to check the result, either source advanced or the whole list was checked and the old
         // source is reused.
      }
   }

   // verify there is an available source
   if (!new_sync_source || !new_sync_source->current() ||
       new_sync_source->is_transactions_only_connection()) {
...
bool sync_manager::is_sync_required(uint32_t fork_head_block_num) {
   ...
   return (sync_last_requested_num < sync_known_lib_num || fork_head_block_num < sync_last_requested_num);
}
...
void sync_manager::start_sync(const connection_ptr& c, uint32_t target) {
   ...
   uint32_t lib_num             = 0;
   uint32_t fork_head_block_num = 0;
   std::tie(lib_num, std::ignore, fork_head_block_num, std::ignore, std::ignore, std::ignore) =
      my_impl->get_chain_info();
...
void sync_manager::recv_handshake(const connection_ptr& c, const handshake_message& msg) {
   ...
   std::tie(lib_num, std::ignore, head, std::ignore, std::ignore, head_id) = my_impl->get_chain_info();
   ...
   // 2x for time it takes for message to reach back to peer node, +1 to compensate for integer division
   // truncation
   ...
   peer_dlog(c,
             "Network latency is ${lat}ms, ${num} blocks discrepancy by network latency, ${tot_num} blocks "
             "discrepancy expected once message received",
             ("lat", network_latency_ns / 1000000)("num", nblk_behind_by_net_latency)("tot_num",
                                                                                      nblk_combined_latency));
   ...
   // 2. my lib > peer head num + nblk_combined_latency - send last_irr_catch_up notice if not the first
   // generation
   //
   // 3  my head block num + nblk_combined_latency < peer head block num - update sync state and send a
   // catchup request 4  my head block num >= peer block num + nblk_combined_latency send a notice catchup if
   ...
   if (head + nblk_combined_latency < msg.head_num) {
      ...
      app().post(
         priority::medium,
         [chain_plug = my_impl->chain_plug, c, msg_head_num = msg.head_num, msg_head_id = msg.head_id]() {
            bool on_fork = true;
            try {
...
bool sync_manager::verify_catchup(const connection_ptr& c, uint32_t num, const block_id_type& id) {
   ...
   if (req.req_blocks.mode == catch_up) {
      ...
      uint32_t      lib;
      block_id_type head_id;
      std::tie(lib, std::ignore, std::ignore, std::ignore, std::ignore, head_id) = my_impl->get_chain_info();
...
void sync_manager::sync_recv_notice(const connection_ptr& c, const notice_message& msg) {
   ...
   EOS_ASSERT(msg.known_blocks.mode == catch_up || msg.known_blocks.mode == last_irr_catch_up,
              plugin_exception,
   ...
   if (msg.known_blocks.mode == catch_up) {
      if (msg.known_blocks.ids.size() == 0) {
         ...
         peer_ilog(c,
                   ...
                   ("p", msg.known_blocks.pending)("n", block_header::num_from_id(id))(
...
// called from connection strand
void sync_manager::sync_update_expected(const connection_ptr& c,
                                        const block_id_type&  blk_id,
                                        uint32_t              blk_num,
                                        bool                  blk_applied) {
   std::unique_lock<std::mutex> g_sync(sync_mtx);
...
// called from c's connection strand
void sync_manager::sync_recv_block(const connection_ptr& c,
                                   const block_id_type&  blk_id,
                                   uint32_t              blk_num,
                                   bool                  blk_applied) {
...
bool dispatch_manager::add_peer_block(const block_id_type& blkid, uint32_t connection_id) {
   ...
   if (added) {
      ...
      blk_state.modify(bptr, [](auto& pb) { pb.have_block = true; });
   }
   return added;
}
...
bool dispatch_manager::add_peer_txn(const transaction_id_type id,
                                    const time_point_sec&     trx_expires,
                                    uint32_t                  connection_id,
   ...
   if (added) {
      ...
      local_txns.insert(
         node_transaction_state{ .id = id, .expires = expires, .connection_id = connection_id });
   }
   return added;
}
...
void dispatch_manager::recv_block(const connection_ptr& c, const block_id_type& id, uint32_t bnum) {
   ...
   if (c && c->last_req && c->last_req->req_blocks.mode != none && !c->last_req->req_blocks.ids.empty() &&
...
void dispatch_manager::bcast_transaction(const packed_transaction_ptr& trx) {
   ...
   for_each_connection([this, &trx, &now, &buff_factory](auto& cp) {
      ...
      fc_dlog(
         logger, "sending trx: ${id}, to connection ${cid}", ("id", trx->id())("cid", cp->connection_id));
      cp->strand.post([cp, sb{ std::move(sb) }]() { cp->enqueue_buffer(sb, no_reason); });
      return true;
   });
}
...
bool connection::resolve_and_connect() {
   switch (no_retry) {
      ...
      case wrong_version:
      case benign_other:
      case duplicate: // attempt reconnect in case connection has been dropped, should quickly disconnect if
                      // duplicate
         break;
      default:
   ...
   if (colon == std::string::npos || colon == 0) {
      fc_elog(
         logger, "Invalid peer address. must be \"host:port[:<blk>|<trx>]\": ${p}", ("p", peer_address()));
      return false;
   }
   ...
   if (consecutive_immediate_connection_close > def_max_consecutive_immediate_connection_close ||
       no_retry == benign_other) {
      auto connector_period_us =
         std::chrono::duration_cast<std::chrono::microseconds>(my_impl->connector_period);
   ...
   strand.post([c]() {
      ...
      string            port =
         c->peer_address().substr(colon + 1, colon2 == string::npos ? string::npos : colon2 - (colon + 1));
      ...
      resolver->async_resolve(
         ...
         boost::asio::bind_executor(c->strand,
                                    [resolver, weak_conn, host, port](const boost::system::error_code& err,
...
// called from connection strand
void connection::connect(const std::shared_ptr<tcp::resolver>& resolver,
                         tcp::resolver::results_type           endpoints) {
   switch (no_retry) {
      case no_reason:
      case wrong_version:
                                    ...
                                    if (!err && socket->is_open() && socket == c->socket) {
                                       ...
                                       fc_elog(logger,
                                               ...
                                               ("host", endpoint.address().to_string())(
                                                  "port", endpoint.port())("error", err.message()));
                                       c->close(false);
                                    }
                                 }));
}
   ...
   new_connection->strand.post([this, new_connection = std::move(new_connection)]() {
      acceptor->async_accept(
         *new_connection->socket,
         boost::asio::bind_executor(
            new_connection->strand,
            [new_connection, socket = new_connection->socket, this](boost::system::error_code ec) {
               if (!ec) {
                  uint32_t                  visitors  = 0;
                  ...
                  if (rec) {
                     ...
                     if (from_addr < max_nodes_per_host &&
                        ...
                        // new_connection never added to connections and start_session not called, lifetime
                        // will end
                        boost::system::error_code ec;
...
void connection::start_read_message() {
   try {
      ...
      if (write_queue_size > def_max_write_queue_size) {
         peer_elog(
            this, "write queue full ${s} bytes, giving up on connection, closing", ("s", write_queue_size));
         close(false);
         return;
      }
      ...
      boost::asio::async_read(
         *socket,
         ...
         boost::asio::bind_executor(
            strand,
            [conn = shared_from_this(), socket = socket](boost::system::error_code ec,
               ...
               try {
                  if (!ec) {
                     if (bytes_transferred > conn->pending_message_buffer.bytes_to_write()) {
                        peer_elog(
                           conn,
                           "async_read_some callback: bytes_transfered = ${bt}, buffer.bytes_to_write = "
                           "${btw}",
                           ("bt", bytes_transferred)("btw", conn->pending_message_buffer.bytes_to_write()));
                     }
                     EOS_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(),
                                plugin_exception,
                                "");
                     ...
                     while (conn->pending_message_buffer.bytes_to_read() > 0) {
                        ...
                        if (bytes_in_buffer < message_header_size) {
                           ...
                           if (message_length > def_send_buffer_size * 2 || message_length == 0) {
                              peer_elog(
                                 conn, "incoming message length unexpected (${i})", ("i", message_length));
                           ...
                           if (bytes_in_buffer >= total_message_bytes) {
                              ...
                              if (outstanding_message_bytes > available_buffer_bytes) {
                                 conn->pending_message_buffer.add_space(outstanding_message_bytes -
...
bool connection::process_next_block_message(uint32_t message_length) {
   ...
   if (!my_impl->sync_master
           ->syncing_with_peer()) { // guard against peer thinking it needs to send us old blocks
      uint32_t lib = 0;
      std::tie(lib, std::ignore, std::ignore, std::ignore, std::ignore, std::ignore) =
         my_impl->get_chain_info();
      if (blk_num < lib) {
   ...
   auto is_webauthn_sig = [](const fc::crypto::signature& s) {
      return s.which() ==
             fc::get_index<fc::crypto::signature::storage_type, fc::crypto::webauthn::signature>();
   };
   ...
   if (exts.count(additional_sigs_eid)) {
      const auto& additional_sigs =
         std::get<additional_block_signatures_extension>(exts.lower_bound(additional_sigs_eid)->second)
            .signatures;
...
net_plugin_impl::get_chain_info() const {
   std::lock_guard<std::mutex> g(chain_info_mtx);
   return std::make_tuple(chain_lib_num,
                          chain_head_blk_num,
                          chain_fork_head_blk_num,
                          chain_lib_id,
                          chain_head_blk_id,
      ...
      if (peer_address().empty() || last_handshake_recv.node_id == fc::sha256()) {
         ...
         for (const auto& check : my_impl->connections) {
            ...
            if (check->connected() && check->last_handshake_recv.node_id == msg.node_id) {
               if (net_version < proto_dup_goaway_resolution ||
                   msg.network_version < proto_dup_goaway_resolution) {
                  ...
                  if (msg.time + c_time <= check_time)
                     continue;
               } else if (net_version < proto_dup_node_id_goaway ||
                          msg.network_version < proto_dup_node_id_goaway) {
                  if (my_impl->p2p_address < msg.p2p_address) {
                     ...
                     // only the connection from lower p2p_address to higher p2p_address will be considered as
                     // a duplicate, so there is no chance for both connections to be closed
                     continue;
                  }
                  ...
                  // only the connection from lower node_id to higher node_id will be considered as a
                  // duplicate, so there is no chance for both connections to be closed
                  continue;
               }

               g_cnts.unlock();
               peer_dlog(
                  this, "sending go_away duplicate, msg.p2p_address: ${add}", ("add", msg.p2p_address));
...
void connection::handle_message(const go_away_message& msg) {
   ...
   if (msg.reason == wrong_version) {
      ...
      if (retry)
         peer_dlog(this, "received benign_other reason, retrying to connect");
   } else {
      retry = false;
   }
   flush_queues();
...
void connection::handle_message(const time_message& msg) {
   ...
   if (logger.is_enabled(fc::log_level::all))
      logger.log(
         FC_LOG_MESSAGE(all, "Clock offset is ${o}ns (${us}us)", ("o", offset)("us", offset / NsecPerUsec)));
   org = 0;
   rec = 0;
...
void connection::handle_message(const notice_message& msg) {
   ...
   if (msg.known_trx.mode != none) {
      if (logger.is_enabled(fc::log_level::debug)) {
         const block_id_type& blkid = msg.known_blocks.ids.empty() ? block_id_type{}
                                                                   : msg.known_blocks.ids.back();
         peer_dlog(this,
...
void connection::handle_message(packed_transaction_ptr trx) {
   ...
   my_impl->chain_plug->accept_transaction(
      trx,
      [weak = weak_from_this(),
       trx](const std::variant<fc::exception_ptr, transaction_trace_ptr>& result) mutable {
         // next (this lambda) called from application thread
         if (std::holds_alternative<fc::exception_ptr>(result)) {
            fc_dlog(
               logger, "bad packed_transaction : ${m}", ("m", std::get<fc::exception_ptr>(result)->what()));
         } else {
      ...
      boost::asio::post(my_impl->thread_pool->get_executor(),
                        [dispatcher = my_impl->dispatcher.get(), cid = c->connection_id, blk_id, msg]() {
                           fc_dlog(logger,
                                   ...
                                   ("n", msg->block_num())("id", blk_id.str().substr(8, 16)));
...
void net_plugin_impl::start_conn_timer(boost::asio::steady_timer::duration du,
   ...
   connector_check_timer->async_wait(
      [my = shared_from_this(), from_connection](boost::system::error_code ec) {
...
void net_plugin_impl::connection_monitor(std::weak_ptr<connection> from_connection, bool reschedule) {
   ...
   while (it != connections.end()) {
      if (fc::time_point::now() >= max_time) {
         ...
         g.unlock();
         fc_dlog(logger,
                 "Exiting connection monitor early, ran out of time: ${t}",
                 ("t", max_time - fc::time_point::now()));
         fc_ilog(
            logger,
...
void net_plugin_impl::on_irreversible_block(const block_state_ptr& block) {
   fc_dlog(logger,
           ...
           ("num", block->block_num)("id", block->id));
   update_chain_info();
}
...
void net_plugin_impl::transaction_ack(const std::pair<fc::exception_ptr, packed_transaction_ptr>& results) {
   dispatcher->strand.post([this, results]() {
      ...
      if (results.first) {
         ...
         std::tie(std::ignore, head_blk_num, std::ignore, std::ignore, std::ignore, std::ignore) =
            get_chain_info();
...
bool net_plugin_impl::authenticate_peer(const handshake_message& msg) const {
   ...
   if (msg.sig != chain::signature_type() && msg.token != sha256()) {
      ...
      try {
         ...
         fc_elog(
            logger, "Peer ${peer} sent a handshake with an unrecoverable key.", ("peer", msg.p2p_address));
         return false;
      }
      if ((allowed_connections & (Producers | Specified)) && peer_key != msg.key) {
         fc_elog(
            logger, "Peer ${peer} sent a handshake with an unauthenticated key.", ("peer", msg.p2p_address));
         return false;
      }
   } else if (allowed_connections & (Producers | Specified)) {
      fc_dlog(logger,
              "Peer sent a handshake with blank signature and token, but this node accepts only "
...
chain::signature_type net_plugin_impl::sign_compact(const chain::public_key_type& signer,
...
void net_plugin::set_program_options(options_description& /*cli*/, options_description& cfg) {
   cfg.add_options()("p2p-listen-endpoint",
                     bpo::value<string>()->default_value("0.0.0.0:9876"),
      ...
      "The public endpoint of a peer node to connect to. Use multiple p2p-peer-address options as needed to "
      "compose a network.\n"
      "  Syntax: host:port[:<trx>|<blk>]\n"
      "  The optional 'trx' and 'blk' indicates to node that only transactions 'trx' or blocks 'blk' should "
      "be sent."
      "  Examples:\n"
      "    p2p.eos.io:9876\n"
      "    p2p.trx.eos.io:9876:trx\n"
      "    p2p.blk.eos.io:9876:blk\n")("p2p-max-nodes-per-host",
                                       bpo::value<int>()->default_value(def_max_nodes_per_host),
      ...
      "agent-name",
      bpo::value<string>()->default_value("EOS Test Agent"),
      "The name supplied to identify this node amongst the peers.")(
      "allowed-connection",
      bpo::value<vector<string>>()->multitoken()->default_value({ "any" }, "any"),
      "Can be 'any' or 'producers' or 'specified' or 'none'. If 'specified', peer-key must be specified at "
      "least once. If only 'producers', peer-key is not required. 'producers' and 'specified' may be "
      "combined.")("peer-key",
      ...
      "net-threads",
      bpo::value<uint16_t>()->default_value(my->thread_pool_size),
      "Number of worker threads in net_plugin thread pool")(
      "sync-fetch-span",
      bpo::value<uint32_t>()->default_value(def_sync_fetch_span),
      ...
      "peer-log-format",
      bpo::value<string>()->default_value("[\"${_name}\" - ${_cid} ${_ip}:${_port}] "),
      ...
      bpo::value<int>()->default_value(def_keepalive_interval),
...
void net_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      ...
      my->p2p_dedup_cache_expire_time_us =
         fc::seconds(options.at("p2p-dedup-cache-expire-time-sec").as<uint32_t>());
      ...
      EOS_ASSERT(my->keepalive_interval.count() > 0,
                 chain::plugin_config_exception,
                 "p2p-keepalive_interval-ms must be greater than 0");

      if (options.count("p2p-keepalive-interval-ms")) {
         my->heartbeat_timeout =
            std::chrono::milliseconds(options.at("p2p-keepalive-interval-ms").as<int>() * 2);
      }
      ...
      if (options.count("p2p-listen-endpoint") && options.at("p2p-listen-endpoint").as<string>().length()) {
         ...
         EOS_ASSERT(my->p2p_address.length() <= max_p2p_address_length,
                    chain::plugin_config_exception,
                    "p2p-listen-endpoint too long, must be less than ${m}",
                    ("m", max_p2p_address_length));
      }
      if (options.count("p2p-server-address")) {
         my->p2p_server_address = options.at("p2p-server-address").as<string>();
         EOS_ASSERT(my->p2p_server_address.length() <= max_p2p_address_length,
                    chain::plugin_config_exception,
                    "p2p_server_address too long, must be less than ${m}",
                    ("m", max_p2p_address_length));
      }

      my->thread_pool_size = options.at("net-threads").as<uint16_t>();
      EOS_ASSERT(my->thread_pool_size > 0,
                 chain::plugin_config_exception,
                 "net-threads ${num} must be greater than 0",
      ...
      if (options.count("agent-name")) {
         ...
         EOS_ASSERT(my->user_agent_name.length() <= max_handshake_str_length,
                    chain::plugin_config_exception,
                    "agent-name too long, must be less than ${m}",
                    ("m", max_handshake_str_length));
      }
      ...
      if (options.count("peer-private-key")) {
         ...
         for (const std::string& key_id_to_wif_pair_string : key_id_to_wif_pair_strings) {
            auto key_id_to_wif_pair =
               dejsonify<std::pair<chain::public_key_type, std::string>>(key_id_to_wif_pair_string);
...
void net_plugin::plugin_startup() {
   ...
   try {
...

      {
         ...
         cc.accepted_block.connect([my = my](const block_state_ptr& s) { my->on_accepted_block(s); });
         cc.pre_accepted_block.connect(
            [my = my](const signed_block_ptr& s) { my->on_pre_accepted_block(s); });
         cc.irreversible_block.connect([my = my](const block_state_ptr& s) { my->on_irreversible_block(s); });
      }
      ...
      my->incoming_transaction_ack_subscription =
         app().get_channel<compat::channels::transaction_ack>().subscribe(
