...
private:
   ...
   static constexpr int64_t block_interval_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::milliseconds(config::block_interval_ms))
         .count();
...
public:
   ...
   inline void reset_last_requested_num(const std::unique_lock<std::mutex>& lock) { sync_last_requested_num = 0; }
};
 ...
 *  rm -f gap.txt; prev=0; for a in $(cat sorted.txt); do echo $prev $((0x$a - 0x$prev)) $a >> gap.txt; prev=$a; done;
 * sort -k2 -n gap.txt | tail
 *
...
void connection::set_connection_type(const string& peer_add) {
   ...
   if (type.empty()) {
      ...
      fc_dlog(
         logger, "Setting connection ${c} type for: ${peer} to blocks only", ("c", connection_id)("peer", peer_add));
      connection_type = blocks_only;
   } else {
      fc_wlog(
         logger, "Unknown connection ${c} type: ${t}, for ${peer}", ("c", connection_id)("t", type)("peer", peer_add));
   }
}
...
void connection::blk_send_branch(const block_id_type& msg_head_id) {
   uint32_t head_num                                                                   = 0;
   std::tie(std::ignore, std::ignore, head_num, std::ignore, std::ignore, std::ignore) = my_impl->get_chain_info();
   ...
   app().post(
      priority::medium, [chain_plug = my_impl->chain_plug, c = shared_from_this(), lib_num, head_num, msg_head_id]() {
         ...
         if (unknown_block) {
            ...
            c->strand.post(
               [c, msg_head_num, lib_num, head_num]() { c->blk_send_branch_impl(msg_head_num, lib_num, head_num); });
         }
      });
}
...
void connection::blk_send(const block_id_type& blkid) {
   ...
   app().post(priority::medium, [blkid, weak{ std::move(weak) }]() {
      ...
      try {
         ...
         if (b) {
            fc_dlog(
               logger, "fetch_block_by_id num ${n}, connection ${cid}", ("n", b->block_num())("cid", c->connection_id));
         ...
         boost::asio::bind_executor(c->strand, [c, socket = c->socket](boost::system::error_code ec, std::size_t w) {
            try {
               ...
               if (!c->socket_is_open() || socket != c->socket) {
                  peer_ilog(
                     c, "async write socket ${r} before callback", ("r", c->socket_is_open() ? "changed" : "closed"));
                  c->close();
                  return;
               }
...
void sync_manager::request_next_chunk(std::unique_lock<std::mutex> g_sync, const connection_ptr& conn) {
   ...
   if (fork_head_block_num < sync_last_requested_num && sync_source && sync_source->current()) {
      fc_ilog(logger,
              "ignoring request, head is ${h} last req = ${r}, sync_next_expected_num: ${e}, sync_known_lib_num: ${k}, "
              "sync_req_span: ${s}, source connection ${c}",
   ...
   if (conn && conn->current()) {
      ...
      if (my_impl->connections.size() == 0) {
         ...
         // no need to check the result, either source advanced or the whole list was checked and the old source is
         // reused.
      }
   }
...
void sync_manager::recv_handshake(const connection_ptr& c, const handshake_message& msg) {
   ...
   if (network_latency_ns < 0) {
      peer_wlog(
         c, "Peer sent a handshake with a timestamp skewed by at least ${t}ms", ("t", network_latency_ns / 1000000));
   ...
   peer_dlog(
      c,
      "Network latency is ${lat}ms, ${num} blocks discrepancy by network latency, ${tot_num} blocks discrepancy "
      "expected once message received",
   ...
   // 4  my head block num >= peer block num + nblk_combined_latency send a notice catchup if this is not the first
   // generation
   ...
   if (req.req_blocks.mode == catch_up) {
      {
         ...
         peer_ilog(c,
                   ...
                   ("s", stage_str(sync_state))("fhn", num)("lib", sync_known_lib_num)("ne", sync_next_expected_num)(
...
bool dispatch_manager::add_peer_txn(const transaction_id_type id,
   ...
   if (added) {
      ...
      local_txns.insert(node_transaction_state{ .id = id, .expires = expires, .connection_id = connection_id });
   }
   return added;
}
   ...
   boost::asio::async_connect(
      *socket,
      ...
      boost::asio::bind_executor(
         strand,
         [resolver, c = shared_from_this(), socket = socket](const boost::system::error_code& err,
            ...
            [conn = shared_from_this(), socket = socket](boost::system::error_code ec, std::size_t bytes_transferred) {
               ...
               try {
                  if (!ec) {
                     ...
                     EOS_ASSERT(
                        bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
...
bool connection::process_next_block_message(uint32_t message_length) {
   ...
   if (exts.count(additional_sigs_eid)) {
      const auto& additional_sigs =
         std::get<additional_block_signatures_extension>(exts.lower_bound(additional_sigs_eid)->second).signatures;
...
std::tuple<uint32_t, uint32_t, uint32_t, block_id_type, block_id_type, block_id_type> net_plugin_impl::get_chain_info()
   const {
   std::lock_guard<std::mutex> g(chain_info_mtx);
            ...
            if (check->connected() && check->last_handshake_recv.node_id == msg.node_id) {
               if (net_version < proto_dup_goaway_resolution || msg.network_version < proto_dup_goaway_resolution) {
                  ...
                  if (my_impl->p2p_address < msg.p2p_address) {
                     ...
                     // only the connection from lower p2p_address to higher p2p_address will be considered as a
...
void connection::handle_message(const time_message& msg) {
   ...
   if (logger.is_enabled(fc::log_level::all))
      logger.log(FC_LOG_MESSAGE(all, "Clock offset is ${o}ns (${us}us)", ("o", offset)("us", offset / NsecPerUsec)));
   org = 0;
   rec = 0;
...
void connection::handle_message(const request_message& msg) {
   if (msg.req_blocks.ids.size() > 1) {
      peer_elog(this, "Invalid request_message, req_blocks.ids.size ${s}, closing", ("s", msg.req_blocks.ids.size()));
      close();
      return;
   }
...
void connection::process_signed_block(const block_id_type& blk_id, signed_block_ptr msg) {
   ...
   try {
      if (cc.fetch_block_by_id(blk_id)) {
         c->strand.post(
            [sync_master = my_impl->sync_master.get(), dispatcher = my_impl->dispatcher.get(), c, blk_id, blk_num]() {
...
void net_plugin_impl::connection_monitor(std::weak_ptr<connection> from_connection, bool reschedule) {
   ...
   while (it != connections.end()) {
      if (fc::time_point::now() >= max_time) {
         ...
         g.unlock();
         fc_dlog(
            logger, "Exiting connection monitor early, ran out of time: ${t}", ("t", max_time - fc::time_point::now()));
         fc_ilog(logger,
...
void net_plugin_impl::on_pre_accepted_block(const signed_block_ptr& block) {
   ...
   if (cc.is_trusted_producer(block->producer)) {
      dispatcher->strand.post([this, block]() {
         ...
         fc_dlog(
            logger, "signaled pre_accepted_block, blk num = ${num}, id = ${id}", ("num", block->block_num())("id", id));
...
void net_plugin_impl::transaction_ack(const std::pair<fc::exception_ptr, packed_transaction_ptr>& results) {
   dispatcher->strand.post([this, results]() {
      ...
      if (results.first) {
         fc_dlog(
            logger, "signaled NACK, trx-id = ${id} : ${why}", ("id", id)("why", results.first->to_detail_string()));
...
void net_plugin::set_program_options(options_description& /*cli*/, options_description& cfg) {
   cfg.add_options()("p2p-listen-endpoint",
      ...
      "The public endpoint of a peer node to connect to. Use multiple p2p-peer-address options as needed to compose a "
      "network.\n"
      ...
      "The string used to format peers when logging messages about them.  Variables are escaped with ${<variable "
      "name>}.\n"
      "Available Variables:\n"
...
void net_plugin::plugin_startup() {
   ...
   try {
...

      {
         ...
         cc.pre_accepted_block.connect([my = my](const signed_block_ptr& s) { my->on_pre_accepted_block(s); });
