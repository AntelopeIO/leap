...
using data_handler_function = std::function<std::tuple<fc::variant, std::optional<fc::variant>>(
    const std::variant<action_trace_v0, action_trace_v1>& action_trace_t, const yield_function&)>;

namespace detail {
class response_formatter {
 public:
   static fc::variant process_block(const data_log_entry& trace, bool irreversible,
...
class request_handler {
 public:
   ...
   fc::variant get_block_trace(uint32_t block_height, const yield_function& yield = {}) {
      ...
      auto data_handler = [this](const auto&           action,
                                 const yield_function& yield) -> std::tuple<fc::variant, std::optional<fc::variant>> {
         return std::visit(
   ...
   fc::variant get_transaction_trace(chain::transaction_id_type trxid, uint32_t block_height,
      ...
      if (!resp.is_null()) {
         ...
         if (b_mvo.contains("transactions")) {
            ...
            if (result.is_null())
               _log("Exhausted all " + std::to_string(transactions.size()) + " transactions in block " +
                    b_mvo["number"].as_string() + " without finding trxid " + trxid.str());
         }
      }
      return result;
   }
