...
class slice_directory {
 public:
    ...
    */
   uint32_t slice_number(uint32_t block_height) const { return block_height / _width; }
 ...
 private:
   ...
   template <typename F>
   void process_irreversible_slice_range(uint32_t lib, uint32_t upper_bound_block,
...
class store_provider {
 public:
   ...
   store_provider(const boost::filesystem::path& slice_dir, uint32_t stride_width,
                  std::optional<uint32_t> minimum_irreversible_history_blocks,
                  std::optional<uint32_t> minimum_uncompressed_irreversible_history_blocks,
                  size_t                  compression_seek_point_stride);
   ...
   get_block_n get_trx_block_number(const chain::transaction_id_type& trx_id,
                                    std::optional<uint32_t>           minimum_irreversible_history_blocks,
                                    const yield_function&             yield = {});

   void start_maintenance_thread(log_handler log) { _slice_directory.start_maintenance_thread(std::move(log)); }
   void stop_maintenance_thread() { _slice_directory.stop_maintenance_thread(); }
 ...
 protected:
   ...
   std::optional<data_log_entry> read_data_log(uint32_t block_height, uint64_t offset) {
      ...
      if (!_slice_directory.find_trace_slice(slice_number, open_state::read, trace)) {
         ...
         throw malformed_slice_file("Requested offset: " + offset_str + " to retrieve block number: " + bh_str +
