...
class abi_data_handler {
 public:
    ...
    * @return tuple where the first element is a variant representing the `data` field of the action interpreted by
    * known ABIs OR an empty variant, and the second element represents the `return_value` field of the trace.
    */
   std::tuple<fc::variant, std::optional<fc::variant>>
   serialize_to_variant(const std::variant<action_trace_v0, action_trace_v1>& action, const yield_function& yield);
