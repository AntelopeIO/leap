...
chain::transaction_trace_ptr make_transaction_trace(const chain::transaction_id_type& id, uint32_t block_number,
                                                    ...
                                                    std::vector<chain::action_trace>&&             actions) {
   return std::make_shared<chain::transaction_trace>(chain::transaction_trace{id,
                                                                              block_number,
...
auto make_transfer_action(chain::name from, chain::name to, chain::asset quantity, std::string memo) {
   return chain::action(std::vector<chain::permission_level>{{from, chain::config::active_name}}, "eosio.token"_n,
                        "transfer"_n, make_transfer_data(from, to, quantity, std::move(memo)));
}
...
chain::action_trace make_action_trace(uint64_t global_sequence, chain::action act, chain::name receiver) {
   ...
   result.receipt.emplace(chain::action_receipt{receiver, digest_type::hash(act), global_sequence, 0, {}, 0, 0});
...
struct extraction_test_fixture {
   ...
   struct mock_logfile_provider_type {
      ...
      void append_lib(uint32_t lib) { fixture.max_lib = std::max(fixture.max_lib, lib); }

      void append_trx_ids(const block_trxs_entry& tt) { fixture.id_log[tt.block_num] = tt.ids; }
   ...
   void signal_accepted_block(const chain::block_state_ptr& bsp) { extraction_impl.signal_accepted_block(bsp); }
...
BOOST_FIXTURE_TEST_CASE(basic_single_transaction_block, extraction_test_fixture) {
   ...
   signal_applied_transaction(
       make_transaction_trace(ptrx1.id(), 1, 1, chain::transaction_receipt_header::executed, {actt1, actt2, actt3}),
   ...
   const transaction_trace_v3 expected_transaction_trace{
       {ptrx1.id(), expected_action_traces,
        fc::enum_type<uint8_t, chain::transaction_receipt_header::status_enum>{bsp1->block->transactions[0].status},
        bsp1->block->transactions[0].cpu_usage_us, bsp1->block->transactions[0].net_usage_words, ptrx1.get_signatures(),
        make_trx_header(ptrx1.get_transaction())}};

   const block_trace_v2 expected_block_trace{bsp1->id,
                                             1,
                                             bsp1->prev(),
                                             ...
                                             std::vector<transaction_trace_v3>{expected_transaction_trace}};
...
BOOST_FIXTURE_TEST_CASE(basic_multi_transaction_block, extraction_test_fixture) {
   ...
   signal_applied_transaction(
       make_transaction_trace(ptrx1.id(), 1, 1, chain::transaction_receipt_header::executed, {actt1}),
       std::make_shared<packed_transaction>(ptrx1));
   signal_applied_transaction(
       make_transaction_trace(ptrx2.id(), 1, 1, chain::transaction_receipt_header::executed, {actt2}),
       std::make_shared<packed_transaction>(ptrx2));
   signal_applied_transaction(
       make_transaction_trace(ptrx3.id(), 1, 1, chain::transaction_receipt_header::executed, {actt3}),
       std::make_shared<packed_transaction>(ptrx3));

   // accept the block with three transaction
   auto bsp1 = make_block_state(
       chain::block_id_type(), 1, 1, "bp.one"_n,
   ...
   const std::vector<transaction_trace_v3> expected_transaction_traces{
       {{ptrx1.id(), expected_action_trace1,
         fc::enum_type<uint8_t, chain::transaction_receipt_header::status_enum>{bsp1->block->transactions[0].status},
         bsp1->block->transactions[0].cpu_usage_us, bsp1->block->transactions[0].net_usage_words,
         ptrx1.get_signatures(), make_trx_header(ptrx1.get_transaction())}},
       {{ptrx2.id(), expected_action_trace2,
         fc::enum_type<uint8_t, chain::transaction_receipt_header::status_enum>{bsp1->block->transactions[1].status},
         bsp1->block->transactions[1].cpu_usage_us, bsp1->block->transactions[1].net_usage_words,
         ptrx2.get_signatures(), make_trx_header(ptrx2.get_transaction())}},
       {{ptrx3.id(), expected_action_trace3,
         fc::enum_type<uint8_t, chain::transaction_receipt_header::status_enum>{bsp1->block->transactions[2].status},
         bsp1->block->transactions[2].cpu_usage_us, bsp1->block->transactions[2].net_usage_words,
         ptrx3.get_signatures(), make_trx_header(ptrx3.get_transaction())}}};
...
BOOST_FIXTURE_TEST_CASE(onerror_transaction_block, extraction_test_fixture) {
   ...
   auto onerror_trace = make_transaction_trace(ptrx1.id(), 1, 1, chain::transaction_receipt_header::executed, {actt1});
   auto transfer_trace =
       make_transaction_trace(transfer_trx.id(), 1, 1, chain::transaction_receipt_header::soft_fail, {actt2});
   ...
   auto bsp1 = make_block_state(chain::block_id_type(), 1, 1, "bp.one"_n, {chain::packed_transaction(transfer_trx)});
   signal_accepted_block(bsp1);

   const std::vector<action_trace_v1> expected_action_trace{{{0,
                                                              "eosio.token"_n,
                                                              "eosio"_n,
                                                              "onerror"_n,
                                                              {{"alice"_n, "active"_n}},
   ...
   const std::vector<transaction_trace_v3> expected_transaction_traces{
       {{transfer_trx.id(), // transfer_trx.id() because that is the trx id known to the user
         ...
         fc::enum_type<uint8_t, chain::transaction_receipt_header::status_enum>{bsp1->block->transactions[0].status},
         bsp1->block->transactions[0].cpu_usage_us, bsp1->block->transactions[0].net_usage_words,
         transfer_trx.get_signatures(), make_trx_header(transfer_trx.get_transaction())}}};
