...
fc::sha256 operator"" _h(const char* input, std::size_t) { return fc::sha256(input); }

chain::name operator"" _n(const char* input, std::size_t) { return chain::name(input); }

chain::asset operator"" _t(const char* input, std::size_t) { return chain::asset::from_string(input); }
...
auto make_block_state(chain::block_id_type previous, uint32_t height, uint32_t slot, chain::name producer,
   ...
   auto bsp                           = std::make_shared<chain::block_state>(
       std::move(pbhs), std::move(block), eosio::chain::deque<chain::transaction_metadata_ptr>(),
       chain::protocol_feature_set(),
       [](chain::block_timestamp_type timestamp, const fc::flat_set<chain::digest_type>& cur_features,
...
bool operator==(const authorization_trace_v0& lhs, const authorization_trace_v0& rhs) {
   return lhs.account == rhs.account && lhs.permission == rhs.permission;
}

bool operator==(const action_trace_v0& lhs, const action_trace_v0& rhs) {
   return lhs.global_sequence == rhs.global_sequence && lhs.receiver == rhs.receiver && lhs.account == rhs.account &&
          lhs.action == rhs.action && lhs.authorization == rhs.authorization && lhs.data == rhs.data;
}

bool operator==(const transaction_trace_v0& lhs, const transaction_trace_v0& rhs) {
   return lhs.id == rhs.id && lhs.actions == rhs.actions;
}

bool operator==(const transaction_trace_v2& lhs, const transaction_trace_v2& rhs) {
   return lhs.id == rhs.id && lhs.actions == rhs.actions && lhs.status == rhs.status &&
          lhs.cpu_usage_us == rhs.cpu_usage_us && lhs.net_usage_words == rhs.net_usage_words &&
          lhs.signatures == rhs.signatures && lhs.trx_header.expiration == rhs.trx_header.expiration &&
...
bool operator==(const block_trace_v0& lhs, const block_trace_v0& rhs) {
   return lhs.id == rhs.id && lhs.number == rhs.number && lhs.previous_id == rhs.previous_id &&
          lhs.timestamp == rhs.timestamp && lhs.producer == rhs.producer && lhs.transactions == rhs.transactions;
}

bool operator==(const block_trace_v2& lhs, const block_trace_v2& rhs) {
   return lhs.id == rhs.id && lhs.number == rhs.number && lhs.previous_id == rhs.previous_id &&
          lhs.timestamp == rhs.timestamp && lhs.producer == rhs.producer &&
          lhs.transaction_mroot == rhs.transaction_mroot && lhs.action_mroot == rhs.action_mroot &&
          lhs.schedule_version == rhs.schedule_version && lhs.transactions == rhs.transactions;
}
...
bool operator==(const block_entry_v0& lhs, const block_entry_v0& rhs) {
   return lhs.id == rhs.id && lhs.number == rhs.number && lhs.offset == rhs.offset;
}

bool operator!=(const block_entry_v0& lhs, const block_entry_v0& rhs) { return !(lhs == rhs); }

bool operator==(const lib_entry_v0& lhs, const lib_entry_v0& rhs) { return lhs.lib == rhs.lib; }

bool operator!=(const lib_entry_v0& lhs, const lib_entry_v0& rhs) { return !(lhs == rhs); }
...
bool operator==(const abi_def& lhs, const abi_def& rhs) { return fc::raw::pack(lhs) == fc::raw::pack(rhs); }

bool operator!=(const abi_def& lhs, const abi_def& rhs) { return !(lhs == rhs); }
