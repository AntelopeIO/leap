...
struct compressed_file_impl {
   ...
   void seek(uint64_t loc, fc::cfile& file) {
      ...
      if (seek_point_count > 0) {
         ...
         auto iter = std::lower_bound(seek_point_map.begin(), seek_point_map.end(), loc,
...
void compressed_file::seek(uint64_t loc) { impl->seek(loc, *file_ptr); }

void compressed_file::read(char* d, size_t n) { impl->read(d, n, *file_ptr); }
...
bool compressed_file::process(const fc::path& input_path, const fc::path& output_path, size_t seek_point_stride) {
   if (!fc::exists(input_path)) {
      throw std::ios_base::failure(std::string("Attempting to create compressed_file from file that does not exist: ") +
                                   input_path.generic_string());
   }

   const size_t input_size = fc::file_size(input_path);
   if (input_size == 0) {
      throw std::ios_base::failure(std::string("Attempting to create compressed_file from file that is empty: ") +
                                   input_path.generic_string());
   }
   ...
   if (seek_point_map.size() > 0) {
      output_file.write(reinterpret_cast<const char*>(seek_point_map.data()),
                        seek_point_map.size() * sizeof(seek_point_entry));
   }
