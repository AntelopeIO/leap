...
std::string to_iso8601_datetime(const fc::time_point& t) { return (std::string)t + "Z"; }
...
fc::variants process_authorizations(const std::vector<authorization_trace_v0>& authorizations,
   ...
   for (const auto& a : authorizations) {
      ...
      result.emplace_back(
          fc::mutable_variant_object()("account", a.account.to_string())("permission", a.permission.to_string()));
   }
...
fc::variants process_actions(const std::vector<ActionTrace>& actions, const data_handler_function& data_handler,
   ...
   for (int index : indices) {
      ...
      common_mvo("global_sequence", a.global_sequence)("receiver", a.receiver.to_string())(
          "account", a.account.to_string())("action", a.action.to_string())(
          "authorization", process_authorizations(a.authorization, yield))("data",
                                                                           fc::to_hex(a.data.data(), a.data.size()));
...
fc::variants process_transactions(const std::vector<TransactionTrace>& transactions,
                                  const data_handler_function& data_handler, const yield_function& yield) {
   fc::variants result;
   ...
   for (const auto& t : transactions) {
      ...
      if constexpr (std::is_same_v<TransactionTrace, transaction_trace_v0>) {
         result.emplace_back(fc::mutable_variant_object()("id", t.id.str())(
             "actions", process_actions<action_trace_v0>(t.actions, data_handler, yield)));
      } else {
         auto common_mvo = fc::mutable_variant_object();
         common_mvo("status", t.status)("cpu_usage_us", t.cpu_usage_us)("net_usage_words", t.net_usage_words)(
             "signatures", t.signatures)("transaction_header", t.trx_header);
         if constexpr (std::is_same_v<TransactionTrace, transaction_trace_v1>) {
            result.emplace_back(fc::mutable_variant_object()("id", t.id.str())(
                "actions", process_actions<action_trace_v0>(t.actions, data_handler, yield))(std::move(common_mvo)));
         } else if constexpr (std::is_same_v<TransactionTrace, transaction_trace_v2>) {
            result.emplace_back(fc::mutable_variant_object()("id", t.id.str())(
                "actions", process_actions<action_trace_v1>(std::get<std::vector<action_trace_v1>>(t.actions),
                                                            data_handler, yield))(std::move(common_mvo)));
         } else if constexpr (std::is_same_v<TransactionTrace, transaction_trace_v3>) {
            result.emplace_back(fc::mutable_variant_object()("id", t.id.str())("block_num", t.block_num)(
                "block_time", t.block_time)("producer_block_id", t.producer_block_id)(
                "actions", process_actions<action_trace_v1>(std::get<std::vector<action_trace_v1>>(t.actions),
                                                            data_handler, yield))(std::move(common_mvo)));
         }
      }
...
fc::variant response_formatter::process_block(const data_log_entry& trace, bool irreversible,
                                              const data_handler_function& data_handler, const yield_function& yield) {
   auto common_mvo = std::visit(
       [&](auto&& arg) -> fc::mutable_variant_object {
          return fc::mutable_variant_object()("id", arg.id.str())("number", arg.number)(
              "previous_id", arg.previous_id.str())("status", irreversible ? "irreversible" : "pending")(
              "timestamp", to_iso8601_datetime(arg.timestamp))("producer", arg.producer.to_string());
       },
       trace);
   if (std::holds_alternative<block_trace_v0>(trace)) {
      auto& block_trace = std::get<block_trace_v0>(trace);
      return fc::mutable_variant_object()(std::move(common_mvo))(
      ...
      return fc::mutable_variant_object()(std::move(common_mvo))("transaction_mroot", block_trace.transaction_mroot)(
          "action_mroot", block_trace.action_mroot)("schedule_version", block_trace.schedule_version)(
      ...
      auto  transactions = std::visit([&](auto&& arg) { return process_transactions(arg, data_handler, yield); },
                                     block_trace.transactions);
      return fc::mutable_variant_object()(std::move(common_mvo))("transaction_mroot", block_trace.transaction_mroot)(
          "action_mroot", block_trace.action_mroot)("schedule_version", block_trace.schedule_version)("transactions",
                                                                                                      transactions);
   } else {
