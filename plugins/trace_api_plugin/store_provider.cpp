...
static constexpr int         _max_filename_size    = std::char_traits<char>::length(_trace_index_prefix) + 10 + 1 + 10 +
                                          std::char_traits<char>::length(_compressed_trace_ext) +
                                          1; // "trace_index_" + 10-digits + '-' + 10-digits + ".clog" + null-char

std::string make_filename(const char* slice_prefix, const char* slice_ext, uint32_t slice_number,
                          uint32_t slice_width) {
   char           filename[_max_filename_size] = {};
   const uint32_t slice_start                  = slice_number * slice_width;
   const int      size_written = snprintf(filename, _max_filename_size, "%s%010d-%010d%s", slice_prefix, slice_start,
                                          (slice_start + slice_width), slice_ext);
   ...
   if (size_written >= _max_filename_size) {
      ...
      throw std::runtime_error(
          ...
          "format was changed and the code was not updated accordingly. Filename created: " +
          filename);
   }
...
store_provider::store_provider(const bfs::path& slice_dir, uint32_t stride_width,
...
get_block_t store_provider::get_block(uint32_t block_height, const yield_function& yield) {
   ...
   scan_metadata_log_from(
       block_height, 0,
       [&block_height, &trace_offset, &irreversible](const metadata_log_entry& e) -> bool {
...
get_block_n store_provider::get_trx_block_number(const chain::transaction_id_type& trx_id,
                                                 std::optional<uint32_t>           minimum_irreversible_history_blocks,
...
bool slice_directory::find_index_slice(uint32_t slice_number, open_state state, fc::cfile& index_file,
...
bool slice_directory::find_trace_slice(uint32_t slice_number, open_state state, fc::cfile& trace_file,
                                       bool open_file) const {
...
std::optional<compressed_file> slice_directory::find_compressed_trace_slice(uint32_t slice_number,
                                                                            bool     open_file) const {
...
bool slice_directory::find_slice(const char* slice_prefix, uint32_t slice_number, fc::cfile& slice_file,
                                 bool open_file) const {
...
void slice_directory::find_or_create_slice_pair(uint32_t slice_number, open_state state, fc::cfile& trace,
                                                fc::cfile& index) {
   ...
   if (trace_found != index_found) {
      ...
      elog("Trace file is ${ts}, but it's metadata file is ${is}. This means the files are not consistent.",
           ("ts", trace_status)("is", index_status));
   }
}

bool slice_directory::find_or_create_trx_id_slice(uint32_t slice_number, open_state state,
...
bool slice_directory::find_trx_id_slice(uint32_t slice_number, open_state state, fc::cfile& trx_id_file,
                                        bool open_file) const {
...
template <typename F>
void slice_directory::process_irreversible_slice_range(uint32_t lib, uint32_t min_irreversible,
                                                       std::optional<uint32_t>& lower_bound_slice, F&& f) {
...
void slice_directory::run_maintenance_tasks(uint32_t lib, const log_handler& log) {
   if (_minimum_irreversible_history_blocks) {
      process_irreversible_slice_range(
          lib, *_minimum_irreversible_history_blocks, _last_cleaned_up_slice, [this, &log](uint32_t slice_to_clean) {
             fc::cfile trace;
             fc::cfile index;
             fc::cfile trx_id;
   ...
   if (_minimum_uncompressed_irreversible_history_blocks &&
        ...
        *_minimum_uncompressed_irreversible_history_blocks < *_minimum_irreversible_history_blocks)) {
      process_irreversible_slice_range(
          lib, *_minimum_uncompressed_irreversible_history_blocks, _last_compressed_slice,
          [this, &log](uint32_t slice_to_compress) {
             fc::cfile  trace;
             const bool dont_open_file = false;
             ...
             if (trace_found) {
                ...
                compressed_file::process(trace.get_file_path(), compressed_path.generic_string(),
                                         _compression_seek_point_stride);
