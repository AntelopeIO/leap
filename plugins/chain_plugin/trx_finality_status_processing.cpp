...
void trx_finality_status_processing_impl::signal_applied_transaction(const chain::transaction_trace_ptr&  trace,
   ...
   if (iter != _storage.index().cend()) {
      ...
      _storage.insert(finality_status_object{ .trx_id          = trx_id,
...
void trx_finality_status_processing_impl::handle_rollback() {
   ...
   for (const auto& trx_iter : trxs) {
      _storage.modify(trx_iter, [](finality_status_object& obj) { obj.forked_out = true; });
   }
}
...
bool trx_finality_status_processing_impl::ensure_storage() {
   ...
   while (storage_to_free > 0) {
      ...
      if (oldest_block_iter == block_indx.end()) {
         ...
         for (; oldest_block_iter != block_indx.end() && oldest_block_iter->block_num() == block_num;
              ++oldest_block_iter) {
            reduce_storage(oldest_block_iter);
         }
         const auto oldest_failure_upper_bound =
            status_expiry_indx.upper_bound(std::make_tuple(false, block_timestamp.to_time_point()));
...
trx_finality_status_processing::chain_state trx_finality_status_processing::get_chain_state() const {
   return { .head_id                   = _my->_head_block_id,
            ...
            .irr_block_timestamp       = _my->_irr_block_timestamp,
            .earliest_tracked_block_id = _my->_earliest_tracked_block_id };
}
...
std::optional<trx_finality_status_processing::trx_state> trx_finality_status_processing::get_trx_state(
   ...
   return trx_finality_status_processing::trx_state{ .block_id        = iter->block_id,
                                                     .block_timestamp = iter->block_timestamp,
                                                     .received        = iter->received,
                                                     .expiration      = iter->trx_expiry,
                                                     .status          = status };
}
