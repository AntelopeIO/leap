...
using permission_info_index_t = multi_index_container<
   permission_info,
   indexed_by<ordered_unique<tag<by_owner_name>,
              ...
              ordered_non_unique<tag<by_last_updated_height>,
...
struct account_query_db_impl
{
   ...
   void add_to_bimaps(const permission_info& pi, const chain::permission_object& po)
   {
      ...
      for (const auto& k : po.auth.keys) {
         ...
         key_bimap.insert(key_bimap_t::value_type{
            {std::move(key), k.weight},
            pi
         });
      }
   }
   ...
   uint32_t last_updated_time_to_height(const fc::time_point& last_updated)
   {
      ...
      if (last_updated > lib_time) {
         ...
         EOS_ASSERT(iter != time_to_block_num.end(),
                    chain::plugin_exception,
   ...
   void rollback_to_before(const chain::block_state_ptr& bsp)
   {
      ...
      const auto& permission_by_owner =
         controller.db().get_index<chain::permission_index>().indices().get<chain::by_owner>();
   ...
   auto commit_block_prelock(const chain::block_state_ptr& bsp) const
   {
      ...
      auto process_trace = [&](const chain::transaction_trace_ptr& trace) {
         for (const auto& at : trace->action_traces) {
            if (std::tie(at.receiver, at.act.account) !=
                std::tie(chain::config::system_account_name, chain::config::system_account_name)) {
               continue;
            }
      ...
      const account_query_db::get_accounts_by_authorizers_params& args) const
   {
      ...
      auto push_results = [&result](const auto& begin, const auto& end) {
         for (auto itr = begin; itr != end; ++itr) {
            ...
            result.accounts.emplace_back(
               result_t::account_result{ pi.owner,
                                         pi.name,
      ...
      for (const auto& a : account_set) {
         if (a.permission.empty()) {
            ...
            const auto begin = name_bimap.left.lower_bound(
               weighted<chain::permission_level>::lower_bound_for({ a.actor, a.permission }));
            const auto next_account_name = chain::name(a.actor.to_uint64_t() + 1);
            const auto end               = name_bimap.left.lower_bound(
               weighted<chain::permission_level>::lower_bound_for({ next_account_name, a.permission }));
