...
public:
   action_result push_action(const account_name&   signer,
      ...
      return base_tester::push_action(std::move(act),
                                      (auth                         ? signer
                                       : signer == "bob111111111"_n ? "alice1111111"_n
                                                                    : "bob111111111"_n)
                                         .to_uint64_t());
   }
   ...
   read_only::get_account_results get_account_info(const account_name acct) {
      ...
      chain_apis::read_only         plugin(
         *(control.get()), {}, fc::microseconds::maximum(), fc::microseconds::maximum(), {}, {});
...
BOOST_FIXTURE_TEST_CASE(account_results_refund_request_test, chain_plugin_tester) {
   try {
      ...
      BOOST_CHECK_EQUAL(success(),
                        unstake(name("producer1111"),
                                name("producer1111"),
                                core_from_string("150000000.0000"),
                                core_from_string("0.0000")));
      results = get_account_info(name("producer1111"));
      BOOST_CHECK(results.total_resources.get_type() != fc::variant::type_id::null_type);
