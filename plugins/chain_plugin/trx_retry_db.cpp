...
struct tracked_transaction
{
   ...
   uint32_t ready_block_num() const
   {
      ...
      if (num_blocks == lib_totem)
         return std::numeric_limits<uint32_t>::max(); // lib at the end
...
using tracked_transaction_index_t = multi_index_container<
   tracked_transaction,
   indexed_by<
...
struct trx_retry_db_impl
{
   explicit trx_retry_db_impl(const chain::controller& controller,
      ...
      , _transaction_ack_channel(
           appbase::app().get_channel<chain::plugin_interface::compat::channels::transaction_ack>())
   ...
   size_t size() const { return _tracked_trxs.index().size(); }
      ...
      while (!idx.empty()) {
         ...
         itr->next(std::static_pointer_cast<fc::exception>(std::make_shared<expired_tx_exception>(
            FC_LOG_MESSAGE(error,
                           ...
                           ("id", itr->id())("e", itr->ptrx->expiration())("bt", block_timestamp)))));
