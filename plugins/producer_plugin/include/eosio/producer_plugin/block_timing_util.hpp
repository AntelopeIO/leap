...
struct block_timing_util {
   int32_t cpu_effort_us = 0;
   fc::time_point pending_block_deadline;
   ...
   fc::time_point production_round_block_start_time(chain::block_timestamp_type block_time) const {
   ...
   fc::time_point calculate_block_deadline(pending_block_mode mode, chain::block_timestamp_type block_time) {
      const auto hard_deadline =
                  block_time.to_time_point() + fc::microseconds(cpu_effort_us - chain::config::block_interval_us);
      if (mode == pending_block_mode::producing) {
         auto estimated_deadline = production_round_block_start_time(block_time) + fc::microseconds(cpu_effort_us);
         auto now                = fc::time_point::now();
         if (estimated_deadline > now) {
            pending_block_deadline = estimated_deadline;
         } else {
            ...
            pending_block_deadline = std::min(hard_deadline, now + fc::microseconds(cpu_effort_us));
         }
      } else {
         pending_block_deadline = hard_deadline;
      }
      return pending_block_deadline;
   }
};
} // namespace eosio