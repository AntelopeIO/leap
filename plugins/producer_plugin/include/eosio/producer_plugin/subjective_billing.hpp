...
private:
   ...
   uint32_t                      _expired_accumulator_average_window =
...
private:
   ...
   void remove_subjective_billing(const trx_cache_entry& entry, uint32_t time_ordinal) {
      ...
      if (aitr != _account_subjective_bill_cache.end()) {
         ...
         EOS_ASSERT(aitr->second.pending_cpu_us >= 0,
                    chain::tx_resource_exhaustion,
                    "Logic error in subjective account billing ${a}",
                    ("a", entry.account));
         if (aitr->second.empty(time_ordinal, _expired_accumulator_average_window))
            _account_subjective_bill_cache.erase(aitr);
      }
   }
   ...
   void transition_to_expired(const trx_cache_entry& entry, uint32_t time_ordinal) {
      ...
      if (aitr != _account_subjective_bill_cache.end()) {
         aitr->second.pending_cpu_us -= entry.subjective_cpu_bill;
         aitr->second.expired_accumulator.add(
            entry.subjective_cpu_bill, time_ordinal, _expired_accumulator_average_window);
      }
   }
...
public:
   ...
   void subjective_bill(const transaction_id_type& id,
   ...
   void subjective_bill_failure(const account_name&     first_auth,
      ...
      if (!_disabled && !_disabled_accounts.count(first_auth)) {
         ...
         _account_subjective_bill_cache[first_auth].expired_accumulator.add(
            bill, time_ordinal, _expired_accumulator_average_window);
      }
   }
   ...
   int64_t get_subjective_bill(const account_name& first_auth, const fc::time_point& now) const {
      ...
      if (sub_bill_info) {
         ...
         int64_t sub_bill =
            ...
            sub_bill_info->expired_accumulator.value_at(time_ordinal, _expired_accumulator_average_window);
         return sub_bill;
      } else {
         return 0;
      }
   ...
   bool remove_expired(fc::logger&           log,
      ...
      if (!idx.empty()) {
         ...
         while (!idx.empty()) {
            ...
            auto b = idx.begin();
            if (b->expiry > pending_block_time)
               break;
