...
auto make_unique_trx(const chain_id_type& chain_id) {
   ...
   trx.expiration =
      fc::time_point::now() + fc::seconds(nextid % 20 == 0 ? 0 : 60); // fail some transactions via expired
   ...
   try {
      ...
      auto                        ab = chain_plug->chain().accepted_block.connect([&](const block_state_ptr& bsp) {
         ...
         if (bsp->block->transactions.empty()) {
            --num_empty;
            if (num_empty == 0)
               empty_blocks_promise.set_value();
      ...
      for (size_t i = 1; i <= num_pushes; ++i) {
            ...
            app().get_method<plugin_interface::incoming::methods::transaction_async>()(
               ...
               [ptrx, &next_calls, &trace_with_except, &trx_match, &trxs, return_failure_traces](
                  ...
                  if (!std::holds_alternative<fc::exception_ptr>(result) &&
                      !std::get<chain::transaction_trace_ptr>(result)->except) {
         ...
         if (i % 200 == 0) {
            ...
            app().post(priority::high, []() { app().find_plugin<producer_plugin>()->get_integrity_hash(); });
         }
      }
