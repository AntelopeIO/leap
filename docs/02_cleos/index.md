...
`cleos` is a command line tool that interfaces with the REST API exposed by `nodeos`. Developers can also use `cleos` to deploy and test Mandel smart contracts.

## Installation

`cleos` is distributed as part of the [Mandel software suite](https://github.com/eosnetworkfoundation/mandel). To install `cleos` just visit the [Mandel Software Installation](../00_install/index.md) section.
...
```console
Command Line Interface to Mandel Client
