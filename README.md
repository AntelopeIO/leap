...
Both an HTTPS or SSH git clone will yield the same result- a folder named `leap` containg our source code. It doesn't matter which type you use.
...
Next, run the pinned build script. You have to give it three arguments, in the following order:
  ...
  - The number of jobs or CPU cores/threads to use (Note: check the [jobs flag](#step-3-build-the-source-code) warning above).
