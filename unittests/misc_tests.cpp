...
BOOST_AUTO_TEST_CASE(variant_format_string_limited) {
   const string format = "${a} ${b} ${c}";
   {
      ...
      BOOST_CHECK_EQUAL(result,
                        string(target_size, 'a') + "... " + string(target_size, 'b') + "... " +
                           string(target_size, 'c') + "...");
   }
   {
      fc::mutable_variant_object mu;
      signed_block               a;
   ...
   }
   {
      ...
      for (char ch = 'a'; ch < 'c'; ++ch) {
         provided_permissions.insert(
            { name(std::string_view(string(4, ch))), name(std::string_view(string(4, ch + 1))) });
      }
      flat_set<public_key_type> provided_keys;
      auto                      fill_keys = [](const flat_set<permission_level>& provided_permissions,
                          flat_set<public_key_type>&        provided_keys) {
      ...
      for (char ch = 'c'; ch < 'z'; ++ch) {
         provided_permissions.insert(
            { name(std::string_view(string(5, ch))), name(std::string_view(string(5, ch + 1))) });
      }
      fill_keys(provided_permissions, provided_keys);
      ...
      string target_fold_str = "transaction declares authority '" +
                               fc::json::to_string(*provided_permissions.begin(), fc::time_point::maximum());
...
BOOST_AUTO_TEST_CASE(asset_from_string_overflow) {
   ...
   BOOST_CHECK_EXCEPTION(asset::from_string("36.893488147419103232 CUR"),
                         overflow_exception,
                         [](const overflow_exception& e) { return true; });
   BOOST_CHECK_EXCEPTION(asset::from_string("-36.893488147419103232 CUR"),
                         underflow_exception,
                         [](const underflow_exception& e) { return true; });

   // precision = 14, magnitude > 2^76
   BOOST_CHECK_EXCEPTION(asset::from_string("1000000000.00000000000000 CUR"),
                         overflow_exception,
                         [](const overflow_exception& e) { return true; });
   BOOST_CHECK_EXCEPTION(asset::from_string("-1000000000.00000000000000 CUR"),
                         underflow_exception,
...
BOOST_AUTO_TEST_CASE(authority_checker) {
   try {
      ...
      A = authority(3,
                    ...
                    { permission_level_weight{ { name("hello"), name("world") }, 3 } });
      {
      ...
      auto GetAuthority = [d, e](const permission_level& perm) {
         if (perm.actor == name("top"))
            return authority(2,
                             ...
                             { permission_level_weight{ { name("bottom"), name("bottom") }, 1 } });
      ...
      A = authority(5,
                    ...
                    { permission_level_weight{ { name("top"), name("top") }, 5 } });
      {
...
BOOST_AUTO_TEST_CASE(transaction_test) {
   try {
      ...
      ((const signed_transaction&)trx)
         .sign(test.get_private_key(config::system_account_name, "active"), test.control->get_chain_id());
      BOOST_CHECK_EQUAL(0u, trx.signatures.size());
...
BOOST_AUTO_TEST_CASE(transaction_metadata_test) {
   try {
      ...
      packed_transaction_ptr ptrx =
         std::make_shared<packed_transaction>(trx, packed_transaction::compression_type::none);
      packed_transaction_ptr ptrx2 =
         std::make_shared<packed_transaction>(trx, packed_transaction::compression_type::zlib);
      ...
      auto fut2 = transaction_metadata::start_recover_keys(ptrx2,
                                                           thread_pool.get_executor(),
                                                           test.control->get_chain_id(),
                                                           fc::microseconds::maximum(),
                                                           transaction_metadata::trx_type::input);
      ...
      transaction_metadata::start_recover_keys(ptrx,
                                               ...
                                               fc::microseconds::maximum(),
                                               transaction_metadata::trx_type::input);
      transaction_metadata::start_recover_keys(ptrx2,
                                               thread_pool.get_executor(),
                                               test.control->get_chain_id(),
                                               fc::microseconds::maximum(),
                                               transaction_metadata::trx_type::input);
