...
BOOST_FIXTURE_TEST_CASE(missing_sigs, TESTER)
{
   try {
      ...
      BOOST_REQUIRE_THROW(push_reqauth("alice"_n,
                                       {
...
BOOST_FIXTURE_TEST_CASE(delegate_auth, TESTER)
{
   try {
      ...
      auto original_auth = static_cast<authority>(
         control->get_authorization_manager().get_permission({ "alice"_n, config::active_name }).auth);
      wdump((original_auth));

      set_authority("alice"_n, config::active_name, delegated_auth);

      auto new_auth = static_cast<authority>(
         control->get_authorization_manager().get_permission({ "alice"_n, config::active_name }).auth);
      wdump((new_auth));
...
BOOST_AUTO_TEST_CASE(update_auths)
{
   try {
      ...
      chain.set_authority(name("alice"),
                          name("active"),
                          authority(new_active_pub_key),
                          name("owner"),
                          {
                             permission_level{name("alice"), name("active")}
      },
                          { chain.get_private_key(name("alice"), "active") });
      chain.produce_blocks();
      ...
      BOOST_CHECK_THROW(chain.set_authority(name("alice"),
                                            name("spending"),
                                            authority(spending_pub_key),
                                            name("active"),
                                            {
      ...
      BOOST_CHECK_THROW(chain.set_authority(name("alice"),
                                            ...
                                            name("spending"),
                                            {
                                               permission_level{name("alice"), name("spending")}
      },
                                            { spending_priv_key }),
      ...
      chain.set_authority(name("alice"),
                          ...
                          name("trading"),
                          {
                             permission_level{name("alice"), name("trading")}
      },
                          { trading_priv_key });
      chain.produce_blocks();
      ...
      {
         const auto* trading =
            chain.find<permission_object, by_owner>(boost::make_tuple(name("alice"), name("trading")));
         const auto* spending =
            chain.find<permission_object, by_owner>(boost::make_tuple(name("alice"), name("spending")));
      ...
      BOOST_CHECK_THROW(chain.delete_authority(name("alice"),
                                               name("trading"),
                                               {
                                                  permission_level{name("alice"), name("active")}
      },
...
BOOST_AUTO_TEST_CASE(link_auths)
{
   try {
      ...
      BOOST_CHECK_THROW(chain.push_reqauth(name("alice"),
                                           {
      ...
      BOOST_CHECK_THROW(chain.link_authority(name("alice"), name("eosio"), name("spending"), name("reqauth")),
                        action_validate_exception);
...
BOOST_AUTO_TEST_CASE(link_then_update_auth)
{
   try {
      ...
      chain.push_reqauth(name("alice"),
                         {
                            permission_level{"alice"_n, name("first")}
      },
                         { first_priv_key });
...
BOOST_AUTO_TEST_CASE(create_account)
{
   try {
      ...
      const auto& joe_owner_authority =
         chain.get<permission_object, by_owner>(boost::make_tuple(name("joe"), name("owner")));
      ...
      BOOST_TEST(joe_owner_authority.auth.keys[0].key.to_string() ==
                 chain.get_public_key(name("joe"), "owner").to_string());
      BOOST_TEST(joe_owner_authority.auth.keys[0].weight == 1u);

      const auto& joe_active_authority =
         chain.get<permission_object, by_owner>(boost::make_tuple(name("joe"), name("active")));
      ...
      BOOST_TEST(joe_active_authority.auth.keys[0].key.to_string() ==
                 chain.get_public_key(name("joe"), "active").to_string());
      ...
      BOOST_CHECK_EXCEPTION(
         chain.create_account(name("eosio.test2"), name("joe")),
         action_validate_exception,
...
BOOST_AUTO_TEST_CASE(any_auth)
{
   try {
      ...
      BOOST_REQUIRE_THROW(chain.push_reqauth(name("alice"),
                                             {
                                                permission_level{"alice"_n, name("spending")}
      },
                                             { spending_priv_key }),
      ...
      BOOST_REQUIRE_THROW(chain.push_reqauth(name("alice"),
                                             {
                                                permission_level{"bob"_n, name("spending")}
      },
                                             { spending_priv_key }),
                          missing_auth_exception);
