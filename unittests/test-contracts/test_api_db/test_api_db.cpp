...
void test_api_db::test_invalid_access(name _code, uint64_t val, uint32_t index, bool store) {
   ...
   uint64_t value = 0;
   switch (index) {
      case 1: itr = db_idx64_find_primary(code, scope, table, &value, pk); break;
      case 0:
      default: itr = db_find_i64(code, scope, table, pk); break;
   }
   if (store) {
      uint64_t value_to_store = val;
      if (itr < 0) {
         switch (index) {
            case 1: db_idx64_store(scope, table, receiver, pk, &value_to_store); break;
            case 0:
            default: db_store_i64(scope, table, receiver, pk, &value_to_store, sizeof(value_to_store)); break;
         }
      } else {
         switch (index) {
            case 1: db_idx64_update(itr, receiver, &value_to_store); break;
            case 0:
            default: db_update_i64(itr, receiver, &value_to_store, sizeof(value_to_store)); break;
         }
      }
...
void test_api_db::idx_double_nan_lookup_fail(uint32_t lookup_type) {
   ...
   switch (lookup_type) {
      ...
      case 2: // upper bound
         ...
         break;
      default: eosio_assert(false, "idx_double_nan_lookup_fail: unexpected lookup_type");
   }
}
