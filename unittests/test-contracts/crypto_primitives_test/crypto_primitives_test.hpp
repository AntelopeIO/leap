...
public:
   ...
   [[eosio::action]] void testadd(const bytes& op1,
                                  const bytes& op2,
                                  int32_t      expected_error,
                                  const bytes& expected_result);

   [[eosio::action]] void testmul(const bytes& point,
                                  const bytes& scalar,
                                  int32_t      expected_error,
                                  const bytes& expected_result);

   [[eosio::action]] void testpair(const bytes& g1_g2_pairs, int32_t expected_error);

   [[eosio::action]] void testmodexp(const bytes& base,
                                     const bytes& exp,
                                     const bytes& modulo,
                                     int32_t      expected_error,
                                     const bytes& expected_result);

   [[eosio::action]] void testblake2f(uint32_t     rounds,
                                      const bytes& state,
                                      const bytes& message,
                                      const bytes& t0,
                                      const bytes& t1,
   ...
   [[eosio::action]] void testsha3(const bytes& input, const bytes& expected_result);

   [[eosio::action]] void testkeccak(const bytes& input, const bytes& expected_result);

   [[eosio::action]] void testecrec(const bytes& signature,
                                    const bytes& digest,
                                    int32_t      expected_error,
                                    const bytes& expected_result);
};
