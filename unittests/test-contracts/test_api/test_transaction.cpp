...
{
   static eosio::name get_account() { return eosio::name{ ACCOUNT }; }

   static eosio::name get_name() { return eosio::name{ NAME }; }
...
void test_transaction::send_action()
{
   ...
   action act(permissions,
              name{ "testapi" },
              name{ WASM_TEST_ACTION("test_action", "read_action_normal") },
              test_action);
...
void test_transaction::send_action_empty()
{
   ...
   action act(
      permissions, name{ "testapi" }, name{ WASM_TEST_ACTION("test_action", "assert_true") }, test_action);
...
void test_transaction::send_action_recurse()
{
   ...
   test_action_action<"testapi"_n.value, WASM_TEST_ACTION("test_transaction", "send_action_recurse")>
      test_action;
   ...
   action act(permissions,
              name{ "testapi" },
              name{ WASM_TEST_ACTION("test_transaction", "send_action_recurse") },
              test_action);
...
void test_transaction::send_action_inline_fail()
{
   ...
   action act(
      permissions, name{ "testapi" }, name{ WASM_TEST_ACTION("test_action", "assert_false") }, test_action);
...
void test_transaction::send_transaction(uint64_t receiver, uint64_t, uint64_t)
{
   ...
   trx.actions.emplace_back(permissions,
                            name{ "testapi" },
                            name{ WASM_TEST_ACTION("test_action", "read_action_normal") },
                            test_action);
...
void test_transaction::send_action_sender(uint64_t receiver, uint64_t, uint64_t)
{
   ...
   trx.actions.emplace_back(permissions,
                            name{ "testapi" },
                            name{ WASM_TEST_ACTION("test_action", "test_current_sender") },
                            &cur_send);
   trx.send(0, name{ receiver });
}
...
void test_transaction::assert_false_error_handler(const eosio::transaction& dtrx)
{
   ...
   eosio_assert(dtrx.actions[0].authorization[0].actor == "testapi"_n,
                "action's authorization has wrong actor");
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t)
{
   ...
   for (int i = 0; i < 32; i++) {
      ...
      test_action_action<"testapi"_n.value, WASM_TEST_ACTION("test_action", "read_action_normal")>
         test_action;
...
void test_transaction::send_deferred_transaction(uint64_t receiver, uint64_t, uint64_t)
{
   ...
   trx.actions.emplace_back(permissions,
                            name{ "testapi" },
                            name{ WASM_TEST_ACTION("test_transaction", "deferred_print") },
                            test_action);
   trx.delay_sec = 2;
...
void test_transaction::send_deferred_transaction_4k_action(uint64_t receiver, uint64_t, uint64_t)
{
   ...
   trx.actions.emplace_back(permissions,
                            name{ "testapi" },
                            name{ WASM_TEST_ACTION("test_transaction", "send_action_4k") },
                            test_action);
   trx.delay_sec = 2;
   trx.send(0xffffffffffffffff, name{ receiver });
}
...
void test_transaction::send_deferred_tx_with_dtt_action()
{
   ...
   deferred_act.authorization = std::vector<permission_level>{
      {"testapi"_n, name{ dtt_act.permission_name }}
   };
...
{
   char buf[4] = { 1 };
   db_store_i64(eosio::name{ "testtrans" }.value,
                eosio::name{ "table" }.value,
                eosio::name{ "testtrans" }.value,
                0,
                buf,
                4);
}
