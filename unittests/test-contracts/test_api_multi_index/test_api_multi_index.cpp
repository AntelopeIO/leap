...
void idx64_check_without_storing(name receiver)
{
   ...
   {
      std::array<uint64_t, 6> pks{
         {976, 234, 781, 540, 650, 265}
      };
   ...
   // modify and erase
   {
      ...
      table.modify(new_person, payer, [&](auto& r) { r.sec = "billy"_n.value; });
...
void idx128_check_without_storing(name receiver)
{
   ...
   table.modify(table.get(3), payer, [&](auto& r) { r.sec *= 2; });
