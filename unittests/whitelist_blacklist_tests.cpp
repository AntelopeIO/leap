...
public:
   ...
   transaction_trace_ptr transfer(account_name from, account_name to, string quantity = "1.00 TOK") {
      return chain->push_action(
         "eosio.token"_n, "transfer"_n, from, mvo()("from", from)("to", to)("quantity", quantity)("memo", ""));
   }
...
BOOST_AUTO_TEST_CASE(contract_whitelist) {
   try {
      ...
      test.chain->push_action("bob"_n, "create"_n, "bob"_n, mvo()("issuer", "bob")("maximum_supply", "1000000.00 CUR"));

      BOOST_CHECK_EXCEPTION(
         test.chain->push_action(
            "charlie"_n, "create"_n, "charlie"_n, mvo()("issuer", "charlie")("maximum_supply", "1000000.00 CUR")),
         contract_whitelist_exception,
...
BOOST_AUTO_TEST_CASE(deferred_blacklist_failure) {
   try {
      ...
      tester1.chain->push_action("bob"_n,
                                 "defercall"_n,
                                 "alice"_n,
...
BOOST_AUTO_TEST_CASE(actor_blacklist_inline_deferred) {
   try {
      ...
      tester1.chain->push_action("eosio"_n,
                                 "updateauth"_n,
                                 "alice"_n,
      ...
      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action(
            "alice"_n, "inlinecall"_n, "alice"_n, mvo()("contract", "alice")("authorizer", "bob")("payload", 10)),
         fc::exception,
      ...
      // With charlie now in the actor blacklist, retiring the previously scheduled deferred transaction should now not
      // be possible.
      BOOST_CHECK_EXCEPTION(
         tester1.chain->produce_blocks(),
         fc::exception,
      ...
      // With charlie now in the actor blacklist, it is now not possible to schedule a deferred transaction authorized
      // by charlie@active
      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action("charlie"_n,
                                    "defercall"_n,
...
BOOST_AUTO_TEST_CASE(blacklist_sender_bypass) {
   try {
      ...
      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action("bob"_n, "deferfunc"_n, "bob"_n, mvo()("payload", 10)),
         fc::exception,
         fc_exception_message_is("authorizing actor(s) in transaction are on the actor blacklist: [\"bob\"]"));
      ...
      // This is allowed due to the fact that the sender of the deferred transaction (also charlie) is in the sender
      // bypass list.
      tester1.chain->produce_blocks();
      ...
      // Now retire the deferred transaction successfully despite charlie being on both the actor blacklist and bob
      // being on the contract blacklist This is allowed due to the fact that the sender of the deferred transaction
      // (also charlie) is in the sender bypass list.
      tester1.chain->produce_blocks();
...
BOOST_AUTO_TEST_CASE(greylist_limit_tests) {
   try {
      ...
      c.push_action(
         ...
         "setalimits"_n,
         config::system_account_name,
         fc::mutable_variant_object()("account", user_account)("ram_bytes", -1)("net_weight", 1)("cpu_weight", 1));

      c.push_action(config::system_account_name,
                    "setalimits"_n,
                    config::system_account_name,
                    fc::mutable_variant_object()("account", other_account)("ram_bytes", -1)("net_weight", 249'999'999)(
