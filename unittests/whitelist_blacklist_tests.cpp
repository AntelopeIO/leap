...
public:
   ...
   void init(bool bootstrap = true)
   {
      ...
      chain->push_action("eosio.token"_n,
                         "create"_n,
                         "eosio.token"_n,
                         mvo()("issuer", "eosio.token")("maximum_supply", "1000000.00 TOK"));
      chain->push_action("eosio.token"_n,
                         "issue"_n,
                         "eosio.token"_n,
                         mvo()("to", "eosio.token")("quantity", "1000000.00 TOK")("memo", "issue"));
   ...
   transaction_trace_ptr transfer(account_name from, account_name to, string quantity = "1.00 TOK")
   {
      return chain->push_action("eosio.token"_n,
                                "transfer"_n,
                                from,
                                mvo()("from", from)("to", to)("quantity", quantity)("memo", ""));
   }
   ...
   try {
      ...
      trx.actions.emplace_back(
         ...
         fc::raw::pack(transfer_args{
            .from = "alice"_n, .to = "bob"_n, .quantity = asset::from_string("10.00 TOK"), .memo = "" }));
      ...
      BOOST_CHECK_EXCEPTION(
         test.chain->push_transaction(trx),
         actor_whitelist_exception,
...
BOOST_AUTO_TEST_CASE(contract_whitelist)
{
   try {
      ...
      test.chain->push_action(
         "bob"_n, "create"_n, "bob"_n, mvo()("issuer", "bob")("maximum_supply", "1000000.00 CUR"));

      BOOST_CHECK_EXCEPTION(
         test.chain->push_action("charlie"_n,
                                 "create"_n,
                                 "charlie"_n,
                                 mvo()("issuer", "charlie")("maximum_supply", "1000000.00 CUR")),
         contract_whitelist_exception,
...
BOOST_AUTO_TEST_CASE(action_blacklist)
{
   try {
      ...
      test.action_blacklist   = {
           {"charlie"_n, "create"_n}
      };
      test.init();
...
BOOST_AUTO_TEST_CASE(deferred_blacklist_failure)
{
   try {
      ...
      tester1.chain->push_action(
         "bob"_n,
         "defercall"_n,
         "alice"_n,
         mvo()("payer", "alice")("sender_id", 0)("contract", "charlie")("payload", 10));
      ...
      tester1.chain->push_action(
         ...
         "defercall"_n,
         "alice"_n,
         mvo()("payer", "alice")("sender_id", 1)("contract", "charlie")("payload", 10));

      BOOST_CHECK_EXCEPTION(tester1.chain->produce_blocks(),
                            fc::exception,
                            fc_exception_message_is("account 'charlie' is on the contract blacklist"));
...
BOOST_AUTO_TEST_CASE(blacklist_onerror)
{
   try {
      ...
      tester1.chain->push_action(
         ...
         "defercall"_n,
         "alice"_n,
         mvo()("payer", "alice")("sender_id", 0)("contract", "charlie")("payload", 13));

      tester1.chain->produce_blocks();
      tester1.shutdown();

      tester1.action_blacklist = {
         {config::system_account_name, "onerror"_n}
      };
      tester1.init(false);
...
BOOST_AUTO_TEST_CASE(actor_blacklist_inline_deferred)
{
   try {
      ...
      auth.accounts.push_back(permission_level_weight{
         {"alice"_n, config::eosio_code_name},
         1
      });
      ...
      tester1.chain->push_action(
         ...
         "updateauth"_n,
         "alice"_n,
         mvo()("account", "alice")("permission", "active")("parent", "owner")("auth", auth));
      ...
      tester1.chain->push_action(
         ...
         "updateauth"_n,
         "bob"_n,
         mvo()("account", "bob")("permission", "active")("parent", "owner")("auth", auth));

      auth = authority(eosio::testing::base_tester::get_public_key(name("charlie"), "active"));
      auth.accounts.push_back(permission_level_weight{
         {"charlie"_n, config::eosio_code_name},
         1
      });
      ...
      tester1.chain->push_action(
         ...
         "updateauth"_n,
         "charlie"_n,
         mvo()("account", "charlie")("permission", "active")("parent", "owner")("auth", auth));
      ...
      auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const packed_transaction_ptr&> x) {
         auto& t = std::get<0>(x);
         if (!t || t->action_traces.size() == 0)
            return;

         const auto& act = t->action_traces[0].act;
         if (act.account == "eosio"_n && act.name == "onblock"_n)
            return;
      ...
      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action("alice"_n,
                                    "inlinecall"_n,
                                    "alice"_n,
                                    mvo()("contract", "alice")("authorizer", "bob")("payload", 10)),
         fc::exception,
         fc_exception_message_is(
            "authorizing actor(s) in transaction are on the actor blacklist: [\"bob\"]"));

      auto num_deferred =
         tester1.chain->control->db().get_index<generated_transaction_multi_index, by_trx_id>().size();
      ...
      // With charlie now in the actor blacklist, retiring the previously scheduled deferred transaction
      // should now not be possible.
      BOOST_CHECK_EXCEPTION(
         tester1.chain->produce_blocks(),
         fc::exception,
         fc_exception_message_is(
      ...
      // With charlie now in the actor blacklist, it is now not possible to schedule a deferred transaction
      // authorized by charlie@active
      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action(
            "charlie"_n,
            "defercall"_n,
...
BOOST_AUTO_TEST_CASE(blacklist_sender_bypass)
{
   try {
      ...
      BOOST_CHECK_EXCEPTION(tester1.chain->push_action("bob"_n, "deferfunc"_n, "bob"_n, mvo()("payload", 10)),
                            fc::exception,
                            fc_exception_message_is(
                               "authorizing actor(s) in transaction are on the actor blacklist: [\"bob\"]"));

      BOOST_CHECK_EXCEPTION(
         tester1.chain->push_action("charlie"_n, "deferfunc"_n, "charlie"_n, mvo()("payload", 10)),
         fc::exception,
         fc_exception_message_is(
            "authorizing actor(s) in transaction are on the actor blacklist: [\"charlie\"]"));
      ...
      BOOST_CHECK_EXCEPTION(tester1.chain->push_action(
                               "bob"_n,
                               "defercall"_n,
                               "alice"_n,
                               mvo()("payer", "alice")("sender_id", 0)("contract", "bob")("payload", 10)),
                            fc::exception,
                            fc_exception_message_is(
                               "authorizing actor(s) in transaction are on the actor blacklist: [\"bob\"]"));
      ...
      // This is allowed due to the fact that the sender of the deferred transaction (also charlie) is in the
      // sender bypass list.
      tester1.chain->produce_blocks();
      ...
      tester1.chain->push_action("charlie"_n,
                                 "defercall"_n,
                                 "alice"_n,
                                 mvo()("payer", "alice")("sender_id", 1)("contract", "bob")("payload", 10));
      ...
      // Now retire the deferred transaction successfully despite charlie being on both the actor blacklist
      // and bob being on the contract blacklist This is allowed due to the fact that the sender of the
      // deferred transaction (also charlie) is in the sender bypass list.
      tester1.chain->produce_blocks();
      ...
      // Ensure that if there if the sender is not on the sender bypass list, then the contract blacklist is
      // enforced.
      BOOST_CHECK_EXCEPTION(tester1.chain->produce_blocks(),
                            fc::exception,
...
BOOST_AUTO_TEST_CASE(greylist_limit_tests)
{
   try {
      ...
      cfg.min_transaction_cpu_usage =
         100; // Empty blocks (consisting of only onblock) would be below the target.
      ...
      c.push_action(config::system_account_name,
                    ...
                    fc::mutable_variant_object()("account", user_account)("ram_bytes", -1)("net_weight", 1)(
                       "cpu_weight", 1));

      c.push_action(config::system_account_name,
                    "setalimits"_n,
                    config::system_account_name,
                    fc::mutable_variant_object()("account", other_account)("ram_bytes", -1)(
                       "net_weight", 249'999'999)("cpu_weight", 249'999'999));

      const int64_t reqauth_net_charge = 104;
      auto          push_reqauth       = [&](name acnt, name perm, uint32_t billed_cpu_time_us) {
         signed_transaction trx;
         trx.actions.emplace_back(c.get_action(config::system_account_name,
                                               "reqauth"_n,
      ...
      BOOST_REQUIRE_EXCEPTION(push_reqauth(user_account, config::active_name, cfg.min_transaction_cpu_usage),
      ...
      // Finally, dropping the greylist limit to 1 will restrict the user's NET bandwidth so much that this
      // user cannot push even a single reqauth just like when they were under full congestion. However, this
      // time the exception will be due to greylist_net_usage_exceeded rather than tx_net_usage_exceeded.
