...
auto create_test_block_state(deque<transaction_metadata_ptr> trx_metas) {
   ...
   producer_authority_schedule schedule = {
      0, { producer_authority{ block->producer, block_signing_authority_v0{ 1, { { pub_key, 1 } } } } }
   };
...
BOOST_AUTO_TEST_CASE(unapplied_transaction_queue_erase_add) try {
   ...
   while (count && itr != end) {
      ...
      if (count == 6)
         BOOST_CHECK(trx_meta == trx1);
      if (count == 5)
         BOOST_CHECK(trx_meta == trx2);
      if (count == 4)
         BOOST_CHECK(trx_meta == trx3);
      if (count == 3)
         BOOST_CHECK(trx_meta == trx4);
      if (count == 2)
         BOOST_CHECK(trx_meta == trx5);
      if (count == 1)
         BOOST_CHECK(trx_meta == trx6);
      itr = q.erase(itr);
