...
public:
                ...
                abi_serializer::create_yield_function(abi_serializer_max_time))
   {
      ...
      auto result =
         push_action("eosio.token"_n,
                     "create"_n,
                     mutable_variant_object()("issuer", eosio_token)("maximum_supply", "1000000000.0000 CUR")(
      ...
      result = push_action(
         "eosio.token"_n,
         "issue"_n,
         mutable_variant_object()("to", eosio_token)("quantity", "1000000.0000 CUR")("memo", "gggggggggggg"));
      wdump((result));
      produce_block();
   }
...
try {
   ...
   // invalid - contains lower case characters, no validation
   {
      BOOST_CHECK_EXCEPTION(
         symbol malformed(SY(6, EoS)), symbol_type_exception, fc_exception_message_is("invalid symbol: EoS"));
   }
...
try {
   ...
   // set up proxy owner
   {
      ...
      setowner_act.authorization = vector<permission_level>{
         {"proxy"_n, config::active_name}
      };
      setowner_act.data =
         proxy_abi_ser.variant_to_binary("setowner",
                                         mutable_variant_object()("owner", "alice")("delay", 10),
                                         abi_serializer::create_yield_function(abi_serializer_max_time));
...
try {
   ...
   // set up proxy owner
   {
      ...
      setowner_act.data =
         proxy_abi_ser.variant_to_binary("setowner",
                                         mutable_variant_object()("owner", "bob")("delay", 10),
                                         abi_serializer::create_yield_function(abi_serializer_max_time));
      trx.actions.emplace_back(std::move(setowner_act));
   ...
   // set up alice owner
   {
      ...
      setowner_act.data =
         proxy_abi_ser.variant_to_binary("setowner",
                                         mutable_variant_object()("owner", "alice")("delay", 0),
                                         abi_serializer::create_yield_function(abi_serializer_max_time));
      trx.actions.emplace_back(std::move(setowner_act));
