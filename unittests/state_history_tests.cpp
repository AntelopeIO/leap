...
BOOST_AUTO_TEST_CASE(test_deltas_account_permission_modification) {
   ...
   for (auto& key : keys) {
      ...
      if (key.which() != K1_storage_type_which_value)
         ...
         BOOST_REQUIRE_EQUAL(public_key_to_string(accounts_permissions[0].auth.keys[0].key),
...
BOOST_AUTO_TEST_CASE(test_deltas_permission_link) {
   ...
   chain.push_reqauth("newacc"_n,
                      {
                         permission_level{"newacc"_n, "spending"_n}
   },
                      { spending_priv_key });
   ...
   auto permission_links =
      chain.deserialize_data<eosio::ship_protocol::permission_link_v0, eosio::ship_protocol::permission_link>(
         it_permission_link);
...
BOOST_AUTO_TEST_CASE(test_deltas_global_property_history) {
   ...
   auto global_properties =
      chain.deserialize_data<eosio::ship_protocol::global_property_v1, eosio::ship_protocol::global_property>(
         it_global_property);
...
BOOST_AUTO_TEST_CASE(test_deltas_protocol_feature_history) {
   ...
   for (size_t i = 0; i < digest_byte_array.size(); i++)
      digest_array[i] = digest_byte_array[i];
...
BOOST_AUTO_TEST_CASE(test_deltas_contract) {
   ...
   auto contract_tables =
      chain.deserialize_data<eosio::ship_protocol::contract_table_v0, eosio::ship_protocol::contract_table>(
         it_contract_table);
...
BOOST_AUTO_TEST_CASE(test_deltas_resources_history) {
   ...
   trx.actions.emplace_back(chain.get_action(config::system_account_name,
                                             "buyram"_n,
                                             vector<permission_level>{
                                                {config::system_account_name, config::active_name}
   },
                                             mutable_variant_object()("payer", config::system_account_name)(
                                                "receiver", "alice"_n)("quant", core_from_string("1.0000"))));

   trx.actions.emplace_back(
      chain.get_action(config::system_account_name,
                       "delegatebw"_n,
                       vector<permission_level>{
                          {config::system_account_name, config::active_name}
   },
                       mutable_variant_object()("from", config::system_account_name)("receiver", "alice"_n)(
