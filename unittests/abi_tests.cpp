...
struct act_sig
{
   ...
   static action_name get_name() { return "act"_n; }
};
                                              ...
                                              const fc::variant&    var)
{
   try {
      ...
      abi_serializer::from_variant(
         var, obj, get_resolver(), abi_serializer::create_yield_function(max_serialization_time));

      fc::variant var2;
      abi_serializer::to_variant(
         obj, var2, get_resolver(), abi_serializer::create_yield_function(max_serialization_time));
...
BOOST_AUTO_TEST_CASE(general)
{
   try {
      ...
      const char* my_other = R"=====(
    {
      ...
      verify_byte_round_trip_conversion(
         abi_serializer{ abi, abi_serializer::create_yield_function(max_serialization_time) }, "A", var);
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(abi_cycle)
{
      ...
      const char* struct_cycle_abi = R"=====(
   {
      ...
      abi_serializer abis;
      BOOST_CHECK_EXCEPTION(abis.set_abi(abi, abi_serializer::create_yield_function(max_serialization_time)),
   ...
   try {
      ...
      txn.context_free_actions.emplace_back(
         vector<permission_level>{
            {"testapi1"_n, config::active_name}
      },
      ...
      txn.context_free_actions.emplace_back(
         vector<permission_level>{
            {"testapi2"_n, config::active_name}
      },
         action1{ 15, 23, (uint8_t)3 });
      txn.actions.emplace_back(
         vector<permission_level>{
            {"testapi3"_n, config::active_name}
      },
         action2{ 42, 67, (uint8_t)1 });
      txn.actions.emplace_back(
         vector<permission_level>{
            {"testapi4"_n, config::active_name}
      },
...
BOOST_AUTO_TEST_CASE(abi_type_nested_in_vector)
{
      ...
      const char* repeat_abi = R"=====(
   {
     ...
     "tables": []
      ...
      BOOST_CHECK_THROW(abi_serializer abis(fc::json::from_string(repeat_abi).as<abi_def>(),
                                            abi_serializer::create_yield_function(max_serialization_time)),
                        fc::exception);
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"eosio", "action":"hi",
      // "binargs":"ffffffff08"}'
...
BOOST_AUTO_TEST_CASE(abi_is_type_recursion)
{
   try {
      ...
      BOOST_CHECK_THROW(abi_serializer abis(fc::json::from_string(abi_str).as<abi_def>(),
                                            abi_serializer::create_yield_function(max_serialization_time)),
                        fc::exception);
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(abi_large_signature)
{
   try {
    ...
    "variants": []
      ...
      signature_type::storage_type webauth_sig =
      ...
      try {
         abi_serializer::to_variant(large_act,
                                    var,
                                    get_resolver(fc::json::from_string(abi_str).as<abi_def>()),
...
BOOST_AUTO_TEST_CASE(variants)
{
   ...
   try {
      ...
      abi_serializer abis(fc::variant(fc::raw::unpack<abi_def>(bin)).as<abi_def>(),
                          abi_serializer::create_yield_function(max_serialization_time));
   ...
   try {
      ...
      BOOST_CHECK_EXCEPTION(
         abis.variant_to_binary("very_very_very_very_very_very_very_very_very_very_long_struct_name_s3",
                                ...
                                true),
         pack_exception,
         fc_exception_message_is("Missing field 'i1' in input object while processing struct "
                                                                  ...
                                                                  bool                    parsable = true)
{
   ...
   at.act                                        = eosio::chain::action(
      std::vector<eosio::chain::permission_level>{
                                                eosio::chain::permission_level{account_name{ "acctest" }, permission_name{ "active" }}
   },
   ...
   if (return_value && parsable) {
      if (std::is_same<T, std::string>::value) {
         expected_json << ",\"return_value_data\":\"" << *return_value << "\"";
      } else {
         expected_json << ",\"return_value_data\":" << *return_value;
      }
   }
...
BOOST_AUTO_TEST_CASE(abi_to_variant__add_action__bad_return_value)
{
   ...
   std::tie(at, expected_json) =
      generate_action_trace(std::optional<std::string>{ "no return" }, "096e6f2072657475726e", false);
