...
struct block_ref
{
   ...
   block_num_type block_num() const { return eosio::chain::block_header::num_from_id(block_id); }
};
...
struct core
{
   ...
   block_num_type          final_on_strong_qc_block_num_;

   // greg
   block_time_type last_qc_block_timestamp() const {
      const block_ref& ref =  get_block_reference(links.back().target_block_num);
      return ref.timestamp;
   }

   // greg
   block_num_type last_qc_block_num() const {
      return links.back().target_block_num;
   }

   // greg
   block_num_type final_on_strong_qc_block_num() const {
      return final_on_strong_qc_block_num_;
   }
   ...
   static core create_core_for_genesis_block(block_num_type block_num)
   {
      return core {
         ...
         .refs                         = {},
         .final_on_strong_qc_block_num_ = block_num,
      };
   ...
   core next(const block_ref& current_block, const qc_claim& most_recent_ancestor_with_qc) const
   {
      ...
      auto new_block_nums = [&]() -> std::tuple<block_num_type, block_num_type, block_num_type>
         {
            ...
            if (!most_recent_ancestor_with_qc.is_strong_qc) {
               return {last_final_block_num(), links.front().source_block_num, final_on_strong_qc_block_num_};
            }
            ...
            assert(final_on_strong_qc_block_num_ <= link1.target_block_num); // TODO: Show that this is always true.
      ...
      assert(final_on_strong_qc_block_num_ <= new_final_on_strong_qc_block_num); // Satisfied by justifications in new_block_nums.

      next_core.final_on_strong_qc_block_num_ = new_final_on_strong_qc_block_num;
...
struct core
{
   ...
   void check_invariants() {
      ...
      assert(last_final_block_num() <= final_on_strong_qc_block_num_ &&            // 2.
             final_on_strong_qc_block_num_ <= latest_qc_claim().block_num);
      if (refs.empty()) {                                                         // 3.
         assert(links.size() == 1);
      } else {
         assert(all_equal(links.back().target_block_num,                          // 3.
                          links.back().source_block_num,
                          final_on_strong_qc_block_num_,
   ...
   core next(const block_ref& current_block, const qc_claim& most_recent_ancestor_with_qc) const
   {
      ...
      auto new_block_nums = [&]() -> std::pair<block_num_type, block_num_type>
         {
            assert(last_final_block_num() <= final_on_strong_qc_block_num_); // Satisfied by invariant 2.

            if (!most_recent_ancestor_with_qc.is_strong_qc) {
               return {last_final_block_num(), final_on_strong_qc_block_num_};
            }
