...
struct deep_mind_log_fixture
{
   ...
   deep_mind_log_fixture()
   {
      ...
      cfg.appenders.push_back(
         appender_config("deep-mind",
                         "dmlog",
                         mutable_variant_object()("file", log_output.path().preferred_string().c_str())));
...
BOOST_FIXTURE_TEST_CASE(deep_mind, deep_mind_tester)
{
   ...
   push_action(config::system_account_name,
               "updateauth"_n,
               "alice"_n,
               fc::mutable_variant_object()("account", "alice")("permission", "test1")("parent", "active")(
                  "auth",
                  authority{
                     {"eosio"_n, "active"_n}
   }));
