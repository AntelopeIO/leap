...
BOOST_FIXTURE_TEST_CASE(accounts_exists, tester) {
   try {
      ...
      const auto& nobody_active_authority =
         chain1_db.get<permission_object, by_owner>(boost::make_tuple(config::null_account_name, config::active_name));
      ...
      const auto& nobody_owner_authority =
         chain1_db.get<permission_object, by_owner>(boost::make_tuple(config::null_account_name, config::owner_name));
      ...
      for (size_t i = 0; i < std::max(active_auth.size(), active_producers.producers.size()); ++i) {
         ...
         account_name n2 = i < active_producers.producers.size() ? active_producers.producers[i].producer_name
                                                                 : (account_name)0;
         if (n1 != n2)
            diff.push_back(name(n2.to_uint64_t() - n1.to_uint64_t()));
      }
