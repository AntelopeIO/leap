...
public:
   ...
   auto delegate_bandwidth(name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
      auto r = base_tester::push_action(config::system_account_name,
                                        "delegatebw"_n,
                                        from,
                                        mvo()("from", from)("receiver", receiver)("stake_net_quantity", net)(
                                           "stake_cpu_quantity", cpu)("transfer", transfer));
      produce_block();
      return r;
   }
   ...
   auto undelegate_bandwidth(name from, name receiver, asset net, asset cpu) {
      auto r = base_tester::push_action(
         ...
         "undelegatebw"_n,
         from,
         mvo()("from", from)("receiver", receiver)("unstake_net_quantity", net)("unstake_cpu_quantity", cpu));
      produce_block();
      return r;
   }
...
BOOST_FIXTURE_TEST_CASE(bootseq_test, bootseq_tester) {
   try {
      ...
      // set_code_abi("eosio.msig"_n, contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//,
      // &eosio_active_pk); set_code_abi("eosio.token"_n, contracts::eosio_token_wasm(),
      ...
      auto votepro = [&](account_name voter, vector<account_name> producers) {
         ...
         base_tester::push_action(config::system_account_name,
                                  "voteproducer"_n,
                                  voter,
                                  mvo()("voter", name(voter))("proxy", name(0))("producers", producers));
      };
