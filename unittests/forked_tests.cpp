...
BOOST_AUTO_TEST_CASE(forking) try {
   ...
   c2.produce_block(fc::milliseconds(
...
BOOST_AUTO_TEST_CASE(validator_accepts_valid_blocks) try {
   ...
   auto c = n2.control->accepted_block.connect([&](const block_state_ptr& bsp) { first_block = bsp; });
...
BOOST_AUTO_TEST_CASE(push_block_returns_forked_transactions) try {
   ...
   BOOST_REQUIRE_EXCEPTION(
      c.control->get_account("test1"_n), fc::exception, [a = "test1"_n](const fc::exception& e) -> bool {
         return std::string(e.what()).find(a.to_string()) != std::string::npos;
      });
   BOOST_REQUIRE_EXCEPTION(
      c.control->get_account("test2"_n), fc::exception, [a = "test2"_n](const fc::exception& e) -> bool {
         return std::string(e.what()).find(a.to_string()) != std::string::npos;
      });
   BOOST_REQUIRE_EXCEPTION(
      c.control->get_account("test3"_n), fc::exception, [a = "test3"_n](const fc::exception& e) -> bool {
         return std::string(e.what()).find(a.to_string()) != std::string::npos;
      });
   BOOST_REQUIRE_EXCEPTION(
      c.control->get_account("test4"_n), fc::exception, [a = "test4"_n](const fc::exception& e) -> bool {
         return std::string(e.what()).find(a.to_string()) != std::string::npos;
      });
