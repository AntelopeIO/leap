...
template<typename T, typename Tester>
transaction_trace_ptr CallFunction(Tester&                     test,
                                   T                           ac,
                                   const vector<char>&         data,
                                   const vector<account_name>& scope    = { "testapi"_n },
                                   bool                        no_throw = false)
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER)
{
   try {
      ...
      BOOST_CHECK_EXCEPTION(
         CALL_TEST_FUNCTION(*this, "test_action", "require_auth", {}),
         missing_auth_exception,
         [](const missing_auth_exception& e) { return expect_assert_message(e, "missing authority of"); });

      // test require_auth
      auto a3only = std::vector<permission_level>{
         {"acc3"_n, config::active_name}
      };
      BOOST_CHECK_EXCEPTION(
         CALL_TEST_FUNCTION(*this, "test_action", "require_auth", fc::raw::pack(a3only)),
         missing_auth_exception,
         [](const missing_auth_exception& e) { return expect_assert_message(e, "missing authority of"); });

      // test require_auth
      auto a4only = std::vector<permission_level>{
         {"acc4"_n, config::active_name}
      };
      BOOST_CHECK_EXCEPTION(
         CALL_TEST_FUNCTION(*this, "test_action", "require_auth", fc::raw::pack(a4only)),
         missing_auth_exception,
         [](const missing_auth_exception& e) { return expect_assert_message(e, "missing authority of"); });

      // test require_auth
      auto a3a4 = std::vector<permission_level>{
         {"acc3"_n,  config::active_name},
         { "acc4"_n, config::active_name}
      };
      ...
      BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_action", "test_current_time", fc::raw::pack(now)),
                            eosio_assert_message_exception,
                            eosio_assert_message_is("tmp == current_time()"));
...
BOOST_AUTO_TEST_CASE(ram_billing_in_notify_tests)
{
   try {
      ...
      CALL_TEST_FUNCTION(chain,
                         "test_action",
                         "test_ram_billing_in_notify",
                         fc::raw::pack(((unsigned __int128)"testapi2"_n.to_uint64_t() << 64) | 0));

      CALL_TEST_FUNCTION(
         chain,
         "test_action",
         "test_ram_billing_in_notify",
         fc::raw::pack(((unsigned __int128)"testapi2"_n.to_uint64_t() << 64) | "testapi2"_n.to_uint64_t()));
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER)
{
   try {
      ...
      BOOST_CHECK_EXCEPTION(push_transaction(trx), tx_no_auths, [](const fc::assert_exception& e) {
      ...
      BOOST_CHECK_EXCEPTION(push_transaction(trx), tx_no_auths, [](const fc::exception& e) {
         return expect_assert_message(e, "transaction must have at least one authorization");
      });
      ...
      BOOST_CHECK_EXCEPTION(push_transaction(trx), unaccessible_api, [](const fc::exception& e) {
...
try {
   ...
   BOOST_TEST(tx1.sign(priv_key, control->get_chain_id()).to_string() !=
              tx2.sign(priv_key, control->get_chain_id()).to_string());
...
try {
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), fc::exception, [&](const fc::exception& e) {
      return expect_assert_message(e, "only context free api's can be used in this context");
   });
...
try {
   ...
   BOOST_CHECK_EXCEPTION(create_account("testapi2"_n), fc::exception, [&](const fc::exception& e) {
...
try {
   ...
   auto                  cc = other.control->applied_transaction.connect(
               ...
               name              account = "testapi"_n)
{
   ...
   auto fut = transaction_metadata::start_recover_keys(std::move(ptrx),
                                                       test.control->get_thread_pool(),
                                                       test.control->get_chain_id(),
                                                       fc::microseconds::maximum(),
                                                       transaction_metadata::trx_type::input);
   auto res = test.control->push_transaction(fut.get(),
                                             fc::time_point::now() + fc::milliseconds(max_block_cpu_ms),
                                             fc::milliseconds(max_cpu_usage_ms),
                                             billed_cpu_time_us,
...
BOOST_AUTO_TEST_CASE(checktime_fail_tests)
{
   try {
      ...
      BOOST_CHECK_EXCEPTION(call_test(t,
                                      test_api_action<TEST_METHOD("test_checktime", "checktime_failure")>{},
                                      5000,
                                      200,
                                      200,
                                      fc::raw::pack(10000000000000000000ULL)),
                            deadline_exception,
...
(drop (call $memmove
    ...
    (i32.const 33554432)
      ...
      // initialize cache
      BOOST_CHECK_EXCEPTION(
         call_test(*this, test_api_action<TEST_METHOD("doesn't matter", "doesn't matter")>{}, 5000, 10, 10),
         deadline_exception,
         is_deadline_exception);
...
(module
 ...
 (start $start)
   ...
   BOOST_CHECK_EXCEPTION(
      call_test(*this, test_api_action<TEST_METHOD("doesn't matter", "doesn't matter")>{}, 5000, 3, 3),
      deadline_exception,
      is_deadline_exception);
}
FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER)
{
   try {
      ...
      // test for zero auth
      {
         ...
         set_transaction_headers(trx);
         BOOST_CHECK_EXCEPTION(push_transaction(trx), transaction_exception, [](const fc::exception& e) {
            return expect_assert_message(e, "transaction must have at least one authorization");
         });
      }
...
BOOST_AUTO_TEST_CASE(deferred_inline_action_subjective_limit_failure)
{
   try {
      ...
      transaction_trace_ptr trace;
      auto                  c = chain.control->applied_transaction.connect(
         [&](std::tuple<const transaction_trace_ptr&, const packed_transaction_ptr&> x) {
            auto& t = std::get<0>(x);
            if (t->scheduled) {
               trace = t;
      ...
      BOOST_CHECK(!trace);
      BOOST_CHECK_EXCEPTION(chain.produce_block(fc::seconds(2)), fc::exception, [](const fc::exception& e) {
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER)
{
   try {
      ...
      // schedule twice with replace_existing flag (second deferred transaction should replace first one)
      {
         ...
         for (const auto& trx : dtrxs) {
            control->push_scheduled_transaction(
               trx, fc::time_point::maximum(), fc::microseconds::maximum(), billed_cpu_time_us, true);
         }
         BOOST_CHECK_EQUAL(1, count);
         BOOST_CHECK(trace);
         BOOST_CHECK_EQUAL(1, trace->action_traces.size());
...
BOOST_AUTO_TEST_CASE(more_deferred_transaction_tests)
{
   try {
      ...
      trx.actions.emplace_back(chain.get_action(
         ...
         fc::mutable_variant_object()("payer", test_account)("sender_id", 0)("contract", contract_account)(
            "payload", 13)("delay_sec", 1000)("replace_existing", true)));
      trx.actions.emplace_back(chain.get_action(
         contract_account,
         "delayedcall"_n,
         vector<permission_level>{
      ...
      BOOST_REQUIRE_EXCEPTION(
         chain.push_transaction(trx), eosio_assert_message_exception, eosio_assert_message_is("fail"));
      ...
      trx2.actions.emplace_back(chain.get_action(
         ...
         fc::mutable_variant_object()("payer", test_account)("sender_id", 1)("contract", contract_account)(
            "payload", 100)("delay_sec", 1000)("replace_existing", true)));
      trx2.actions.emplace_back(chain.get_action(
         contract_account,
         "delayedcall"_n,
         vector<permission_level>{
         ...
         fc::mutable_variant_object()("payer", test_account)("sender_id", 2)("contract", contract_account)(
            "payload", 101)("delay_sec", 1000)("replace_existing", true)));
      trx2.actions.emplace_back(chain.get_action(
         contract_account,
         "delayedcall"_n,
         vector<permission_level>{
      ...
      BOOST_REQUIRE_EXCEPTION(
         chain.push_transaction(trx2), eosio_assert_message_exception, eosio_assert_message_is("fail"));
...
BOOST_FIXTURE_TEST_CASE(db_tests, TESTER)
{
   try {
      ...
      push_action("testapi"_n,
                  "tia"_n,
                  "testapi"_n,
                  mutable_variant_object() // test_invalid_access
      ...
      BOOST_CHECK_EXCEPTION(
         push_action("testapi2"_n,
                     "tia"_n,
                     "testapi2"_n,
                     mutable_variant_object()("code", "testapi")("val", "20")("index", 0)("store", true)),
      ...
      push_action("testapi"_n,
                  "tia"_n,
                  "testapi"_n,
                  mutable_variant_object()("code", "testapi")("val", 10)("index", 0)("store", false));
      ...
      BOOST_CHECK_EXCEPTION(
         push_action("testapi2"_n,
                     "tia"_n,
                     "testapi2"_n,
                     mutable_variant_object()("code", "testapi")("val", "20")("index", 1)("store", true)),
         table_access_violation,
         fc_exception_message_is("db access violation"));
      ...
      push_action("testapi"_n,
                  "tia"_n,
                  "testapi"_n,
                  mutable_variant_object()("code", "testapi")("val", 10)("index", 1)("store", false));
...
BOOST_FIXTURE_TEST_CASE(crypto_tests, TESTER)
{
   try {
      ...
      CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION(
         *this, "test_crypto", "assert_sha256_false", {}, crypto_api_exception, "hash mismatch");

      CALL_TEST_FUNCTION(*this, "test_crypto", "assert_sha256_true", {});

      CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION(
         *this, "test_crypto", "assert_sha1_false", {}, crypto_api_exception, "hash mismatch");

      CALL_TEST_FUNCTION(*this, "test_crypto", "assert_sha1_true", {});

      CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION(
         *this, "test_crypto", "assert_sha512_false", {}, crypto_api_exception, "hash mismatch");

      CALL_TEST_FUNCTION(*this, "test_crypto", "assert_sha512_true", {});

      CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION(
         *this, "test_crypto", "assert_ripemd160_false", {}, crypto_api_exception, "hash mismatch");
...
BOOST_FIXTURE_TEST_CASE(permission_usage_tests, TESTER)
{
   try {
      ...
      push_reqauth("alice"_n,
                   {
                      {"alice"_n, config::active_name}
      },
                   { get_private_key("alice"_n, "active") });

      CALL_TEST_FUNCTION(*this,
                         "test_permission",
                         "test_permission_last_used",
                         fc::raw::pack(test_permission_last_used_action{
                            "alice"_n, config::active_name, control->pending_block_time() }));
      ...
      CALL_TEST_FUNCTION(
         *this,
         "test_permission",
         "test_permission_last_used",
         fc::raw::pack(test_permission_last_used_action{ "bob"_n, "perm1"_n, permission_creation_time }));
...
BOOST_AUTO_TEST_CASE(action_results_tests)
{
   try {
      ...
      auto call_autoresret_and_check =
         [&](account_name contract, account_name signer, action_name action, auto&& checker) {
            signed_transaction trx;
            trx.actions.emplace_back(
               vector<permission_level>{
                  {signer, config::active_name}
...
BOOST_AUTO_TEST_CASE(get_code_hash_tests)
{
   try {
      ...
      auto check = [&](account_name acc, uint64_t expected_seq) {
         ...
         auto expected = "43:0:" + std::to_string(expected_seq) + ":" + expected_code_hash.str() + ":0:0";

         signed_transaction trx;
         trx.actions.emplace_back(
            vector<permission_level>{
               {"gethash"_n, config::active_name}
         },
            "gethash"_n,
            acc,
            bytes{});
