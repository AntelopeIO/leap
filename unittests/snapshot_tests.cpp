...
public:
      ...
      config_file_handling       copy_files_from_config = config_file_handling::dont_copy_config_files)
   {
      ...
      controller::config copied_config = (copy_files_from_config == copy_config_files)
                                            ? copy_config_and_files(config, ordinal)
                                            : copy_config(config, ordinal);
   ...
   std::function<void(const std::string&, const fc::variant&, const fc::variant&)>&& out)
{
   if (lhs.get_type() != rhs.get_type()) {
      ...
      for (size_t idx = 0; idx < common_count; idx++) {
         ...
         variant_diff_helper(
            l_val, r_val, [&](const std::string& path, const fc::variant& lhs, const fc::variant& rhs) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_exhaustive_snapshot, SNAPSHOT_SUITE, snapshot_suites)
{
   ...
   chain.produce_blocks(1);
   chain.push_action("snapshot"_n, "increment"_n, "snapshot"_n, mutable_variant_object()("value", 1));
   ...
   chain.push_action("snapshot1"_n, "increment"_n, "snapshot1"_n, mutable_variant_object()("value", 1));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_replay_over_snapshot, SNAPSHOT_SUITE, snapshot_suites)
{
   ...
   snapshotted_tester replay_chain(snap_chain.get_config(),
                                   SNAPSHOT_SUITE::get_reader(snapshot),
                                   ordinal++,
                                   config_file_handling::copy_config_files);
   ...
   snapshotted_tester replay2_chain(snap_chain.get_config(),
                                    SNAPSHOT_SUITE::get_reader(snapshot),
                                    ordinal++,
                                    config_file_handling::copy_config_files);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_compatible_versions, SNAPSHOT_SUITE, snapshot_suites)
{
   ...
   for (std::string version : { "v2", "v3", "v4", "v5", "v6" }) {
      if (save_snapshot && version == current_version)
         continue;
      ...
      snapshotted_tester old_snapshot_tester(
         base_chain.get_config(), SNAPSHOT_SUITE::get_reader(old_snapshot), ordinal++);
