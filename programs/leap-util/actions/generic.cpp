...
void generic_actions::setup(CLI::App& app)
{
   ...
   sub->add_subcommand("client", "retrieve basic version information of the client")->callback([this]() {
      cb_version(false);
   });
   sub->add_subcommand("full", "retrieve full version information of the client")->callback([this]() {
      cb_version(true);
   });
}
