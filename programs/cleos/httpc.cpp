                         ...
                         const fc::variant& postdata)
{
   ...
   auto [status_code, re] = do_http_post(
      base_uri, path, config.headers, postjson, !config.no_verify_cert, config.verbose, config.trace);
   ...
   if (!response_result.is_null()) {
      if (status_code == 200 || status_code == 201 || status_code == 202) {
         ...
         for (auto itr = error_details.begin(); itr != error_details.end(); itr++) {
            const auto& context =
               fc::log_context(fc::log_level::error, itr->file.data(), itr->line_number, itr->method.data());
   ...
   EOS_ASSERT(status_code == 200 && !response_result.is_null(),
              http_request_fail,
              "Error code ${c}\n: ${msg}\n",
