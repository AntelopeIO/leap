...
struct ship_log_fixture
{
   ...
   void add(uint32_t index, size_t size, char fillchar)
   {
      ...
      log->write_entry(header, block_for_id(index - 1), [&](auto& f) { f.write(a.data(), a.size()); });
...
BOOST_DATA_TEST_CASE(basic_prune_test,
                     ...
                     vacuum_on_exit_if_small)
{
   try {
      ...
      t.check_n_bounce([&]() { t.check_range_present(2, 4); });
      ...
      t.check_n_bounce([&]() { t.check_range_present(7, 10); });
...
BOOST_DATA_TEST_CASE(basic_test,
                     ...
                     remove_index_on_reopen)
{
   try {
      ...
      t.check_n_bounce([&]() { t.check_range_present(2, 9); });
                     ...
                     remove_index_on_reopen)
{
   try {
      ...
      t.check_n_bounce([&]() { t.check_range_present(6, 15); });
   }
   FC_LOG_AND_RETHROW()
}
