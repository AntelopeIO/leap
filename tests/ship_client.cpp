...
int main(int argc, char *argv[])
{
   ...
   boost::beast::websocket::stream<boost::asio::ip::tcp::socket> tcp_stream(ctx);
   eosio::abi abi;

   unixs::socket unix_socket(ctx);
   boost::beast::websocket::stream<unixs::socket> unix_stream(ctx);
   ...
   cli.add_options()("help,h", bpo::bool_switch(&help)->default_value(false), "Print this help message and exit.")("socket-address,a", bpo::value<std::string>(&socket_address)->default_value(socket_address), "Websocket address and port.")("num-requests,n", bpo::value<uint32_t>(&num_requests)->default_value(num_requests), "number of requests to make");
   ...
   try
   {
      auto run = [&]<typename SocketStream>(SocketStream &stream)
      {
         ...
         while (num_requests--)
         {
            ...
            std::cout << result_sb.GetString() << std::endl
                      << "}" << std::endl;
         ...
         std::cerr << done_sb.GetString() << std::endl
                   << "]" << std::endl;
      };
      ...
      if (statehistory_port.empty())
      {
         ...
         unix_stream.next_layer().connect(boost::asio::local::stream_protocol::endpoint(statehistory_server), ec);
