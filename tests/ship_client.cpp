...
int main(int argc, char *argv[]) {
  ...
  cli.add_options()("help,h", bpo::bool_switch(&help)->default_value(false),
                    "Print this help message and exit.")(
      "socket-address,a",
      bpo::value<std::string>(&socket_address)->default_value(socket_address),
      "Websocket address and port.")(
      "num-requests,n",
      bpo::value<uint32_t>(&num_requests)->default_value(num_requests),
  ...
  if (boost::algorithm::starts_with(socket_address, "ws+unix://") ||
    ...
    statehistory_port = "";
    statehistory_server = socket_address.substr(socket_address.find("unix://") +
                                                strlen("unix://") + 1);
  } else {
    std::string::size_type colon = socket_address.find(':');
    eosio::check(colon != std::string::npos,
                 "Missing ':' seperator in Websocket address and port");
  ...
  try {
    auto run = [&]<typename SocketStream>(SocketStream &stream) {
      ...
      while (num_requests--) {
        ...
        rapidjson::PrettyWriter<rapidjson::StringBuffer> request_writer(
            request_sb);
        ...
        stream.write(boost::asio::buffer(
            request_type.json_to_bin(request_sb.GetString(), []() {})));
        boost::beast::flat_buffer buffer;
        stream.read(buffer);

        eosio::input_stream is((const char *)buffer.data().data(),
                               buffer.data().size());
        rapidjson::Document result_doucment;
        result_doucment.Parse(result_type.bin_to_json(is).c_str());

        eosio::check(!result_doucment.HasParseError(),
                     "Failed to parse result JSON from abieos");
        eosio::check(result_doucment.IsArray(),
                     "result should have been an array (variant) but it's not");
        eosio::check(result_doucment.Size() == 2,
                     "result was an array but did not contain 2 items like a "
                     "variant should");
        eosio::check(std::string(result_doucment[0].GetString()) ==
                         "get_status_result_v0",
                     "result type doesn't look like get_status_result_v0");
        eosio::check(result_doucment[1].IsObject(),
                     "second item in result array is not an object");
        eosio::check(result_doucment[1].HasMember("head"),
                     "cannot find 'head' in result");
        eosio::check(result_doucment[1]["head"].IsObject(),
                     "'head' is not an object");
        eosio::check(result_doucment[1]["head"].HasMember("block_num"),
                     "'head' does not contain 'block_num'");
        eosio::check(result_doucment[1]["head"]["block_num"].IsUint(),
                     "'head.block_num' isn't a number");

        uint32_t this_block_num =
            result_doucment[1]["head"]["block_num"].GetUint();
        ...
        rapidjson::PrettyWriter<rapidjson::StringBuffer> result_writer(
            result_sb);
    ...
    if (statehistory_port.empty()) {
      ...
      if (ec == boost::system::errc::success) {
        std::cerr << "{\n   \"status\": \"successfully connected to unix "
                     "socket\",\n   \"time\": "
                  << time(NULL) << "\n},\n";
      } else {
        check_ec("connect");
    ...
    // tcp socket
      ...
      boost::asio::connect(
          tcp_stream.next_layer(),
          resolver.resolve(statehistory_server, statehistory_port));
