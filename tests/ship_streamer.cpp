...
int main(int argc, char* argv[]) {
   ...
   try {
      ...
      for (;;) {
         ...
         eosio::check(std::string(result_doucment[0].GetString()) == "get_blocks_result_v0",
                      "result type doesn't look like get_blocks_result_v0");
         ...
         if (result_doucment[1].HasMember("this_block") && result_doucment[1]["this_block"].IsObject()) {
            if (result_doucment[1]["this_block"].HasMember("block_num") &&
                result_doucment[1]["this_block"]["block_num"].IsUint()) {
         ...
         if (this_block_num == end_block_num)
            break;
      }
