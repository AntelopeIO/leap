...
private:
   void bounce()
   {
      ...
      if (prune_blocks) {
         ...
         conf->prune_threshold =
            8; // check to prune every 8 bytes; should guarantee always checking to prune for each block added
         if (vacuum_on_exit_if_small)
            conf->vacuum_on_close =
               1024 * 1024 * 1024; // something large: will always vacuum on close for these small tests
      }
...
BOOST_DATA_TEST_CASE(basic_prune_test_genesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     ...
                     vacuum_on_exit_if_small)
{
   try {
      ...
      t.check_n_bounce([&]() { t.check_range_present(1, 2); });

      t.add(3, payload_size(), 'B');
      t.add(4, payload_size(), 'C');
      t.check_n_bounce([&]() { t.check_range_present(1, 4); });

      t.add(5, payload_size(), 'D');
      t.check_n_bounce([&]() { t.check_range_present(2, 5); });

      t.add(6, payload_size(), 'E');
      t.check_n_bounce([&]() { t.check_range_present(3, 6); });

      t.add(7, payload_size(), 'F');
      t.add(8, payload_size(), 'G');
      t.add(9, payload_size(), 'H');
      t.check_n_bounce([&]() { t.check_range_present(6, 9); });
   }
   FC_LOG_AND_RETHROW()
}

BOOST_DATA_TEST_CASE(basic_prune_test_nongenesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
                     vacuum_on_exit_if_small)
{
   try {
      ...
      t.check_n_bounce([&]() { t.check_range_present(10, 10); });

      t.add(11, payload_size(), 'B');
      t.add(12, payload_size(), 'C');
      t.check_n_bounce([&]() { t.check_range_present(10, 12); });

      t.add(13, payload_size(), 'D');
      t.check_n_bounce([&]() { t.check_range_present(10, 13); });

      t.add(14, payload_size(), 'E');
      t.check_n_bounce([&]() { t.check_range_present(11, 14); });

      t.add(15, payload_size(), 'F');
      t.add(16, payload_size(), 'G');
      t.add(17, payload_size(), 'H');
      t.check_n_bounce([&]() { t.check_range_present(14, 17); });
   }
   FC_LOG_AND_RETHROW()
}

// well we do let someone configure a single block prune; so let's make sure that works..
BOOST_DATA_TEST_CASE(single_prune_test_genesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
                     vacuum_on_exit_if_small)
{
   try {
      ...
      t.add(2, payload_size(), 'A');
      t.check_n_bounce([&]() { t.check_range_present(2, 2); });

      t.add(3, payload_size(), 'B');
      t.add(4, payload_size(), 'C');
      t.check_n_bounce([&]() { t.check_range_present(4, 4); });
   }
   FC_LOG_AND_RETHROW()
}

BOOST_DATA_TEST_CASE(single_prune_test_nongenesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
                     vacuum_on_exit_if_small)
{
   try {
      ...
      t.add(11, payload_size(), 'B');
      t.add(12, payload_size(), 'C');
      t.check_n_bounce([&]() { t.check_range_present(12, 12); });
   }
   FC_LOG_AND_RETHROW()
}

BOOST_DATA_TEST_CASE(nonprune_test_genesis,
                     ...
                     remove_index_on_reopen)
{
   try {
      ...
      t.add(5, payload_size(), 'D');
      t.check_n_bounce([&]() { t.check_range_present(1, 5); });

      t.add(6, payload_size(), 'E');
      t.check_n_bounce([&]() { t.check_range_present(1, 6); });

      t.add(7, payload_size(), 'F');
      t.add(8, payload_size(), 'G');
      t.add(9, payload_size(), 'H');
      t.check_n_bounce([&]() { t.check_range_present(1, 9); });
   }
   FC_LOG_AND_RETHROW()
}

BOOST_DATA_TEST_CASE(nonprune_test_nongenesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen)
{
   try {
      ...
      t.add(14, payload_size(), 'E');
      t.check_n_bounce([&]() { t.check_range_present(10, 14); });

      t.add(15, payload_size(), 'F');
      t.add(16, payload_size(), 'G');
      t.add(17, payload_size(), 'H');
      t.check_n_bounce([&]() { t.check_range_present(10, 17); });
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_DATA_TEST_CASE(no_block_log_basic_genesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
...
// Test when prune_blocks is set to 0, no block log is generated
BOOST_DATA_TEST_CASE(no_block_log_basic_nongenesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
...
BOOST_DATA_TEST_CASE(no_block_log_public_functions_genesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
...
// Test when prune_blocks is set to 0, block_log's public methods work
BOOST_DATA_TEST_CASE(no_block_log_public_functions_nogenesis,
                     bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2) * bdata::xrange(2),
                     enable_read,
                     reopen_on_mark,
                     remove_index_on_reopen,
