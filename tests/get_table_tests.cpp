...
transaction_trace_ptr issue_tokens(TESTER&      t,
                                   account_name issuer,
                                   account_name to,
                                   const asset& amount,
                                   std::string  memo           = "",
...
BOOST_FIXTURE_TEST_CASE(get_table_test, TESTER) try {
   ...
   // create currency 2
   act = mutable_variant_object()("issuer", "eosio")("maximum_supply",
                                                     eosio::chain::asset::from_string("1000000000.0000 AAA"));
   push_action("eosio.token"_n, "create"_n, "eosio.token"_n, act);
   // issue
   for (account_name a : accs) {
   ...
   // create currency 3
   act = mutable_variant_object()("issuer", "eosio")("maximum_supply",
                                                     eosio::chain::asset::from_string("1000000000.0000 CCC"));
   push_action("eosio.token"_n, "create"_n, "eosio.token"_n, act);
   // issue
   for (account_name a : accs) {
   ...
   // create currency 3
   act = mutable_variant_object()("issuer", "eosio")("maximum_supply",
                                                     eosio::chain::asset::from_string("1000000000.0000 BBB"));
   push_action("eosio.token"_n, "create"_n, "eosio.token"_n, act);
   // issue
   for (account_name a : accs) {
   ...
   eosio::chain_apis::read_only::get_table_rows_result result =
      plugin.read_only::get_table_rows(p, fc::time_point::maximum());
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
...
BOOST_FIXTURE_TEST_CASE(get_table_by_seckey_test, TESTER) try {
   ...
   base_tester::push_action(config::system_account_name,
                            ...
                            mutable_variant_object()("version", 0)("core", "4,SYS"));

   // bidname
   auto bidname = [this](const account_name& bidder, const account_name& newname, const asset& bid) {
      return push_action("eosio"_n,
                         "bidname"_n,
                         bidder,
                         fc::mutable_variant_object()("bidder", bidder)("newname", newname)("bid", bid));
   };
...
BOOST_FIXTURE_TEST_CASE(get_table_next_key_test, TESTER) try {
   ...
   params.index_position = "2";
   params.lower_bound =
      "2652d68fbbf6000c703b35fdc607b09cd8218cbeea1d108b5c9e84842cdd5ea5"; // This is hash of "thirdinput"
