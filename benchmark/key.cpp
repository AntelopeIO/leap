...
void k1_sign_benchmarking() {
   ...
   auto sign_non_canonical_f = [&]() { key.sign(digest, false); };
...
void wa_benchmarking() {
   ...
   auto sign = [&]() { make_webauthn_sig(priv, auth_data, json); };
   benchmarking("webauthn_sign", sign);

   auto sig     = make_webauthn_sig(priv, auth_data, json);
   auto recover = [&]() { sig.recover(d, true); };
