...
void deferred_transaction_generation_context::reflector_init() {
   ...
   EOS_ASSERT(sender != account_name(),
...
flat_multimap<uint16_t, transaction_extension> transaction::validate_and_extract_extensions() const {
   ...
   for (size_t i = 0; i < transaction_extensions.size(); ++i) {
      ...
      EOS_ASSERT(id >= id_type_lower_bound,
...
bytes packed_transaction::get_raw_transaction() const {
   try {
      switch (compression) {
         ...
         case compression_type::zlib: return zlib_decompress(packed_trx);
...
packed_transaction::packed_transaction(bytes&&                  packed_txn,
...
packed_transaction::packed_transaction(transaction&&            t,
                                       vector<signature_type>&& sigs,
                                       bytes&&                  packed_cfd,
                                       compression_type         _compression)
   : signatures(std::move(sigs))
...
void packed_transaction::local_pack_transaction() {
   try {
      switch (compression) {
         ...
         case compression_type::zlib: packed_trx = zlib_compress_transaction(unpacked_trx); break;
         default: EOS_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
   }
   FC_CAPTURE_AND_RETHROW((compression))
