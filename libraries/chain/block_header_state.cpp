   ...
   const protocol_feature_set&                       pfs) const
{
   ...
   if (new_producers) {
      if (detail::is_builtin_activated(
         ...
         for (const auto& p : new_producers->producers) {
            std::visit(
               [&downgraded_producers, &p](const auto& auth) {
                  EOS_ASSERT(auth.keys.size() == 1 && auth.keys.front().weight == auth.threshold,
                             producer_schedule_exception,
                             "multisig block signing present before enabled!");
                  downgraded_producers.producers.emplace_back(
                     legacy::producer_key{ p.producer_name, auth.keys.front().key });
               },
               p.authority);
         }
   ...
   ) &&
{
   ...
   EOS_ASSERT(h.schedule_version == active_schedule_version,
              producer_schedule_exception,
   ...
   if (h.new_producers || exts.count(producer_schedule_change_extension::extension_id()) > 0) {
      wtmsig_enabled = detail::is_builtin_activated(
         prev_activated_protocol_features, pfs, builtin_protocol_feature_t::wtmsig_block_signatures);
   }
   ...
   if (h.new_producers) {
      ...
      EOS_ASSERT(prev_pending_schedule.schedule.producers.empty(),
                 producer_schedule_exception,
   ...
   if (exts.count(producer_schedule_change_extension::extension_id()) > 0) {
      ...
      EOS_ASSERT(new_producer_schedule.version == active_schedule.version + 1,
                 producer_schedule_exception,
                 "wrong producer schedule version specified");
      EOS_ASSERT(prev_pending_schedule.schedule.producers.empty(),
                 producer_schedule_exception,
   ...
   bool skip_validate_signee) &&
{
   if (!additional_signatures.empty()) {
      ...
      EOS_ASSERT(wtmsig_enabled,
                 producer_schedule_exception,
                 "Block contains multiple signatures before WTMsig block signatures are enabled");
   }
...
void block_header_state::verify_signee() const
{
   ...
   std::tie(is_satisfied, relevant_sig_count) =
      producer_authority::keys_satisfy_and_relevant(keys, valid_block_signing_authority);

   EOS_ASSERT(relevant_sig_count == keys.size(),
              wrong_signing_key,
              "block signed by unexpected key",
              ("signing_keys", keys)("authority", valid_block_signing_authority));

   EOS_ASSERT(is_satisfied,
              wrong_signing_key,
...
const vector<digest_type>& block_header_state::get_new_protocol_feature_activations() const
{
   ...
   return std::get<protocol_feature_activation>(
