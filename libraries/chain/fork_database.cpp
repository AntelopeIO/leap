...
void fork_database::rollback_head_to_root() {
   ...
   while (itr != by_id_idx.end()) {
      by_id_idx.modify(itr, [&](block_state_ptr& bsp) { bsp->validated = false; });
      ++itr;
   }
   my->head = my->root;
}
...
void fork_database::advance_root(const block_id_type& id) {
   ...
   for (auto b = new_root; b;) {
      ...
      EOS_ASSERT(b || blocks_to_remove.back() == my->root->id,
                 fork_database_exception,
...
pair<branch_type, branch_type> fork_database::fetch_branch_from(const block_id_type& first,
   ...
   while (first_branch->block_num > second_branch->block_num) {
      ...
      EOS_ASSERT(first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", prev));
   }
   ...
   while (second_branch->block_num > first_branch->block_num) {
      ...
      EOS_ASSERT(second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", prev));
   }
   ...
   while (first_branch->header.previous != second_branch->header.previous) {
      ...
      EOS_ASSERT(first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", first_prev));
      EOS_ASSERT(second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", second_prev));
   }
...
void fork_database::remove(const block_id_type& id) {
   ...
   for (uint32_t i = 0; i < remove_queue.size(); ++i) {
      EOS_ASSERT(remove_queue[i] != head_id,
                 fork_database_exception,
...
void fork_database::mark_valid(const block_state_ptr& h) {
   ...
   EOS_ASSERT(itr != by_id_idx.end(),
              fork_database_exception,
              "block state not in fork database; cannot mark as valid",
              ("id", h->id));

   by_id_idx.modify(itr, [](block_state_ptr& bsp) { bsp->validated = true; });
