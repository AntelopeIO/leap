...
platform_timer::platform_timer() {
   ...
   if(refcount++ == 0) {
      ...
      EV_SET(&quit_event, impl::quit_event_id, EVFILT_USER, EV_ADD|EV_ENABLE|EV_CLEAR, NOTE_FFNOP, 0, 0);
      FC_ASSERT(kevent(kqueue_fd, &quit_event, 1, NULL, 0, NULL) == 0, "failed to create quit event");

      kevent_thread = std::thread([]() {
         fc::set_os_thread_name("checktime");
         while(true) {
            struct kevent anEvent;
            int c = kevent(kqueue_fd, NULL, 0, &anEvent, 1, NULL);
...
platform_timer::~platform_timer() {
   ...
   if(std::lock_guard guard(timer_ref_mutex); --refcount == 0) {
      struct kevent signal_quit_event;
      EV_SET(&signal_quit_event, impl::quit_event_id, EVFILT_USER, 0, NOTE_TRIGGER, 0, 0);

      if(kevent(kqueue_fd, &signal_quit_event, 1, NULL, 0, NULL) != -1)
         kevent_thread.join();
      close(kqueue_fd);
   }
}
...
void platform_timer::start(fc::time_point tp) {
   ...
   if(x.count() <= 0)
      ...
      struct kevent aTimerEvent;
      EV_SET(&aTimerEvent, my->timerid, EVFILT_TIMER, EV_ADD|EV_ENABLE|EV_ONESHOT, NOTE_USECONDS, x.count(), this);

      expired = 0;
      if(kevent(kqueue_fd, &aTimerEvent, 1, NULL, 0, NULL) != 0)
         expired = 1;
   }
}
...
void platform_timer::stop() {
   ...
   struct kevent stop_timer_event;
   EV_SET(&stop_timer_event, my->timerid, EVFILT_TIMER, EV_DELETE, 0, 0, 0);
   kevent(kqueue_fd, &stop_timer_event, 1, NULL, 0, NULL);
   expired = 1;
}
