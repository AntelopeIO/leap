...
void abi_serializer::set_abi(const abi_def& abi, const yield_function_t& yield) {
   ...
   for (const auto& td : abi.types) {
      EOS_ASSERT(!_is_type(td.new_type_name, ctx),
                 duplicate_abi_type_def_exception,
                 "type already exists",
                 ("new_type_name", impl::limit_size(td.new_type_name)));
   ...
   EOS_ASSERT(variants.size() == abi.variants.value.size(),
              duplicate_abi_variant_def_exception,
              "duplicate variant definition detected");
   EOS_ASSERT(action_results.size() == abi.action_results.value.size(),
              duplicate_abi_action_results_def_exception,
...
bool abi_serializer::_is_type(const std::string_view& rtype, impl::abi_traverse_context& ctx) const {
   ...
   if (structs.find(type) != structs.end())
      return true;
   if (variants.find(type) != variants.end())
      return true;
   return false;
}
      ...
      try {
         ...
         while (itr != typedefs.end()) {
            ...
            EOS_ASSERT(find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(),
                       ...
                       "Circular reference in type ${type}",
                       ("type", impl::limit_size(t.first)));
   ...
   for (const auto& s : structs) {
      try {
         if (s.second.base != type_name()) {
            ...
            while (current->base != type_name()) {
               ...
               EOS_ASSERT(find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(),
                          ...
                          ("type", impl::limit_size(s.second.name)));
         ...
         for (const auto& field : s.second.fields) {
            try {
               ctx.check_deadline();
               EOS_ASSERT(_is_type(_remove_bin_extension(field.type), ctx),
                          invalid_type_inside_abi,
                          "${type}",
                          ("type", impl::limit_size(field.type)));
            }
...
fc::variant abi_serializer::_binary_to_variant(const std::string_view&          type,
   ...
   if (btype != built_in_types.end()) {
      ...
      EOS_RETHROW_EXCEPTIONS(unpack_exception,
                             ...
                             ("class",
                              is_array(rtype) ? "array of built-in"
                              : is_optional(rtype)
                                 ? "optional of built-in"
                                 : "built-in")("type", impl::limit_size(ftype))("p", ctx.get_path_string()))
   }
   if (is_array(rtype)) {
      ...
      EOS_RETHROW_EXCEPTIONS(
         unpack_exception, "Unable to unpack presence flag of optional '${p}'", ("p", ctx.get_path_string()))
      ...
      if (v_itr != variants.end()) {
         ...
         EOS_RETHROW_EXCEPTIONS(
            unpack_exception, "Unable to unpack tag of variant '${p}'", ("p", ctx.get_path_string()))
         EOS_ASSERT((size_t)select < v_itr->second.types.size(),
                    unpack_exception,
   ...
   try {
      ...
      if (btype != built_in_types.end()) {
         ...
         auto h1 = ctx.push_to_path(impl::variant_path_item{
            .variant_itr = v_itr, .variant_ordinal = static_cast<uint32_t>(it - v.types.begin()) });
...
void abi_serializer::variant_to_binary(const std::string_view& type,
                                       const fc::variant&      var,
                                       fc::datastream<char*>&  ds,
                                       const yield_function_t& yield,
                                       bool                    short_path) const {
...
void abi_traverse_context_with_path::set_array_index_of_path_back(uint32_t i) {
   ...
   EOS_ASSERT(std::holds_alternative<array_index_path_item>(b),
              abi_exception,
