      ...
      try {
         ...
         if (!(context_free && control.skip_trx_checks())) {
            ...
            if ((receiver_account->code_hash != digest_type()) &&
                (!(act->account == config::system_account_name && act->name == "setcode"_n &&
...
void apply_context::get_code_hash(account_name account,
...
void apply_context::execute_inline(action&& a) {
   ...
   if (!control.skip_auth_check() && !privileged) {
      try {
         control.get_authorization_manager().check_authorization(
            {
               a
         },
            {},
   ...
   _inline_actions.emplace_back(schedule_action(std::move(a), inline_receiver, false));
...
void apply_context::execute_context_free_inline(action&& a) {
   ...
   auto inline_receiver = a.account;
   _cfa_inline_actions.emplace_back(schedule_action(std::move(a), inline_receiver, true));
   ...
   if (auto ptr =
      ...
      db.create<generated_transaction_object>([&](auto& gtx) {
         ...
         if (auto dm_logger = control.get_deep_mind_logger()) {
            ...
            dm_logger->on_ram_trace(
               RAM_EVENT_ID("${id}", ("id", gtx.id)), "deferred_trx", "update", "deferred_trx_add");
         }
      });
   } else {
      db.create<generated_transaction_object>([&](auto& gtx) {
...
int apply_context::db_store_i64(name                scope,
                                name                table,
                                const account_name& payer,
                                uint64_t            id,
                                const char*         buffer,
...
int apply_context::db_store_i64(name                code,
                                name                scope,
                                name                table,
                                const account_name& payer,
                                uint64_t            id,
...
void apply_context::db_remove_i64(int iterator) {
   ...
   db.modify(table_obj, [&](auto& t) { --t.count; });
   db.remove(obj);
