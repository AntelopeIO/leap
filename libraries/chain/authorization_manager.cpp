...
using authorization_index_set = index_set<permission_index, permission_usage_index, permission_link_index>;
                                                           ...
                                                           const vector<permission_level>& auths) const
{
   ...
   EOS_ASSERT(get_permission(auth).satisfies(*min_permission, _db.get_index<permission_index>().indices()),
              irrelevant_auth_exception,
   ...
   const flat_set<permission_level>& satisfied_authorizations) const
{
   ...
   for (const auto& p : permissions_to_satisfy) {
      ...
      EOS_ASSERT(checker.satisfied(p.first, p.second) || check_but_dont_fail,
                 ...
                 ("auth", p.first)("provided_delay", provided_delay.count() / 1000)("provided_permissions",
                                                                                    provided_permissions)(
                    "provided_keys", provided_keys)("delay_max_limit_ms", delay_max_limit.count() / 1000));
   }
...
void authorization_manager::check_authorization(account_name                      account,
   ...
   fc::microseconds                 provided_delay) const
{
   ...
   for (const auto& act : trx.actions) {
      for (const auto& declared_auth : act.authorization) {
         EOS_ASSERT(checker.satisfied(declared_auth),
                    unsatisfied_authorization,
