...
void chain_config_v0::validate() const {
   ...
   EOS_ASSERT(max_transaction_cpu_usage < (max_block_cpu_usage - min_transaction_cpu_usage),
              action_validate_exception,
              "max transaction cpu usage must be at less than the difference between the max block cpu usage "
              "and the min transaction cpu usage");
...
bool config_entry_validator::operator()(uint32_t id) const {
   ...
   switch (id) {
      case chain_config_v1::max_action_return_value_size_id: {
         ...
         if (!allowed) {
            wlog("action_return_value protocol feature is not active, max_action_return_value_size config is "
                 "not allowed");
         }
      } break;
   }
