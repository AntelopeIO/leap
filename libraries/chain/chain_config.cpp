...
void chain_config_v0::validate() const {
   ...
   EOS_ASSERT(target_block_net_usage_pct >= config::percent_1 / 10,
              action_validate_exception,
              "target block net usage percentage must be at least 0.1%");
   EOS_ASSERT(target_block_cpu_usage_pct <= config::percent_100,
              action_validate_exception,
              "target block cpu usage percentage cannot exceed 100%");
   EOS_ASSERT(target_block_cpu_usage_pct >= config::percent_1 / 10,
              action_validate_exception,
              "target block cpu usage percentage must be at least 0.1%");

   EOS_ASSERT(max_transaction_net_usage < max_block_net_usage,
              action_validate_exception,
              "max transaction net usage must be less than max block net usage");
   EOS_ASSERT(max_transaction_cpu_usage < max_block_cpu_usage,
              action_validate_exception,
              "max transaction cpu usage must be less than max block cpu usage");

   EOS_ASSERT(base_per_transaction_net_usage < max_transaction_net_usage,
              action_validate_exception,
   ...
   EOS_ASSERT(context_free_discount_net_usage_den > 0,
              action_validate_exception,
   ...
   EOS_ASSERT(min_transaction_cpu_usage <= max_transaction_cpu_usage,
              action_validate_exception,
   ...
   EOS_ASSERT(max_transaction_cpu_usage < (max_block_cpu_usage - min_transaction_cpu_usage),
              action_validate_exception,
              "max transaction cpu usage must be at less than the difference between the max block cpu usage and the "
              "min transaction cpu usage");

   EOS_ASSERT(1 <= max_authority_depth, action_validate_exception, "max authority depth should be at least 1");
}
...
bool config_entry_validator::operator()(uint32_t id) const {
   ...
   switch (id) {
      case chain_config_v1::max_action_return_value_size_id: {
         ...
         if (!allowed) {
            wlog("action_return_value protocol feature is not active, max_action_return_value_size config is not "
                 "allowed");
         }
      } break;
   }
