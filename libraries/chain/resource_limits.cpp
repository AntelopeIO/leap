...
void resource_limits_manager::read_from_snapshot( const snapshot_reader_ptr& snapshot ) {
   ...
   resource_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
      ...
      if (std::is_same<value_t, resource_limits_object>::value || std::is_same<value_t, resource_pending_object>::value) {
         ...
         if (std::clamp(header.version, v6::minimum_version, v6::maximum_version) == header.version ) {
            // skip the resource_pending_object as its inlined with resource_limits_object section
            ...
            snapshot->read_section(snapshot_resource_limits_object_v6::section_name, [this]( auto& section ) {
               ...
               while (more) {
                  ...
                  _db.create<resource_limits_object>([&](auto& value) {
            ...
            snapshot->read_section(snapshot_resource_usage_object_v6::section_name, [this]( auto& section ) {
               ...
               while (more) {
                  ...
                  const auto *actual_limits = _db.find<resource_limits_object, by_owner>( snapshot_resource_usage.owner );
                  EOS_ASSERT(actual_limits != nullptr, snapshot_exception, "Unexpected snapshot_resource_usage_object_v6");
                  _db.modify( *actual_limits, [&]( resource_limits_object& value ){
...
void resource_limits_manager::initialize_account(const account_name& account, bool is_trx_transient) {
   ...
   if (auto dm_logger = _get_deep_mind_logger(is_trx_transient)) {
      dm_logger->on_newaccount_resource_limits(limits);
   }
}
...
void resource_limits_manager::update_account_usage(const flat_set<account_name>& accounts, uint32_t time_slot ) {
   ...
   for( const auto& a : accounts ) {
      const auto& usage = _db.get<resource_limits_object,by_owner>( a );
...
void resource_limits_manager::add_pending_ram_usage( const account_name account, int64_t ram_delta, bool is_trx_transient ) {
   ...
   const auto& usage  = _db.get<resource_limits_object,by_owner>( account );
...
int64_t resource_limits_manager::get_account_ram_usage( const account_name& name )const {
   return _db.get<resource_limits_object,by_owner>( name ).ram_usage;
}
...
bool resource_limits_manager::set_account_limits( const account_name& account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight, bool is_trx_transient) {
   ...
   auto find_or_create_pending_limits = [&]() -> const resource_pending_object& {
      ...
      if (pending_limits == nullptr) {
         const auto& actual_limits = _db.get<resource_limits_object, by_owner>( account);
...
void resource_limits_manager::get_account_limits( const account_name& account, int64_t& ram_bytes, int64_t& net_weight, int64_t& cpu_weight ) const {
   ...
   if (pending_buo) {
      ...
      const auto& buo = _db.get<resource_limits_object,by_owner>( account );
...
void resource_limits_manager::process_account_limit_updates() {
   ...
   _db.modify(state, [&](resource_limits_state_object& rso){
      while(pending_idx.begin() != pending_idx.end()) {
         ...
         const auto& actual_entry = _db.get<resource_limits_object, by_owner>(itr->owner);
...
resource_limits_manager::get_account_cpu_limit_ex( const account_name& name, uint32_t greylist_limit, const std::optional<block_timestamp_type>& current_time) const {
   ...
   const auto& state = _db.get<resource_limits_state_object>();
   const auto& usage = _db.get<resource_limits_object, by_owner>(name);
   const auto& config = _db.get<resource_limits_config_object>();
