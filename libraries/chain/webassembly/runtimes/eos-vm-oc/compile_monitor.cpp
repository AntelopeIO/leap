...
struct compile_monitor_session {
   ...
   void read_message_from_nodeos() {
      _nodeos_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         ...
         std::visit(overloaded{ [&, &fds = fds](const compile_wasm_message& compile) {
                                  if (fds.size() != 1) {
                                     connection_dead_signal();
                                     return;
                                  }
   ...
   void read_message_from_compile_task(
      ...
      socket.async_wait(local::datagram_protocol::socket::wait_read, [this, current_compile_it](auto ec) {
         ...
         wasm_compilation_result_message reply{ code,
                                                compilation_result_unknownfailure{},
                                                _allocator->get_free_memory() };
         ...
         try {
            if (success && std::holds_alternative<code_compilation_result_message>(message) && fds.size() == 2) {
               ...
               if (code_ptr == nullptr || mem_ptr == nullptr) {
                  ...
                  reply.result = code_descriptor{ code.code_id,
                                                  code.vm_version,
         ...
         _ctx.post([this, current_compile_it]() { current_compiles.erase(current_compile_it); });
      });
   }
...
wrapped_fd get_connection_to_compile_monitor(int cache_fd) {
   ...
   EOS_ASSERT(std::holds_alternative<initalize_response_message>(message),
              misc_exception,
              "unexpected response from monitor process");
   EOS_ASSERT(!std::get<initalize_response_message>(message).error_message,
              misc_exception,
              "Error message from monitor process: ${e}",
              ("e", *std::get<initalize_response_message>(message).error_message));
