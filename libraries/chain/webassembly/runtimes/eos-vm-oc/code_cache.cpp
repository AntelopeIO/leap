...
code_cache_async::code_cache_async(const bfs::path            data_dir,
...
const code_descriptor* const code_cache_sync::get_descriptor_for_code_sync(const digest_type& code_id,
   ...
   write_message_with_fds(_compile_monitor_write_socket,
   ...
   EOS_ASSERT(std::holds_alternative<wasm_compilation_result_message>(message),
...
code_cache_base::code_cache_base(const boost::filesystem::path data_dir,
                                 const eosvmoc::config&        eosvmoc_config,
                                 const chainbase::database&    db)
   : _db(db)
   ...
   if (!bfs::exists(_cache_file_path)) {
      EOS_ASSERT(eosvmoc_config.cache_size >= allocator_t::get_min_size(total_header_size),
                 database_exception,
   ...
   EOS_ASSERT(cache_header.id == header_id,
              bad_database_version_exception,
              "existing EOS VM OC code cache not compatible with this version");
   ...
   char* code_mapping =
      (char*)mmap(nullptr, eosvmoc_config.cache_size, PROT_READ | PROT_WRITE, MAP_SHARED, _cache_fd, 0);
   ...
   if (cache_header.serialized_descriptor_index) {
      fc::datastream<const char*> ds(code_mapping + cache_header.serialized_descriptor_index,
                                     eosvmoc_config.cache_size - cache_header.serialized_descriptor_index);
      ...
      ilog("EOS VM Optimized Compiler code cache loaded with ${c} entries; ${f} of ${t} bytes free",
           ("c", number_entries)("f", allocator->get_free_memory())("t", allocator->get_size()));
   }
...
void code_cache_base::free_code(const digest_type& code_id, const uint8_t& vm_version) {
   ...
   const std::unordered_map<code_tuple, bool>::iterator compiling_it =
      _outstanding_compiles_and_poison.find({ code_id, vm_version });
