...
static void segv_handler(int sig, siginfo_t* info, void* ctx)
{
   ...
   if ((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
       (uintptr_t)info->si_addr <
          cb_in_main_segment->execution_thread_code_start + cb_in_main_segment->execution_thread_code_length)
...
static intrinsic grow_memory_intrinsic EOSVMOC_INTRINSIC_INIT_PRIORITY(
   "eosvmoc_internal.grow_memory",
   IR::FunctionType::get(IR::ResultType::i32, { IR::ValueType::i32, IR::ValueType::i32 }),
...
static intrinsic eosio_exit_intrinsic(
   "env.eosio_exit",
   IR::FunctionType::get(IR::ResultType::none, { IR::ValueType::i32 }),
   (void*)&eosio_exit,
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context)
{
   ...
   uint64_t max_pages =
      eosio::chain::wasm_constraints::maximum_linear_memory / eosio::chain::wasm_constraints::wasm_page_size;
   ...
   if (code.initdata_prologue_size > memory::max_prologue_size) {
      ...
      memcpy(mem.full_page_memory_base() - code.initdata_prologue_size,
             code_mapping + code.initdata_begin,
             code.initdata_size);
   ...
   context.trx_context.transaction_timer.set_expiration_callback(
      [](void* user) {
   ...
   void (*apply_func)(uint64_t, uint64_t, uint64_t) =
      (void (*)(uint64_t, uint64_t, uint64_t))(cb->running_code_base + code.apply_offset);
      ...
      case 0:
         stack.run([&] {
            std::visit(overloaded{ [&](const no_offset&) {},
                       ...
                       code.start);
            apply_func(context.get_receiver().to_uint64_t(),
                       context.get_action().account.to_uint64_t(),
                       context.get_action().name.to_uint64_t());
         });
         break;
      // case 1: clean eosio_exit
      case EOSVMOC_EXIT_CHECKTIME_FAIL: context.trx_context.checktime(); break;
      case EOSVMOC_EXIT_SEGV: EOS_ASSERT(false, wasm_execution_error, "access violation"); break;
