...
int64_t set_proposed_producers_common(apply_context&               context,
      ...
      std::visit(
         [&p, num_supported_key_types, validate_keys](const auto& a) {
            ...
            for (const auto& kw : a.keys) {
               EOS_ASSERT(kw.key.which() < num_supported_key_types,
   ...
   EOS_ASSERT(producers.size() == unique_producers.size(),
              wasm_execution_error,
...
void interface::set_wasm_parameters_packed(span<const char> packed_parameters) {
   ...
   EOS_ASSERT(version == 0,
              wasm_config_unknown_version,
...
int64_t interface::set_proposed_producers(legacy_span<const char> packed_producer_schedule) {
   ...
   for (const auto& p : old_version) {
      producers.emplace_back(producer_authority{
         p.producer_name, block_signing_authority_v0{1, { { p.block_signing_key, 1 } }}
      });
   }
...
uint32_t interface::get_blockchain_parameters_packed(legacy_span<char> packed_blockchain_parameters) const {
   ...
   if (packed_blockchain_parameters.size() == 0)
      return s;
