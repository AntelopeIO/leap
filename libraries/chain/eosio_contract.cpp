...
void apply_eosio_newaccount(apply_context& context)
{
   ...
   try {
      ...
      db.create<account_metadata_object>([&](auto& a) { a.name = create.name; });
      ...
      const auto& owner_permission =
         authorization.create_permission(create.name, config::owner_name, 0, std::move(create.owner));
      const auto& active_permission = authorization.create_permission(
         create.name, config::active_name, owner_permission.id, std::move(create.active));
...
void apply_eosio_setcode(apply_context& context)
{
   ...
   if (existing_code) {
      ...
      if (old_code_entry.code_ref_count == 1) {
         ...
         context.control.get_wasm_interface().code_block_num_last_used(
            account.code_hash, account.vm_type, account.vm_version, context.control.head_block_num() + 1);
      } else {
         db.modify(old_code_entry, [](code_object& o) { --o.code_ref_count; });
      }
   }

   if (code_size > 0) {
      const code_object* new_code_entry =
         db.find<code_object, by_code_hash>(boost::make_tuple(code_hash, act.vmtype, act.vmversion));
      if (new_code_entry) {
         db.modify(*new_code_entry, [&](code_object& o) { ++o.code_ref_count; });
      } else {
...
void apply_eosio_setabi(apply_context& context)
{
   ...
   db.modify(account, [&](auto& a) { a.abi.assign(act.abi.data(), abi_size); });

   const auto& account_metadata = db.get<account_metadata_object, by_name>(act.account);
   db.modify(account_metadata, [&](auto& a) { a.abi_sequence += 1; });
...
void apply_eosio_updateauth(apply_context& context)
{
   ...
   if (update.auth.waits.size() > 0) {
      ...
      EOS_ASSERT(update.auth.waits.back().wait_sec <= max_delay,
                 action_validate_exception,
   ...
   if (permission) {
      EOS_ASSERT(parent_id == permission->parent,
                 action_validate_exception,
...
void apply_eosio_linkauth(apply_context& context)
{
   ...
   try {
      ...
      if (requirement.requirement != config::eosio_any_name) {
         ...
         EOS_ASSERT(permission != nullptr,
                    permission_query_exception,
                    "Failed to retrieve permission: ${permission}",
      ...
      if (link) {
         ...
         context.add_ram_usage(l.account, (int64_t)(config::billable_size_v<permission_link_object>));
      }
   }
...
void apply_eosio_unlinkauth(apply_context& context)
{
   ...
   context.add_ram_usage(link->account, -(int64_t)(config::billable_size_v<permission_link_object>));
