#include <eosio/chain/qc_database.hpp>
...
*/
   struct qc_database_impl {
      qc_database_impl( qc_database& self, const fc::path& data_dir )
      :self(self)
      ,datadir(data_dir)
      {}
   ...
   fork_database::qc_database( const fc::path& data_dir )
   :my( new qc_database_impl( *this, data_dir ) )
   {}


   void qc_database::open( const std::function<void( block_timestamp_type,
                                                       const flat_set<digest_type>&,
                                                       const vector<digest_type>& )>& validator )
   {
      ...
      auto qc_db_dat = my->datadir / config::qcdb_filename;
      if( fc::exists( qc_db_dat ) ) {
         try {
            //string content;
            //fc::read_file_contents( qc_db_dat, content );
         ...
         } FC_CAPTURE_AND_RETHROW( (qc_db_dat) )

         fc::remove( qc_db_dat );
      }
   }

   void qc_database::close() {
      auto qc_db_dat = my->datadir / config::qcdb_filename;
