...
void deep_mind_handler::on_startup(chainbase::database& db, uint32_t head_block_num) {
   ...
   fc_dlog(_logger,
           ...
           ("block_num", head_block_num)("global_sequence_num",
                                         db.get<dynamic_global_property_object>().global_action_sequence));
   const auto& idx = db.get_index<account_index>();
   for (auto& row : idx.indices()) {
      if (row.abi.size() != 0) {
         fc_dlog(_logger, "ABIDUMP ABI ${contract} ${abi}", ("contract", row.name)("abi", row.abi));
      }
   }
...
void deep_mind_handler::on_switch_forks(const block_id_type& old_head, const block_id_type& new_head) {
   fc_dlog(_logger, "SWITCH_FORK ${from_id} ${to_id}", ("from_id", old_head)("to_id", new_head));
}

void deep_mind_handler::on_onerror(const signed_transaction& etrx) {
   auto packed_trx = fc::raw::pack(etrx);

   fc_dlog(_logger, "TRX_OP CREATE onerror ${id} ${trx}", ("id", etrx.id())("trx", fc::to_hex(packed_trx)));
}

void deep_mind_handler::on_onblock(const signed_transaction& trx) {
   auto packed_trx = fc::raw::pack(trx);

   fc_dlog(_logger, "TRX_OP CREATE onblock ${id} ${trx}", ("id", trx.id())("trx", fc::to_hex(packed_trx)));
}
...
void deep_mind_handler::on_activate_feature(const protocol_feature& feature) {
   fc_dlog(_logger,
           ...
           ("feature_digest", feature.feature_digest)("feature", feature.to_variant()));
}

void deep_mind_handler::on_input_action() {
   fc_dlog(_logger, "CREATION_OP ROOT ${action_id}", ("action_id", _action_id));
}
void deep_mind_handler::on_end_action() {
   ++_action_id;
}
void deep_mind_handler::on_require_recipient() {
   fc_dlog(_logger, "CREATION_OP NOTIFY ${action_id}", ("action_id", _action_id));
}
void deep_mind_handler::on_send_inline() {
   fc_dlog(_logger, "CREATION_OP INLINE ${action_id}", ("action_id", _action_id));
}
...
void deep_mind_handler::on_fail_deferred() {
   fc_dlog(_logger, "DTRX_OP FAILED ${action_id}", ("action_id", _action_id));
}
...
void deep_mind_handler::on_newaccount_resource_limits(const resource_limits::resource_limits_object& limits,
   ...
   fc_dlog(_logger, "RLIMIT_OP ACCOUNT_LIMITS INS ${data}", ("data", limits));
   fc_dlog(_logger, "RLIMIT_OP ACCOUNT_USAGE INS ${data}", ("data", usage));
}
void deep_mind_handler::on_update_account_usage(const resource_limits::resource_usage_object& usage) {
   fc_dlog(_logger, "RLIMIT_OP ACCOUNT_USAGE UPD ${data}", ("data", usage));
}
void deep_mind_handler::on_set_account_limits(const resource_limits::resource_limits_object& limits) {
   fc_dlog(_logger, "RLIMIT_OP ACCOUNT_LIMITS UPD ${data}", ("data", limits));
}
void deep_mind_handler::on_ram_trace(std::string&& event_id,
                                     const char*   family,
...
void deep_mind_handler::on_create_permission(const permission_object& p) {
   fc_dlog(_logger,
           ...
           ("action_id", _action_id)("permission_id", p.id)("data", p));
}
void deep_mind_handler::on_modify_permission(const permission_object& old_permission,
                                             const permission_object& new_permission) {
   fc_dlog(_logger,
           "PERM_OP UPD ${action_id} ${permission_id} ${data}",
           ("action_id", _action_id)("permission_id", new_permission.id)(
              "data", fc::mutable_variant_object()("old", old_permission)("new", new_permission)));
}
void deep_mind_handler::on_remove_permission(const permission_object& permission) {
   fc_dlog(_logger,
           "PERM_OP REM ${action_id} ${permission_id} ${data}",
           ("action_id", _action_id)("permission_id", permission.id)("data", permission));
}
