...
const std::unordered_map<builtin_protocol_feature_t,
                         builtin_protocol_feature_spec,
                         enum_hash<builtin_protocol_feature_t>>
   builtin_protocol_feature_codenames = boost::assign::map_list_of<builtin_protocol_feature_t,
                                                                   builtin_protocol_feature_spec>(
 ...
 // SHA256 hash of the raw message below within the comment delimiters (do not modify message below).
  ...
  Adds privileged intrinsic to enable a contract to pre-activate a protocol feature specified by its
  digest.   Pre-activated protocol features must be activated in the next block.
  */
         {    },
      ...
      builtin_protocol_feature_spec{
         ...
         Ensures transactions generated by contracts for deferred execution are adjusted to avoid transaction
         ID conflicts. Also allows a contract to send a deferred transaction in a manner that enables the
         contract to know the transaction ID ahead of time.
         */
         { builtin_protocol_feature_t::replace_deferred } })(
      ...
      builtin_protocol_feature_spec{
         ...
         Removes the restriction on eosio::linkauth for non-native actions named one of the five special
         action names: updateauth, deleteauth, linkauth, unlinkauth, or canceldelay.
         */
         {} })(
      ...
      builtin_protocol_feature_spec{
         ...
         Disallows bypass of authorization checks by unprivileged contracts when sending inline actions or
         deferred transactions. The original protocol rules allow a bypass of authorization checks for actions
         sent by a contract to itself. This protocol feature removes that bypass.
         */
         {} })(
      ...
      builtin_protocol_feature_spec{
         ...
         Modifies the restrictions on operations within actions that increase RAM usage of accounts other than
         the receiver.

         An unprivileged contract responding to a notification:
         is not allowed to schedule a deferred transaction in which the RAM costs are paid by an account other
         than the receiver; but is allowed to execute database operations that increase RAM usage of an
         account other than the receiver as long as the action's net effect on RAM usage for the account is to
         not increase it.

         An unprivileged contract executing an action (but not as a response to a notification):
         is not allowed to schedule a deferred transaction in which the RAM costs are paid by an account other
         than the receiver unless that account authorized the action; but is allowed to execute database
         operations that increase RAM usage of an account other than the receiver as long as either the
         account authorized the action or the action's net effect on RAM usage for the account is to not
         increase it.
         */
         {} })(
      ...
      builtin_protocol_feature_spec{
         ...
         Enables new `set_action_return_value` intrinsic which sets a value that is included in
         action_receipt.
         */
         {} })(
   ...
   (builtin_protocol_feature_t::get_code_hash,
    builtin_protocol_feature_spec{
      ...
      builtin_protocol_feature_spec{
         ...
         - Add, multiply, and pairing check functions for the alt_bn128 elliptic curve. (alt_bn128_add,
         alt_bn128_mul, alt_bn128_pair)
...
void protocol_feature_base::reflector_init() {
   ...
   if (protocol_feature_type == builtin_protocol_feature::feature_type_string) {
      ...
      EOS_THROW(protocol_feature_validation_exception,
                "Unsupported protocol feature type: ${type}",
                ("type", protocol_feature_type));
   }
}

const char* builtin_protocol_feature::feature_type_string = "builtin";

builtin_protocol_feature::builtin_protocol_feature(
   builtin_protocol_feature_t                      codename,
   const digest_type&                              description_digest,
   flat_set<digest_type>&&                         dependencies,
   const protocol_feature_subjective_restrictions& restrictions)
   : protocol_feature_base(protocol_feature_t::builtin,
                           description_digest,
...
builtin_protocol_feature protocol_feature_set::make_default_builtin_protocol_feature(
   ...
   return {
      itr->first, itr->second.description_digest, std::move(dependencies), itr->second.subjective_restrictions
   };
}
...
const protocol_feature& protocol_feature_set::add_feature(const builtin_protocol_feature& f) {
   ...
   for (const auto& d : f.dependencies) {
      ...
      EOS_ASSERT(itr != _recognized_protocol_features.end(),
                 protocol_feature_exception,
                 "builtin protocol feature with codename '${codename}' and digest of ${digest} has a "
                 "dependency on a protocol feature with digest ${dependency_digest} that is not recognized",
                 ("codename", f.builtin_feature_codename)("digest", feature_digest)("dependency_digest", d));

      if (itr->builtin_feature &&
          expected_builtin_dependencies.find(*itr->builtin_feature) != expected_builtin_dependencies.end()) {
   ...
   if (expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size()) {
      ...
      std::set_difference(expected_builtin_dependencies.begin(),
                          expected_builtin_dependencies.end(),
                          satisfied_builtin_dependencies.begin(),
                          satisfied_builtin_dependencies.end(),
      ...
      for (const auto& builtin_codename : missing_builtins) {
         ...
         EOS_ASSERT(
            itr != builtin_protocol_feature_codenames.end(), protocol_feature_exception, "Unexpected error");
         missing_builtins_with_names.emplace_back(itr->second.codename);
      }

      EOS_THROW(protocol_feature_validation_exception,
                "Not all the builtin dependencies of the builtin protocol feature with codename "
                "'${codename}' and digest of ${digest} were satisfied.",
                ("missing_dependencies", missing_builtins_with_names));
   }

   auto res = _recognized_protocol_features.insert(
      protocol_feature{ feature_digest,
   ...
   EOS_ASSERT(res.second,
              protocol_feature_exception,
              "builtin protocol feature with codename '${codename}' has a digest of ${digest} but another "
              "protocol feature with the same digest has already been added",
...
void protocol_feature_manager::activate_feature(const digest_type& feature_digest,
   ...
   EOS_ASSERT(itr != _protocol_feature_set.end(),
              protocol_feature_exception,
              "unrecognized protocol feature digest: ${digest}",
              ("digest", feature_digest));

   if (_activated_protocol_features.size() > 0) {
      const auto& last = _activated_protocol_features.back();
      EOS_ASSERT(
         last.activation_block_num <= current_block_num,
         protocol_feature_exception,
   ...
   EOS_ASSERT(_builtin_protocol_features[indx].activation_block_num ==
...
std::optional<builtin_protocol_feature> read_builtin_protocol_feature(const fc::path& p) {
   try {
      ...
      dlog("unknown problem encountered while reading '${path}'", ("path", p.generic_string()));
   }
   return {};
}
...
protocol_feature_set initialize_protocol_features(const fc::path& p, bool populate_missing_builtins) {
      ...
      if (f.subjective_restrictions.enabled) {
         if (f.subjective_restrictions.preactivation_required) {
            if (f.subjective_restrictions.earliest_allowed_activation_time == time_point{}) {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is "
                    "enabled with preactivation required",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest",
                                                                                      feature_digest));
            } else {
               ilog(
                  "Support for builtin protocol feature '${codename}' (with digest of '${digest}') is "
                  "enabled with preactivation required and with an earliest allowed activation time of "
                  "${earliest_time}",
            ...
            if (f.subjective_restrictions.earliest_allowed_activation_time == time_point{}) {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is "
                    "enabled without activation restrictions",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest",
                                                                                      feature_digest));
            } else {
               ilog(
                  "Support for builtin protocol feature '${codename}' (with digest of '${digest}') is "
                  "enabled without preactivation required but with an earliest allowed activation time of "
                  "${earliest_time}",
                  ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                     "earliest_time", f.subjective_restrictions.earliest_allowed_activation_time));
            }
         ...
         ilog("Recognized builtin protocol feature '${codename}' (with digest of '${digest}') but support "
              "for it is not enabled",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest));
      }
   };
      ...
      for (directory_iterator enditr, itr{ p }; itr != enditr; ++itr) {
         ...
         EOS_ASSERT(res.second,
                    ...
                    ("codename", builtin_protocol_feature_codename(f->get_codename()))(
                       "current_file", file_path.generic_string())("previous_file",
                                                                   res.first->second.generic_string()));
   ...
   // Add builtin protocol features to the protocol feature manager in the right order (to satisfy
   // dependencies)
   ...
   std::function<void(const itr_type&)> add_protocol_feature =
      [&pfs,
       &builtin_protocol_features_to_add,
       &visited_builtins,
       &log_recognized_protocol_feature,
       &add_protocol_feature](const itr_type& itr) -> void {
   ...
   for (auto itr = builtin_protocol_features_to_add.begin(); itr != builtin_protocol_features_to_add.end();
        ++itr) {
   ...
   auto output_protocol_feature = [&p](const builtin_protocol_feature& f, const digest_type& feature_digest) {
      ...
      EOS_ASSERT(!fc::exists(file_path),
                 plugin_exception,
                 "Could not save builtin protocol feature with codename '${codename}' because a file at the "
                 "following path already exists: ${path}",
                 ("codename",
                  builtin_protocol_feature_codename(f.get_codename()))("path", file_path.generic_string()));

      if (fc::json::save_to_file(f, file_path)) {
         ilog("Saved default specification for builtin protocol feature '${codename}' (with digest of "
              "'${digest}') to: ${path}",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                 "path", file_path.generic_string()));
      } else {
         elog("Error occurred while writing default specification for builtin protocol feature '${codename}' "
              "(with digest of '${digest}') to: ${path}",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                 "path", file_path.generic_string()));
      }
   };
   ...
   std::function<digest_type(builtin_protocol_feature_t)> add_missing_builtins =
      [&pfs,
       &visited_builtins,
       &output_protocol_feature,
       &log_recognized_protocol_feature,
       &add_missing_builtins,
       populate_missing_builtins](builtin_protocol_feature_t codename) -> digest_type {
      ...
      auto f = protocol_feature_set::make_default_builtin_protocol_feature(
         codename, [&add_missing_builtins](builtin_protocol_feature_t d) { return add_missing_builtins(d); });
