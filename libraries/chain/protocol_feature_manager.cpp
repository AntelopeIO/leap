...
const std::unordered_map<builtin_protocol_feature_t,
  ...
  */
      ...
      builtin_protocol_feature_spec{
         ...
         Disallows bypass of authorization checks by unprivileged contracts when sending inline actions or deferred
         transactions. The original protocol rules allow a bypass of authorization checks for actions sent by a contract
         to itself. This protocol feature removes that bypass.
         */
      ...
      builtin_protocol_feature_spec{
         ...
         Modifies the restrictions on operations within actions that increase RAM usage of accounts other than the
         receiver.

         An unprivileged contract responding to a notification:
         is not allowed to schedule a deferred transaction in which the RAM costs are paid by an account other than the
         receiver; but is allowed to execute database operations that increase RAM usage of an account other than the
         receiver as long as the action's net effect on RAM usage for the account is to not increase it.

         An unprivileged contract executing an action (but not as a response to a notification):
         is not allowed to schedule a deferred transaction in which the RAM costs are paid by an account other than the
         receiver unless that account authorized the action; but is allowed to execute database operations that increase
         RAM usage of an account other than the receiver as long as either the account authorized the action or the
         action's net effect on RAM usage for the account is to not increase it.
         */
...
protocol_feature_base::protocol_feature_base(protocol_feature_t                              feature_type,
   ...
   , _type(feature_type) {
   switch (feature_type) {
      case protocol_feature_t::builtin: protocol_feature_type = builtin_protocol_feature::feature_type_string; break;
      default: {
...
bool protocol_feature_set::validate_dependencies(const digest_type&                             feature_digest,
...
const protocol_feature& protocol_feature_set::add_feature(const builtin_protocol_feature& f) {
   ...
   for (const auto& d : f.dependencies) {
      ...
      EOS_ASSERT(itr != _recognized_protocol_features.end(),
                 protocol_feature_exception,
                 "builtin protocol feature with codename '${codename}' and digest of ${digest} has a dependency on a "
                 "protocol feature with digest ${dependency_digest} that is not recognized",
   ...
   if (expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size()) {
      ...
      for (const auto& builtin_codename : missing_builtins) {
         ...
         EOS_ASSERT(itr != builtin_protocol_feature_codenames.end(), protocol_feature_exception, "Unexpected error");
         missing_builtins_with_names.emplace_back(itr->second.codename);
      }

      EOS_THROW(protocol_feature_validation_exception,
                "Not all the builtin dependencies of the builtin protocol feature with codename '${codename}' and "
                "digest of ${digest} were satisfied.",
                ("missing_dependencies", missing_builtins_with_names));
   }

   auto res =
      _recognized_protocol_features.insert(protocol_feature{ feature_digest,
   ...
   EOS_ASSERT(res.second,
              protocol_feature_exception,
              "builtin protocol feature with codename '${codename}' has a digest of ${digest} but another protocol "
              "feature with the same digest has already been added",
...
uint32_t protocol_feature_manager::const_iterator::activation_ordinal() const {
   ...
   EOS_ASSERT(_index != end_index, protocol_feature_iterator_exception, "called activation_ordinal() on end iterator");

   return _index;
}

uint32_t protocol_feature_manager::const_iterator::activation_block_num() const {
   EOS_ASSERT(_pfm, protocol_feature_iterator_exception, "called activation_block_num() on singular iterator");
   EOS_ASSERT(
      _index != end_index, protocol_feature_iterator_exception, "called activation_block_num() on end iterator");
...
protocol_feature_set initialize_protocol_features(const fc::path& p, bool populate_missing_builtins) {
      ...
      if (f.subjective_restrictions.enabled) {
         if (f.subjective_restrictions.preactivation_required) {
            if (f.subjective_restrictions.earliest_allowed_activation_time == time_point{}) {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is enabled with "
                    "preactivation required",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest));
            } else {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is enabled with "
                    "preactivation required and with an earliest allowed activation time of ${earliest_time}",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                       "earliest_time", f.subjective_restrictions.earliest_allowed_activation_time));
            }
         } else {
            if (f.subjective_restrictions.earliest_allowed_activation_time == time_point{}) {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is enabled "
                    "without activation restrictions",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest));
            } else {
               ilog("Support for builtin protocol feature '${codename}' (with digest of '${digest}') is enabled "
                    "without preactivation required but with an earliest allowed activation time of ${earliest_time}",
                    ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                       "earliest_time", f.subjective_restrictions.earliest_allowed_activation_time));
            }
         }
      } else {
         ilog("Recognized builtin protocol feature '${codename}' (with digest of '${digest}') but support for it is "
              "not enabled",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest));
      }
   };
      ...
      for (directory_iterator enditr, itr{ p }; itr != enditr; ++itr) {
         ...
         EOS_ASSERT(
            ...
            ("codename", builtin_protocol_feature_codename(f->get_codename()))(
               "current_file", file_path.generic_string())("previous_file", res.first->second.generic_string()));

         const auto feature_digest = f->digest();

         builtin_protocol_features_to_add.emplace(
            std::piecewise_construct, std::forward_as_tuple(feature_digest), std::forward_as_tuple(*f, false));
      }
   }
   ...
   std::function<void(const itr_type&)> add_protocol_feature = [&pfs,
   ...
   auto output_protocol_feature = [&p](const builtin_protocol_feature& f, const digest_type& feature_digest) {
      ...
      EOS_ASSERT(!fc::exists(file_path),
                 plugin_exception,
                 "Could not save builtin protocol feature with codename '${codename}' because a file at the following "
      ...
      if (fc::json::save_to_file(f, file_path)) {
         ilog("Saved default specification for builtin protocol feature '${codename}' (with digest of '${digest}') to: "
              "${path}",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                 "path", file_path.generic_string()));
      } else {
         elog("Error occurred while writing default specification for builtin protocol feature '${codename}' (with "
              "digest of '${digest}') to: ${path}",
              ("codename", builtin_protocol_feature_codename(f.get_codename()))("digest", feature_digest)(
                 "path", file_path.generic_string()));
      }
   };
