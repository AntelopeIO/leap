...
void compute_and_print_timer_accuracy(platform_timer& timer) {
   ...
   bacc::accumulator_set<int, bacc::stats<bacc::tag::mean, bacc::tag::min, bacc::tag::max, bacc::tag::variance>, float>
      samples;
...
#define TIMER_STATS                                                                                                    \
   ...
   if (bacc::mean(samples) + sqrt(bacc::variance(samples)) * 2 > 250)
      wlog("Checktime timer accuracy on this platform and hardware combination is poor; accuracy of subjective "
           "transaction deadline enforcement will suffer");
