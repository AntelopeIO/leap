...
void istream_snapshot_reader::validate() const {
   ...
   try {
      ...
      EOS_ASSERT(actual_version == expected_version,
                 snapshot_exception,
      ...
      snapshot_exception fce(
         FC_LOG_MESSAGE(warn, "Binary snapshot validation threw IO exception (${what})", ("what", e.what())));
      throw fce;
   }
}
...
void istream_json_snapshot_reader::validate() const {
   try {
      ...
      snapshot_exception fce(
         FC_LOG_MESSAGE(warn, "JSON snapshot validation threw IO exception (${what})", ("what", e.what())));
      throw fce;
   }
}
...
void istream_json_snapshot_reader::set_section(const string& section_name) {
   EOS_ASSERT(impl->doc.HasMember(section_name.c_str()),
              ...
              ("sec", section_name));
   EOS_ASSERT(impl->doc[section_name.c_str()].HasMember("num_rows"),
              snapshot_exception,
              "JSON snapshot ${sec} num_rows not found",
              ("sec", section_name));
   EOS_ASSERT(impl->doc[section_name.c_str()].HasMember("rows"),
              snapshot_exception,
              "JSON snapshot ${sec} rows not found",
              ("sec", section_name));
   EOS_ASSERT(impl->doc[section_name.c_str()]["rows"].IsArray(),
              snapshot_exception,
              "JSON snapshot ${sec} rows is not an array",
