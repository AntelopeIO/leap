...
public:
   ...
   public:
            ...
            if (auto dm_logger = context.control.get_deep_mind_logger()) {
               std::string event_id =
                  RAM_EVENT_ID("${code}:${scope}:${table}:${index_name}",
                               ("code", t.code)("scope", t.scope)("table", t.table)("index_name", name(id)));
      ...
      void remove(int iterator) {
         ...
         context.db.modify(table_obj, [&](auto& t) { --t.count; });
      ...
      void update(int iterator, account_name payer, secondary_key_proxy_const_type secondary) {
         ...
         if (obj.payer != payer) {
            ...
            if (auto dm_logger = context.control.get_deep_mind_logger()) {
               dm_logger->on_ram_trace(
                  std::move(event_id), "secondary_index", "add", "secondary_index_update_add_new_payer");
            }
      ...
      int find_secondary(uint64_t                       code,
                         uint64_t                       scope,
                         uint64_t                       table,
         ...
         const auto* obj =
            context.db.find<ObjectType, by_secondary>(secondary_key_helper_t::create_tuple(*tab, secondary));
         if (!obj)
      ...
      int lowerbound_secondary(uint64_t                 code,
                               uint64_t                 scope,
                               uint64_t                 table,
      ...
      int upperbound_secondary(uint64_t                 code,
                               uint64_t                 scope,
                               uint64_t                 table,
                               secondary_key_proxy_type secondary,
                               uint64_t&                primary) {
      ...
      int previous_secondary(int iterator, uint64_t& primary) {
         ...
         auto itr = idx.iterator_to(obj);
         if (itr == idx.begin())
            return -1; // cannot decrement past beginning iterator of index
      ...
      int find_primary(uint64_t                 code,
                       uint64_t                 scope,
                       uint64_t                 table,
                       secondary_key_proxy_type secondary,
                       uint64_t                 primary) {
...
public:
   ...
   void schedule_deferred_transaction(const uint128_t& sender_id,
                                      account_name     payer,
                                      transaction&&    trx,
...
public:
   ...
   void get_code_hash(account_name account,
   ...
   /// Console methods:
public:
   void console_append(std::string_view val) { _pending_console_output += val; }

   /// Database methods:
public:
   void update_db_usage(const account_name& payer, int64_t delta);

   int  db_store_i64(name                scope,
                     name                table,
                     const account_name& payer,
                     uint64_t            id,
                     const char*         buffer,
...
// FC_REFLECT(eosio::chain::apply_context::apply_results,
// (applied_actions)(deferred_transaction_requests)(deferred_transactions_count))
