...
struct chain_config_v0 {
   ...
   friend inline bool operator==(const chain_config_v0& lhs, const chain_config_v0& rhs) {
      return std::tie(lhs.max_block_net_usage,
                      ...
                      lhs.max_authority_depth) == std::tie(rhs.max_block_net_usage,
...
protected:
   ...
   Stream& log(Stream& out) const {
      return out << "Max Block Net Usage: " << max_block_net_usage << ", "
                 ...
                 << ((double)target_block_cpu_usage_pct / (double)config::percent_1) << "%, "
...
inline DataStream& operator<<(DataStream&                                                           s,
   ...
   switch (entry.id) {
      case chain_config_v0::max_block_net_usage_id: fc::raw::pack(s, entry.config.max_block_net_usage); break;
      ...
      case chain_config_v0::base_per_transaction_net_usage_id:
         ...
         break;
      case chain_config_v0::net_usage_leeway_id: fc::raw::pack(s, entry.config.net_usage_leeway); break;
      ...
      case chain_config_v0::context_free_discount_net_usage_den_id:
         ...
         break;
      case chain_config_v0::max_block_cpu_usage_id: fc::raw::pack(s, entry.config.max_block_cpu_usage); break;
...
inline DataStream& operator>>(
   ...
   switch (entry.id) {
      ...
      case chain_config_v0::base_per_transaction_net_usage_id:
         ...
         break;
      case chain_config_v0::net_usage_leeway_id: fc::raw::unpack(s, entry.config.net_usage_leeway); break;
      case chain_config_v0::context_free_discount_net_usage_num_id:
...
inline DataStream& operator<<(
   ...
   for (auto uid : selection.ids) {
      uint32_t id = uid;
      EOS_ASSERT(id < visited.size(),
                 config_parse_error,
                 "provided id ${id} should be less than ${size}",
                 ("id", id)("size", visited.size()));
...
inline DataStream& operator>>(DataStream&                                                        s,
   ...
   for (uint32_t i = 0; i < length; ++i) {
      ...
      EOS_ASSERT(id.value < visited.size(),
                 config_parse_error,
                 "provided id ${id} should be less than ${size}",
                 ("id", id)("size", visited.size()));
      EOS_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
