...
inline constexpr bool operator==(const wasm_config& lhs, const wasm_config& rhs) {
   return lhs.max_mutable_global_bytes == rhs.max_mutable_global_bytes &&
          lhs.max_table_elements == rhs.max_table_elements && lhs.max_section_elements == rhs.max_section_elements &&
          lhs.max_linear_memory_init == rhs.max_linear_memory_init &&
          lhs.max_func_local_bytes == rhs.max_func_local_bytes &&
          lhs.max_nested_structures == rhs.max_nested_structures && lhs.max_symbol_bytes == rhs.max_symbol_bytes &&
          lhs.max_module_bytes == rhs.max_module_bytes && lhs.max_code_bytes == rhs.max_code_bytes &&
          lhs.max_pages == rhs.max_pages && lhs.max_call_depth == rhs.max_call_depth;
}
inline constexpr bool operator!=(const wasm_config& lhs, const wasm_config& rhs) {
   return !(lhs == rhs);
}
