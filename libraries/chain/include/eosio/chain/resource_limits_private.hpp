...
struct exponential_moving_average_accumulator {
   ...
   void add(uint64_t units, uint32_t ordinal, uint32_t window_size /* must be positive */) {
      ...
      auto value_ex_contrib =
         downgrade_cast<uint64_t>(integer_divide_ceil((uint128_t)units * Precision, (uint128_t)window_size));
...
struct exponential_decay_accumulator {
   ...
   uint64_t value_ex_at(uint32_t ordinal, uint32_t window_size) const {
      if (last_ordinal < ordinal) {
         if ((uint64_t)last_ordinal + window_size > (uint64_t)ordinal) {
            const auto delta = ordinal - last_ordinal; // clearly 0 < delta < window_size
            const auto decay = make_ratio((uint128_t)window_size - delta, (uint128_t)window_size);
   ...
   void add(uint64_t units, uint32_t ordinal, uint32_t window_size /* must be positive */) {
      ...
      // saturate the value
      uint128_t new_value_ex =
         std::min<uint128_t>(units_ex + (uint128_t)value_ex, std::numeric_limits<uint64_t>::max());
...
FC_REFLECT(
   ...
   (cpu_limit_parameters)(net_limit_parameters)(account_cpu_usage_average_window)(account_net_usage_average_window))
FC_REFLECT(
   eosio::chain::resource_limits::resource_limits_state_object,
   (average_block_net_usage)(average_block_cpu_usage)(pending_net_usage)(pending_cpu_usage)(total_net_weight)(total_cpu_weight)(total_ram_bytes)(virtual_net_limit)(virtual_cpu_limit))
