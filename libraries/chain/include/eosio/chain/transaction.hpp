...
struct deferred_transaction_generation_context : fc::reflect_init {
   ...
   deferred_transaction_generation_context(const transaction_id_type& sender_trx_id,
                                           uint128_t                  sender_id,
                                           account_name               sender)
...
struct signed_transaction : public transaction {
   ...
   fc::microseconds      get_signature_keys(const chain_id_type&       chain_id,
                                            fc::time_point             deadline,
                                            flat_set<public_key_type>& recovered_pub_keys,
                                            bool                       allow_duplicate_keys = false) const;
};
...
struct packed_transaction : fc::reflect_init {
   ...
   packed_transaction(bytes&&                  packed_txn,
   ...
   packed_transaction(bytes&&                  packed_txn,
                      ...
                      vector<bytes>&&          cfd,
                      compression_type         _compression);
   packed_transaction(transaction&&            t,
                      vector<signature_type>&& sigs,
                      bytes&&                  packed_cfd,
                      compression_type         _compression);
...
FC_REFLECT(eosio::chain::transaction_header,
