...
const static name majority_producers_permission_name{
   "prod.major"_n
}; // greater than 1/2 of producers needed to authorize
const static name minority_producers_permission_name{
   "prod.minor"_n
}; // greater than 1/3 of producers needed to authorize0
...
const static uint32_t default_min_transaction_cpu_usage =
   100; /// min trx cpu usage in microseconds (10000 TPS equiv)
const static uint32_t default_subjective_cpu_leeway_us =
   31000; /// default subjective cpu leeway in microseconds
...
const static uint32_t default_block_cpu_effort_pct =
   80 * percent_1; // percentage of block time used for producing block
...
static_assert(MAX_SIZE_OF_BYTE_ARRAYS == 20 * 1024 * 1024,
              "Changing MAX_SIZE_OF_BYTE_ARRAYS breaks consensus. Make sure this is expected");
...
// (unless net_usage_leeway is set to 0 and so are the net limits of all accounts that can help with resetting
// blockchain parameters).

const static uint32_t fixed_net_overhead_of_packed_trx = 16; // TODO: is this reasonable?

const static uint32_t fixed_overhead_shared_vector_ram_bytes =
   16; ///< overhead accounts for fixed portion of size of shared_vector field
const static uint32_t overhead_per_row_per_index_ram_bytes =
   32; ///< overhead accounts for basic tracking structures in a row per index
const static uint32_t overhead_per_account_ram_bytes =
   2 * 1024; ///< overhead accounts for basic account storage and pre-pays features like account recovery
const static uint32_t setcode_ram_bytes_multiplier =
   10; ///< multiplier on contract size to account for multiple copies and cached compilation
...
const static eosio::chain::wasm_interface::vm_type default_wasm_runtime =
   eosio::chain::wasm_interface::vm_type::eos_vm_jit;
#else
const static eosio::chain::wasm_interface::vm_type default_wasm_runtime =
   eosio::chain::wasm_interface::vm_type::eos_vm;
#endif

const static uint32_t default_abi_serializer_max_time_us =
