 ...
 *
 * Takes two parallel ranges, a Data range containing data values, and a Marker range containing markers on
 * the corresponding data values. Returns a new Data range containing only the values corresponding to markers
 * which match markerValue
 *
 * For example:
 * @code{.cpp}
...
DataRange filter_data_by_marker(DataRange data, MarkerRange markers, const Marker& markerValue) {
   ...
   // Zip the ranges together, filter out data with markers that don't match, and return the data without the
   // markers
