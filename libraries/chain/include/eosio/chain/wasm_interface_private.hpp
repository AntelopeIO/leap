...
struct wasm_interface_impl {
...
#ifdef EOSIO_EOS_VM_OC_RUNTIME_ENABLED
      if (eosvmoc_tierup) {
         EOS_ASSERT(vm != wasm_interface::vm_type::eos_vm_oc,
                    wasm_exception,
                    "You can't use EOS VM OC as the base runtime when tier up is activated");
   ...
   const std::unique_ptr<wasm_instantiated_module_interface>& get_instantiated_module(
      ...
      if (!it->module) {
         ...
         IR::Module      module;
         std::vector<U8> bytes = { (const U8*)codeobject->code.data(),
