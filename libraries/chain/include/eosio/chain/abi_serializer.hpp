...
struct abi_serializer
{
   ...
   [[deprecated("use the overload with "
                "yield_function_t[=create_yield_function(max_serialization_time)]")]] abi_serializer(
      const abi_def&          abi,
   ...
   fc::variant binary_to_variant(const std::string_view& type,
                                 const bytes&            binary,
   ...
   [[deprecated(
      "use the overload with yield_function_t[=create_yield_function(max_serialization_time)]")]] bytes
         variant_to_binary(const std::string_view& type,
                           ...
                           const fc::microseconds& max_serialization_time,
                           bool                    short_path = false) const;
   bytes variant_to_binary(const std::string_view& type,
                           const fc::variant&      var,
                           const yield_function_t& yield,
                           bool                    short_path = false) const;
   [[deprecated(
      "use the overload with yield_function_t[=create_yield_function(max_serialization_time)]")]] void
        variant_to_binary(const std::string_view& type,
                          ...
                          const fc::microseconds& max_serialization_time,
                          bool                    short_path = false) const;
   void variant_to_binary(const std::string_view& type,
                          const fc::variant&      var,
                          fc::datastream<char*>&  ds,
                          const yield_function_t& yield,
                          bool                    short_path = false) const;
...
private:
   ...
   fc::variant _binary_to_variant(const std::string_view&          type,
                                  const bytes&                     binary,
                                  impl::binary_to_variant_context& ctx) const;
   fc::variant _binary_to_variant(const std::string_view&          type,
                                  fc::datastream<const char*>&     binary,
                                  impl::binary_to_variant_context& ctx) const;
   void        _binary_to_variant(const std::string_view&          type,
                                  fc::datastream<const char*>&     stream,
                                  fc::mutable_variant_object&      obj,
                                  impl::binary_to_variant_context& ctx) const;

   bytes _variant_to_binary(const std::string_view&          type,
                            const fc::variant&               var,
                            impl::variant_to_binary_context& ctx) const;
   void  _variant_to_binary(const std::string_view&          type,
                            const fc::variant&               var,
                            fc::datastream<char*>&           ds,
                            impl::variant_to_binary_context& ctx) const;
...
{
   abi_traverse_context_with_path(const abi_serializer&            abis,
                                  abi_serializer::yield_function_t yield,
                                  const std::string_view&          type)
      : abi_traverse_context(std::move(yield))
      , abis(abis)
   {
      set_path_root(type);
   }
...
{
   return std::is_base_of<transaction, T>::value || std::is_same<T, packed_transaction>::value ||
          std::is_same<T, transaction_trace>::value || std::is_same<T, transaction_receipt>::value ||
          std::is_same<T, action_trace>::value || std::is_same<T, signed_transaction>::value ||
          std::is_same<T, signed_block>::value || std::is_same<T, action>::value;
}
...
{
   static constexpr bool value() { return single_type_requires_abi_v<T>(); }
};
...
struct abi_to_variant
{
   ...
   template<typename Resolver>
   static bool add_special_logging(mutable_variant_object& mvo,
                                   const char*             name,
                                   const action&           act,
                                   Resolver&               resolver,
                                   abi_traverse_context&   ctx)
    ...
    */
   template<typename Resolver>
   static void add(mutable_variant_object& out,
                   const char*             name,
                   const action&           act,
                   Resolver                resolver,
                   abi_traverse_context&   ctx)
    ...
    *
    * This matches the FC_REFLECT for this type, but this is provided to extract the contents of
    * action_trace.return_value
    * @tparam Resolver
    * @param action_trace
    * @param resolver
    * @return
                   ...
                   abi_traverse_context&   ctx)
   {
      ...
      if (exts.count(deferred_transaction_generation_context::extension_id()) > 0) {
         const auto& deferred_transaction_generation = std::get<deferred_transaction_generation_context>(
            exts.lower_bound(deferred_transaction_generation_context::extension_id())->second);
                   ...
                   abi_traverse_context&   ctx)
   {
      ...
      if (header_exts.count(protocol_feature_activation::extension_id() > 0)) {
         const auto& new_protocol_features =
            std::get<protocol_feature_activation>(
               header_exts.lower_bound(protocol_feature_activation::extension_id())->second)
               .protocol_features;
      ...
      if (header_exts.count(producer_schedule_change_extension::extension_id())) {
         const auto& new_producer_schedule = std::get<producer_schedule_change_extension>(
            header_exts.lower_bound(producer_schedule_change_extension::extension_id())->second);
      ...
      if (block_exts.count(additional_block_signatures_extension::extension_id()) > 0) {
         const auto& additional_signatures = std::get<additional_block_signatures_extension>(
            block_exts.lower_bound(additional_block_signatures_extension::extension_id())->second);
...
struct abi_from_variant
{
   ...
   static void extract(const fc::variant& v, action& act, Resolver resolver, abi_traverse_context& ctx)
   {
      ...
      EOS_ASSERT(valid_empty_data || !act.data.empty(),
...
void abi_to_variant::add(mutable_variant_object& mvo,
                         const char*             name,
                         const M&                v,
                         Resolver                resolver,
                         abi_traverse_context&   ctx)
...
template<typename T, typename Resolver>
void abi_serializer::to_variant(const T& o, fc::variant& vo, Resolver resolver, const yield_function_t& yield)
try {
   mutable_variant_object     mvo;
