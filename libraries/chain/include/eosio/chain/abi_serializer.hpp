      ...
      */
      ...
      static void add( mutable_variant_object &out, const char* name, const signed_block& block, const Resolver& resolver, abi_traverse_context& ctx )
      {
         ...
         if ( qc_extension_count > 0) {
            EOS_ASSERT(qc_extension_count == 1, ill_formed_quorum_certificate_extension, "At most one quorum certificate is allowed in block extension, which has ${c}.", ("c", qc_extension_count));
