...
struct shared_public_key {
   ...
   std::string to_string() const { return this->operator public_key_type().to_string(); }
   ...
   friend bool operator==(const shared_public_key& lhs, const shared_public_key& rhs) {
      ...
      return std::visit(
         overloaded{ [&](const fc::ecc::public_key_shim& k1) {
                     ...
                     [&](const shared_string& wa) { return wa == std::get<shared_string>(rhs.pubkey); } },
         lhs.pubkey);
   }
   ...
   friend bool operator==(const shared_public_key& l, const public_key_type& r) {
      ...
      return std::visit(overloaded{ [&](const fc::ecc::public_key_shim& k1) {
...
inline bool validate(const Authority& auth) {
   ...
   }
   {
      ...
      for (const auto& w : auth.waits) {
         if (prev && (prev->wait_sec >= w.wait_sec))
            return false;
