...
private:
   ...
   typedef multi_index_container<
      ...
      indexed_by<
         ...
         ordered_non_unique<
            tag<by_expiry>,
            const_mem_fun<unapplied_transaction, fc::time_point_sec, &unapplied_transaction::expiration>>>>
   ...
   void add_incoming(const transaction_metadata_ptr& trx, bool api_trx, bool return_failure_trace, next_func_t next) {
      ...
      if (itr == queue.get<by_trx_id>().end()) {
         auto insert_itr = queue.insert({ trx,
                                          api_trx ? trx_enum_type::incoming_api : trx_enum_type::incoming_p2p,
...
private:
   ...
   void added(Itr itr) {
      ...
      if (itr->trx_type == trx_enum_type::incoming_p2p || itr->trx_type == trx_enum_type::incoming_api) {
         ++incoming_count;
         EOS_ASSERT(size_in_bytes + size < max_transaction_queue_size,
