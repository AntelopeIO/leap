...
Float parseNaN(const char*& nextChar, ParseState& state)
{
   ...
   WAVM_ASSERT_THROW(nextChar[0] == 'n' && nextChar[1] == 'a' && nextChar[2] == 'n');
   nextChar += 3;
...
Float parseFloat(const char*& nextChar, ParseState& state)
{
   ...
   if (hasUnderscores) {
      firstChar = noUnderscoreString.c_str();
   }
   ...
   switch (state.nextToken->type) {
      case t_decimalInt:
         ...
         u64        = parseDecimalUnsignedInt(
            nextChar, state, isNegative ? U64(-minSignedValue) : maxUnsignedValue, "int literal");
         break;
      case t_hexInt:
         isNegative = parseSign(nextChar);
...
bool tryParseFloat(ParseState& state, Float& outFloat)
{
   ...
   switch (state.nextToken->type) {
      ...
      default: parseErrorf(state, state.nextToken, "expected float literal"); return false;
   };
