...
static Uptr createImport(ParseState&                   state,
                         Name                          name,
...
static void parseImport(ModuleParseState& state) {
   ...
   parseParenthesized(state, [&] {
      ...
      switch (importKindToken->type) {
         ...
         case t_table:
         case t_memory:
         case t_global: ++state.nextToken; break;
         default: parseErrorf(state, state.nextToken, "invalid import type"); throw RecoverParseException();
      }
      ...
      switch (importKindToken->type) {
         case t_func: {
            ...
            const UnresolvedFunctionType unresolvedFunctionType =
               parseFunctionTypeRefAndOrDecl(state, localNameToIndexMap, localDissassemblyNames);
            const Uptr importIndex                         = createImport(state,
                                                  name,
                                                  std::move(moduleName),
                                                  std::move(exportName),
         ...
         case t_global: {
            ...
            createImport(state,
                         ...
                         state.disassemblyNames.globals,
                         globalType);
            break;
         }
...
static void parseExport(ModuleParseState& state) {
   ...
   parseParenthesized(state, [&] {
      ...
      switch (state.nextToken->type) {
         ...
         default: parseErrorf(state, state.nextToken, "invalid export kind"); throw RecoverParseException();
      };
      ++state.nextToken;
      ...
      state.postDeclarationCallbacks.push_back([=](ModuleParseState& state) {
         ...
         switch (exportKind) {
            case ObjectKind::function:
               exportedObjectIndex =
                  resolveRef(state, state.functionNameToIndexMap, state.module.functions.size(), exportRef);
               break;
            case ObjectKind::table:
               exportedObjectIndex =
                  resolveRef(state, state.tableNameToIndexMap, state.module.tables.size(), exportRef);
               break;
            case ObjectKind::memory:
               exportedObjectIndex =
                  resolveRef(state, state.memoryNameToIndexMap, state.module.memories.size(), exportRef);
               break;
            case ObjectKind::global:
               exportedObjectIndex =
                  resolveRef(state, state.globalNameToIndexMap, state.module.globals.size(), exportRef);
               break;
            default: Errors::unreachable();
         }
      });
   ...
   state.postDeclarationCallbacks.push_back([tableRef, tableSegmentIndex, elementReferences, elemToken](
      ...
      if (!state.module.tables.size()) {
         ...
         tableSegment.tableIndex =
            tableRef ? resolveRef(state, state.tableNameToIndexMap, state.module.tables.size(), tableRef) : 0;
...
static void parseObjectDefOrImport(ModuleParseState&             state,
                                   NameToIndexMap&               nameToIndexMap,
...
static void parseFunc(ModuleParseState& state) {
   parseObjectDefOrImport(
      ...
      [&](ModuleParseState& state) {
         ...
         std::vector<std::string>     localDisassemblyNames;
         const UnresolvedFunctionType unresolvedFunctionType =
            parseFunctionTypeRefAndOrDecl(state, localNameToIndexMap, localDisassemblyNames);
         ...
         if (!hasSizeConstraints) {
            parseParenthesized(state, [&] {
               ...
               const Uptr numElements = parseElemSegmentBody(
                  state, Reference(U32(tableIndex)), InitializerExpression((I32)0), state.nextToken - 1);
      ...
      [](ModuleParseState& state, const Token*) {
         ...
         if (!tryParseSizeConstraints(state, IR::maxMemoryPages, sizeConstraints)) {
            ...
            state.module.dataSegments.push_back(
               { state.module.memories.size(), InitializerExpression(I32(0)), std::move(dataVector) });
         }
...
static void parseStart(ModuleParseState& state) {
   ...
   state.postDeclarationCallbacks.push_back([functionRef](ModuleParseState& state) {
      state.module.startFunctionIndex =
         resolveRef(state, state.functionNameToIndexMap, state.module.functions.size(), functionRef);
   });
}
