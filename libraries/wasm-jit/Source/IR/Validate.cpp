...
ValueType validateGlobalIndex(const Module& module,
                              Uptr          globalIndex,
                              bool          mustBeMutable,
   ...
   switch (expression.type) {
      ...
      case InitializerExpression::Type::get_global: {
         const ValueType globalValueType = validateGlobalIndex(
            module, expression.globalIndex, false, true, true, "initializer expression global index");
...
struct FunctionValidationContext {
   ...
   void call_indirect(CallIndirectImm imm) {
      ...
      popAndValidateOperands(
         "call_indirect arguments", calleeType->parameters.data(), (Uptr)calleeType->parameters.size());
      pushOperand(calleeType->ret);
   }
