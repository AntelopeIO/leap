...
{
   void operator()(std::ostream& osm, const fc::variant& v) { ::operator<<(osm, v); }
};

template<>
struct print_log_value<fc::variant_object>
{
   void operator()(std::ostream& osm, const fc::variant_object& v) { ::operator<<(osm, v); }
};

template<>
struct print_log_value<fc::variant_object::entry>
{
   void operator()(std::ostream& osm, const fc::variant_object::entry& e) { ::operator<<(osm, e); }
};
...
struct webauthn_private_key
{
   ...
   public_key get_public_key(webauthn::public_key::user_presence_t presence =
                                webauthn::public_key::user_presence_t::USER_PRESENCE_NONE) const
   {
...
public:
   ...
   action get_action(account_name             code,
                     action_name              acttype,
   ...
   void set_authority(account_name                    account,
   ...
   void delete_authority(account_name                    account,
                         permission_name                 perm,
                         const vector<permission_level>& auths,
                         const vector<private_key_type>& keys);
   ...
   transaction_trace_ptr push_reqauth(account_name                    from,
                                      const vector<permission_level>& auths,
                                      const vector<private_key_type>& keys);
   ...
   const controller::config& get_config() const { return cfg; }
...
{
public:
   tester(setup_policy            policy                                      = setup_policy::full,
          db_read_mode            read_mode                                   = db_read_mode::HEAD,
          std::optional<uint32_t> genesis_max_inline_action_size              = std::optional<uint32_t>{},
          std::optional<uint32_t> config_max_nonprivileged_inline_action_size = std::optional<uint32_t>{})
   {
   ...
   tester(controller::config config, const genesis_state& genesis) { init(config, genesis); }

   tester(controller::config config) { init(config); }
...
public:
   ...
   static unique_ptr<controller> create_validating_node(controller::config   vcfg,
                                                        const genesis_state& genesis,
                                                        bool                 use_genesis,
                                                        deep_mind_handler*   dmlog = nullptr)
   {
      unique_ptr<controller> validating_node =
         std::make_unique<controller>(vcfg, make_protocol_feature_set(), genesis.compute_chain_id());
   ...
   signed_block_ptr produce_block_no_validation(
      fc::microseconds skip_time = fc::milliseconds(config::block_interval_ms))
   {
      return _produce_block(skip_time, false);
   }
