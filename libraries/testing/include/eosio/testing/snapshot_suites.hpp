...
struct variant_snapshot_suite {
   ...
   static auto get_writer() { return std::make_shared<writer>(std::make_shared<write_storage_t>()); }
   ...
   static auto get_reader(const snapshot_t& buffer) { return std::make_shared<reader>(buffer); }
...
struct json_snapshot_suite {
   ...
   struct reader : public reader_t {
      explicit reader(const fc::path& p)
         : reader_t(p) {}
      ~reader() { remove(json_snapshot_suite::temp_file().c_str()); }
   };
