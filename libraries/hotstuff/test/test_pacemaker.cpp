...
namespace eosio::hotstuff {
   ...
   void test_pacemaker::send_hs_proposal_msg(const hs_proposal_message& msg, name id, const std::optional<uint32_t>& connection_id) {
      if (!connection_id.has_value())
         _pending_message_queue.push_back(std::make_pair(id, msg));
   };

   void test_pacemaker::send_hs_vote_msg(const hs_vote_message& msg, name id, const std::optional<uint32_t>& connection_id) {
      if (!connection_id.has_value())
         _pending_message_queue.push_back(std::make_pair(id, msg));
   };

   void test_pacemaker::send_hs_new_block_msg(const hs_new_block_message& msg, name id, const std::optional<uint32_t>& connection_id) {
      if (!connection_id.has_value())
         _pending_message_queue.push_back(std::make_pair(id, msg));
   };

   void test_pacemaker::send_hs_new_view_msg(const hs_new_view_message& msg, name id, const std::optional<uint32_t>& connection_id) {
      if (!connection_id.has_value())
         _pending_message_queue.push_back(std::make_pair(id, msg));
   };
   ...
   void test_pacemaker::on_hs_proposal_msg(const hs_proposal_message& msg, name id) {
      ...
      while (qc_itr != _qcc_store.end()){
         ...
         if (qcc_ptr->get_id_i() != id && is_qc_chain_active(qcc_name) )
            qcc_ptr->on_hs_proposal_msg(msg, std::make_optional<uint32_t>(0));
         qc_itr++;
      }
   }
   ...
   void test_pacemaker::on_hs_vote_msg(const hs_vote_message& msg, name id) {
      ...
      while (qc_itr != _qcc_store.end()) {
         ...
         std::shared_ptr<qc_chain> & qcc_ptr  = qc_itr->second;
         if (qcc_ptr->get_id_i() != id && is_qc_chain_active(qcc_name) )
            qcc_ptr->on_hs_vote_msg(msg, std::make_optional<uint32_t>(0));
         qc_itr++;
      }
   }
   ...
   void test_pacemaker::on_hs_new_block_msg(const hs_new_block_message& msg, name id) {
      ...
      while (qc_itr != _qcc_store.end()) {
         ...
         std::shared_ptr<qc_chain> & qcc_ptr  = qc_itr->second;
         if (qcc_ptr->get_id_i() != id && is_qc_chain_active(qcc_name) )
            qcc_ptr->on_hs_new_block_msg(msg, std::make_optional<uint32_t>(0));
         qc_itr++;
      }
   }
   ...
   void test_pacemaker::on_hs_new_view_msg(const hs_new_view_message& msg, name id) {
      ...
      while (qc_itr != _qcc_store.end()){
         ...
         std::shared_ptr<qc_chain> & qcc_ptr  = qc_itr->second;
         if (qcc_ptr->get_id_i() != id && is_qc_chain_active(qcc_name) )
            qcc_ptr->on_hs_new_view_msg(msg, std::make_optional<uint32_t>(0));
         qc_itr++;
      }
   }
