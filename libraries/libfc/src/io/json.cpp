...
char parseEscape(T& in)
{
   if (in.peek() == '\\') {
      try {
         in.get();
         switch (in.peek()) {
            case 't': in.get(); return '\t';
            case 'n': in.get(); return '\n';
            case 'r': in.get(); return '\r';
            case '\\': in.get(); return '\\';
            default: return in.get();
         }
      }
   ...
   while (true) {
      switch (in.peek()) {
         ...
         case '\r':
            ...
            in.get();
            break;
         case '\0': FC_THROW_EXCEPTION(eof_exception, "unexpected end of file"); break;
...
std::string stringFromStream(T& in)
{
   ...
   try {
      ...
      while (!in.eof()) {
         switch (c = in.peek()) {
            case '\\': token << parseEscape(in); break;
            case 0x04:
               FC_THROW_EXCEPTION(parse_error_exception,
                                  "EOF before closing '\"' in string '${token}'",
                                  ("token", token.str()));
            case '"': in.get(); return token.str();
            default: token << c; in.get();
         }
      }
      FC_THROW_EXCEPTION(
         parse_error_exception, "EOF before closing '\"' in string '${token}'", ("token", token.str()));
   }
   ...
   try {
      ...
      while (!in.eof()) {
         switch (c = in.peek()) {
            ...
            case '\t':
            case ' ':
            case '\n': in.get(); return token.str();
            case '\0': FC_THROW_EXCEPTION(eof_exception, "unexpected end of file");
            default:
...
variant number_from_stream(T& in)
{
   ...
   if (str == "-." || str == "." || str == "-") // check the obviously wrong things we could have encountered
      FC_THROW_EXCEPTION(
         parse_error_exception, "Can't parse token \"${token}\" as a JSON numeric constant", ("token", str));
   if (dot)
      return parser_type == json::parse_type::legacy_parser_with_string_doubles ? variant(str)
...
variant variant_from_stream(T& in, uint32_t max_depth)
{
   ...
   while (1) {
      ...
      switch (c) {
         ...
         case '\t':
         case '\n':
         case '\r': in.get(); continue;
         case '"': return stringFromStream(in);
         case '{': return objectFromStream<T, parser_type>(in, max_depth - 1);
         case '[': return arrayFromStream<T, parser_type>(in, max_depth - 1);
         case '-':
         case '.':
         case '0':
         case '1':
         ...
         case '7':
         case '8':
         case '9': return number_from_stream<T, parser_type>(in);
         // null, true, false, or 'warning' / string
         case 'n':
         case 't':
         case 'f': return token_from_stream(in);
      ...
      switch (ptype) {
         ...
         case parse_type::legacy_parser_with_string_doubles:
            return variant_from_stream<std::stringstream,
                                       json::parse_type::legacy_parser_with_string_doubles>(in, max_depth);
         ...
         default: FC_ASSERT(false, "Unknown JSON parser type {ptype}", ("ptype", static_cast<int>(ptype)));
      }
   }
                          ...
                          bool                          escape_control_chars)
{
   ...
   for (auto itr = str.begin(); itr != str.end(); ++i, ++itr) {
      if (i % json::escape_string_yield_check_count == 0)
         yield(init_size + r.size());
      switch (*itr) {
         case '\x00': r += "\\u0000"; break;
         case '\x01': r += "\\u0001"; break;
         case '\x02': r += "\\u0002"; break;
...
void to_stream(T&                            os,
               const variants&               a,
               const json::yield_function_t& yield,
               const json::output_formatting format)
{
   yield(os.tellp());
   os << '[';
   auto itr = a.begin();
               ...
               const json::output_formatting format)
{
   yield(os.tellp());
   switch (v.get_type()) {
      case variant::null_type: os << "null"; return;
      case variant::int64_type: {
         int64_t i = v.as_int64();
         if (format == json::output_formatting::stringify_large_ints_and_doubles && i > 0xffffffff)
            os << '"' << v.as_string() << '"';
         else
            os << i;
      ...
      case variant::double_type:
         if (format == json::output_formatting::stringify_large_ints_and_doubles)
            ...
            os << v.as_string();
         return;
      case variant::bool_type: os << v.as_string(); return;
      case variant::string_type: os << '"' << escape_string(v.get_string(), yield) << '"'; return;
      case variant::blob_type: os << '"' << escape_string(v.as_string(), yield) << '"'; return;
      ...
      default:
         FC_THROW_EXCEPTION(fc::invalid_arg_exception,
                            "Unsupported variant type: " + std::to_string(v.get_type()));
   }
}
   ...
   for (uint32_t i = 0; i < v.size(); ++i) {
      switch (v[i]) {
         ...
         case ',':
            ...
            break;
         case 'n':
            // If we're in quotes and see a \n, \b, \f, \r, \t, or \u, just print it literally but unset the
            // escape flag.
         case 'b':
         case 'f':
         case 'r':
...
bool json::save_to_file(const variant&                v,
                        const fc::path&               fi,
                        const bool                    pretty,
                        const json::output_formatting format)
{
...
variant json::from_file(const fc::path& p, const json::parse_type ptype, const uint32_t max_depth)
{
   ...
   switch (ptype) {
      case json::parse_type::legacy_parser:
         return variant_from_stream<boost::filesystem::ifstream, json::parse_type::legacy_parser>(bi,
                                                                                                  max_depth);
      case json::parse_type::legacy_parser_with_string_doubles:
         return variant_from_stream<boost::filesystem::ifstream,
                                    json::parse_type::legacy_parser_with_string_doubles>(bi, max_depth);
...
); case strict_parser: return json_relaxed::variant_from_stream<buffered_istream, true>( in, max_depth ); case
relaxed_parser: return json_relaxed::variant_from_stream<buffered_istream, false>( in, max_depth ); default:
...
bool json::is_valid(const std::string& utf8_str, const json::parse_type ptype, const uint32_t max_depth)
{
   ...
   switch (ptype) {
      case json::parse_type::legacy_parser:
         ...
         break;
      case json::parse_type::legacy_parser_with_string_doubles:
         variant_from_stream<std::stringstream, json::parse_type::legacy_parser_with_string_doubles>(
            in, max_depth);
         break;
      case json::parse_type::strict_parser:
