...
namespace fc::crypto::blslib {
   ...
   {  
      ...
      std::array<uint8_t, 48> bytes = fc::crypto::blslib::serialize_base64<std::array<uint8_t, 48>>(data_str);
   ...
   std::string bls_public_key::to_string(const yield_function_t& yield)const {
      ...
      std::string data_str = fc::crypto::blslib::deserialize_base64<std::array<uint8_t, 48>>(bytes, yield); 
