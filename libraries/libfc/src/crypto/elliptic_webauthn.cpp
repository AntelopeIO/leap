...
struct webauthn_json_handler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, webauthn_json_handler> {
   ...
   bool Null() { return basic_stuff(); }
   bool Bool(bool) { return basic_stuff(); }
   bool Int(int) { return basic_stuff(); }
   bool Uint(unsigned) { return basic_stuff(); }
   bool Int64(int64_t) { return basic_stuff(); }
   ...
   bool String(const char* str, rapidjson::SizeType length, bool copy) {
      switch (current_state) {
         ...
         case EXPECT_TYPE_VALUE:
            ...
            current_state = EXPECT_FIRST_OBJECT_KEY;
            return true;
         case EXPECT_FIRST_OBJECT_DONTCARE_VALUE: current_state = EXPECT_FIRST_OBJECT_KEY; return true;
   ...
   bool StartObject() {
      switch (current_state) {
         ...
         case EXPECT_FIRST_OBJECT_DONTCARE_VALUE:
            ...
            return true;
         case IN_NESTED_CONTAINER: ++current_nested_container_depth; return true;
   ...
   bool Key(const char* str, rapidjson::SizeType length, bool copy) {
      switch (current_state) {
         ...
         case EXPECT_CHALLENGE_VALUE:
         case EXPECT_ORIGIN_VALUE:
         case EXPECT_TYPE_VALUE: return false;
         case EXPECT_FIRST_OBJECT_KEY: {
            if ("challenge"s == str)
...
public_key::public_key(const signature& c, const fc::sha256& digest, bool) {
   ...
   FC_ASSERT(handler.found_origin.compare(0, https_len, required_origin_scheme) == 0,
   ...
   FC_ASSERT(memcmp(c.auth_data.data(), fc::sha256::hash(rpid).data(), sizeof(fc::sha256)) == 0,
