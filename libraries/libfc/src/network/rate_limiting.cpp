   ...
   */
   ...
   virtual void perform_operation() override
   {
      if (raw_buffer)
         ...
         asio::async_write_some(socket, shared_buffer, permitted_length, offset, completion_promise);
   }
};
...
public:
   ...
   virtual void perform_operation() override
   {
      if (raw_buffer)
         ...
         asio::async_read_some(socket, shared_buffer, permitted_length, offset, completion_promise);
   }
};
...
rate_limiting_group_impl::rate_limiting_group_impl(uint32_t upload_bytes_per_second,
                                                   uint32_t download_bytes_per_second,
...
rate_limiting_group::rate_limiting_group(uint32_t upload_bytes_per_second,
                                         uint32_t download_bytes_per_second,
                                         uint32_t burstiness_in_seconds /* = 1 */)
