...
try {
   ...
   BOOST_CHECK_EXCEPTION(make_webauthn_sig(priv, auth_data, json).recover(d, true),
                         ...
                         [](const fc::assert_exception& e) {
                            return e.to_detail_string().find("webauthn origin must begin with https://") !=
                                   std::string::npos;
                         });
}
FC_LOG_AND_RETHROW();
...
try {
   ...
   BOOST_CHECK_EXCEPTION(
      make_webauthn_sig(priv, auth_data, json).recover(d, true), fc::exception, [](const fc::exception& e) {
...
try {
   ...
   BOOST_CHECK_EXCEPTION(make_webauthn_sig(priv, auth_data, json).recover(d, true),
                         fc::assert_exception,
                         [](const fc::assert_exception& e) {
                            return e.to_detail_string().find("webauthn signature type not an assertion") !=
                                   std::string::npos;
                         });
}
FC_LOG_AND_RETHROW();
...
try {
   ...
   BOOST_CHECK_EXCEPTION(make_webauthn_sig(priv, auth_data, json).recover(d, true),
                         fc::assert_exception,
                         [](const fc::assert_exception& e) {
                            return e.to_detail_string().find("webauthn rpid hash doesn't match origin") !=
                                   std::string::npos;
                         });
}
FC_LOG_AND_RETHROW();
...
try {
   webauthn::public_key wa_pub(
      pub.serialize(), webauthn::public_key::user_presence_t::USER_PRESENCE_NONE, "fctesting.invalid");
   std::string json =
      "{\"type\":\"webauthn.get\",\"challenge\":\"" + fc::base64url_encode(d.data(), d.data_size()) + "\"}";
