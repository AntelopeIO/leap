...
BOOST_AUTO_TEST_CASE(dir_copy) try {
   ...
   BOOST_CHECK_EQUAL(std::filesystem::exists(src_dir), true);
   const string test_file_name = "fc_copy_test_file";
   if (!std::filesystem::exists(src_dir + "/" + test_file_name)) {
      std::ofstream  ofs { src_dir + "/" + test_file_name};
      ofs << "This the test of fc system copy \n";
      ofs.close();
   }
   BOOST_CHECK_EQUAL(std::filesystem::exists(src_dir + "/" + test_file_name), true);

   const string tgt_dir {"/tmp/fc_copy_test_tgt"};
   if (std::filesystem::exists(tgt_dir)) {
      std::filesystem::remove_all(tgt_dir);
   }
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir), false);

   std::filesystem::copy(src_dir, tgt_dir);
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir), true);
   // check not copied recursively
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir + "/" + test_file_name), false);

   // 2. check whether exception be thrown (no overwritten) when target dir does exist,
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir), true);
   BOOST_CHECK_EXCEPTION(std::filesystem::copy(src_dir, tgt_dir), fc::exception,  [](const fc::exception& e) {
...
BOOST_AUTO_TEST_CASE(file_copy) try {
   ...
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir), true);
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir + "/" + test_file_name), false);
   std::filesystem::copy(src_dir + "/" + test_file_name, tgt_dir + "/" + test_file_name);
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir + "/" + test_file_name), true);
   ...
   BOOST_CHECK_EQUAL(std::filesystem::exists(tgt_dir + "/" + test_file_name), true);
   BOOST_CHECK_EXCEPTION(std::filesystem::copy(src_dir + "/" + test_file_name, tgt_dir + "/" + test_file_name),
                         fc::exception,
