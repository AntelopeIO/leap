...
const json::yield_function_t yield_deadline_exception_in_mid = [](size_t s) {
   if (s >= exception_limit_size) {
      throw fc::timeout_exception(
         fc::exception_code::timeout_exception_code, "timeout_exception", "execution timed out");
   }
};

const json::yield_function_t yield_length_exception = [](size_t s) { FC_ASSERT(s <= exception_limit_size); };
...
BOOST_AUTO_TEST_CASE(to_string_test) {
   ...
   BOOST_CHECK_EXCEPTION(
      deadline_exception_at_start_str = json::to_string(
         v, fc::time_point::min(), json::output_formatting::stringify_large_ints_and_doubles, json::max_length_limit),
...
}
{
   ...
   BOOST_CHECK_EXCEPTION(
      deadline_exception_at_start_str = json::to_string(
         v, fc::time_point::maximum(), json::output_formatting::stringify_large_ints_and_doubles, max_len),
      fc::assert_exception,
...
{ // to_string( const variant& v, const yield_function_t& yield, output_formatting format =
  // stringify_large_ints_and_doubles);
   ...
   {
      std::string deadline_exception_at_start_str;
      BOOST_CHECK_EXCEPTION(deadline_exception_at_start_str =
                               json::to_string(v, json_test_util::yield_deadline_exception_at_start),
                            fc::timeout_exception,
                            json_test_util::time_except_verf_func);
      BOOST_CHECK_EQUAL(deadline_exception_at_start_str.empty(), true);
   }
   {
      ...
      BOOST_CHECK_EXCEPTION(deadline_exception_in_mid_str =
                               json::to_string(v, json_test_util::yield_deadline_exception_in_mid),
                            fc::timeout_exception,
                            json_test_util::time_except_verf_func);
...
{ // to_string template call
   ...
   BOOST_REQUIRE_THROW(
      id_ret_2 =
         json::to_string(id, fc::time_point::maximum(), json::output_formatting::stringify_large_ints_and_doubles, len),
      fc::assert_exception);
   BOOST_CHECK_EQUAL(id_ret_2.empty(), true);

   // time_out
   std::string id_ret_3;
   BOOST_REQUIRE_THROW(id_ret_3 = json::to_string(id, fc::time_point::now() - fc::milliseconds(1)),
                       fc::timeout_exception);
...
BOOST_AUTO_TEST_CASE(to_pretty_string_test) {
   ...
   std::string deadline_exception_at_start_str;
   BOOST_CHECK_EXCEPTION(
      deadline_exception_at_start_str = json::to_pretty_string(
         v, fc::time_point::min(), json::output_formatting::stringify_large_ints_and_doubles, json::max_length_limit),
      fc::timeout_exception,
      json_test_util::time_except_verf_func);
   BOOST_CHECK_EQUAL(deadline_exception_at_start_str.empty(), true);
...
{ // to_pretty_string( const variant& v, const yield_function_t& yield, output_formatting format =
  // stringify_large_ints_and_doubles );
   const variant v(json_test_util::repeat_chars);
   BOOST_CHECK_LT(json_test_util::exception_limit_size, json_test_util::repeat_chars.size());
   {
      std::string deadline_exception_at_start_str;
      BOOST_CHECK_EXCEPTION(deadline_exception_at_start_str =
                               json::to_pretty_string(v, json_test_util::yield_deadline_exception_at_start),
                            fc::timeout_exception,
                            json_test_util::time_except_verf_func);
      BOOST_CHECK_EQUAL(deadline_exception_at_start_str.empty(), true);
   }
   {
      std::string deadline_exception_in_mid_str;
      BOOST_CHECK_EXCEPTION(deadline_exception_in_mid_str =
                               json::to_pretty_string(v, json_test_util::yield_deadline_exception_in_mid),
                            fc::timeout_exception,
                            json_test_util::time_except_verf_func);
      BOOST_CHECK_EQUAL(deadline_exception_in_mid_str.empty(), true);
   }
   {
      std::string length_exception_in_mid_str;
      BOOST_CHECK_EXCEPTION(length_exception_in_mid_str =
                               json::to_pretty_string(v, json_test_util::yield_length_exception),
                            fc::assert_exception,
                            json_test_util::length_limit_except_verf_func);
      BOOST_CHECK_EQUAL(length_exception_in_mid_str.empty(), true);
   }
...
{ // to_pretty_string template call
   ...
   std::string id_ret_2;
   BOOST_REQUIRE_THROW(
      id_ret_2 = json::to_pretty_string(
         id, fc::time_point::maximum(), json::output_formatting::stringify_large_ints_and_doubles, len),
      fc::assert_exception);
   BOOST_CHECK_EQUAL(id_ret_2.empty(), true);

   // time_out
   std::string id_ret_3;
   BOOST_REQUIRE_THROW(id_ret_3 = json::to_pretty_string(id, fc::time_point::now() - fc::milliseconds(1)),
                       fc::timeout_exception);
   BOOST_CHECK_EQUAL(id_ret_3.empty(), true);
}
}
...
BOOST_AUTO_TEST_CASE(escape_string_test) {
   ...
   BOOST_CHECK_LT(json_test_util::escape_input_str.size() - json_test_util::repeat_char_num,
                  json_test_util::exception_limit_size); // by using size_different to calculate expected string
   {
      // simulate exceed time exception at the beginning of processing
      BOOST_CHECK_EXCEPTION(
         escape_string(json_test_util::escape_input_str, json_test_util::yield_deadline_exception_at_start),
         fc::timeout_exception,
         json_test_util::time_except_verf_func);
   }
   {
      ...
      BOOST_CHECK_EXCEPTION(
         escape_string(json_test_util::escape_input_str, json_test_util::yield_deadline_exception_in_mid),
         fc::timeout_exception,
         json_test_util::time_except_verf_func);
   }
   {
