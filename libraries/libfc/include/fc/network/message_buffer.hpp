...
public:
    ...
    */
   char* read_ptr() { return &buffers[read_ind.first]->at(read_ind.second); }

   /*
    *  Returns the current write pointer pointing to the memory location
    *  of the next byte to be written to.
    */
   char* write_ptr() { return &buffers[write_ind.first]->at(write_ind.second); }
    ...
    */
   uint32_t bytes_to_read() const { return bytes_to_read_from_index(read_ind); }
    ...
    */
   uint32_t bytes_to_write() const { return total_bytes() - write_ind.first * buffer_len - write_ind.second; }

   /*
    *  Returns the total number of bytes in the buffer chain.
    */
   uint32_t total_bytes() const { return buffer_len * buffers.size(); }
   ...
   std::vector<boost::asio::mutable_buffer> get_buffer_sequence_for_boost_async_read() {
      ...
      seq.push_back(
         boost::asio::buffer(&buffers[write_ind.first]->at(write_ind.second), buffer_len - write_ind.second));
...
private:
    ...
    */
   char* get_ptr(const index_t& index) const { return &buffers[index.first]->at(index.second); }
...
public:
   ...
   inline bool read(char* d, size_t s) {
      ...
      fc::detail::throw_datastream_range_error(
         "peek", mb.bytes_to_read_from_index(index), s - mb.bytes_to_read_from_index(index));
   }
