...
public:
   ...
   fc::path get_file_path() const { return _file_path; }
   ...
   auto fileno() const
   {
      ...
      if (-1 == fd) {
         throw std::ios_base::failure(
            "cfile: " + _file_path.generic_string() +
            " unable to convert file pointer to file descriptor, error: " + std::to_string(errno));
      }
      return fd;
   }
   ...
   size_t tellp() const
   {
      ...
      if (result == -1)
         throw std::ios_base::failure(
            "cfile: " + get_file_path().generic_string() +
   ...
   void seek(long loc)
   {
      if (0 != fseek(_file.get(), loc, SEEK_SET)) {
         ...
         throw std::ios_base::failure("cfile: " + _file_path.generic_string() + " unable to SEEK_SET to: " +
                                      std::to_string(loc) + ", ferror: " + std::to_string(err));
      }
   }
   ...
   void write(const char* d, size_t n)
   {
      ...
      if (result != n) {
         throw std::ios_base::failure("cfile: " + _file_path.generic_string() + " unable to write " +
                                      std::to_string(n) + " bytes; only wrote " + std::to_string(result));
      }
   }
