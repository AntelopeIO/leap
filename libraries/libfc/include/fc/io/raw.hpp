...
// fc::string
template<typename Stream>
inline void pack(Stream& s, const fc::string& v)
{
...
template<typename Stream>
inline void unpack(Stream& s, fc::string& v)
{
   std::vector<char> tmp;
   fc::raw::unpack(s, tmp);
   if (tmp.size())
...
// bip::basic_string
template<typename Stream>
inline void pack(Stream& s, const shared_string& v)
{
   FC_ASSERT(v.size() <= MAX_SIZE_OF_BYTE_ARRAYS);
   fc::raw::pack(s, unsigned_int((uint32_t)v.size()));
   if (v.size())
...
template<typename Stream>
inline void unpack(Stream& s, shared_string& v)
{
   std::vector<char> tmp;
   fc::raw::unpack(s, tmp);
   FC_ASSERT(v.size() == 0);
