   ...
   try {
      ...
      while (true) {
         switch (c = in.peek()) {
            ...
            case '\0':
            case '\n':
            case '\x04': in.get(); return token.str();
            case 'a':
            case 'b':
            case 'c':
            case 'd':
      ...
      switch (c) {
         case '\'':
            if (strict)
               FC_THROW_EXCEPTION(parse_error_exception, "expected: '\"' at beginning of string, got '\''");
            // falls through
         case '"': return quoteStringFromStream<T, strict, true>(in);
         case 'r':
            if (strict)
         ...
         case EOF: FC_THROW_EXCEPTION(eof_exception, "unexpected end of file");
         default: FC_THROW_EXCEPTION(parse_error_exception, "expected: string");
      }
   }
...
fc::variant parseInt(const std::string& token, size_t start)
{
   ...
   while (true) {
      ...
      if (vc == 0xFF)
         FC_THROW_EXCEPTION(
            parse_error_exception, "illegal character ${c} in integer of base ${b}", ("c", c)("b", base));
      if (val > maxb4mul)
         ...
         case '0':
            ...
            switch (token[i]) {
               ...
               default:
                  ...
                  if (strict)
                     FC_THROW_EXCEPTION(
                        parse_error_exception, "expected '.'|'e'|'E' parsing number, got '${c}'", ("c", c));
            }
            break;
         case '1':
         case '2':
            ...
            case '.':
               ...
               if (dot_ok) {
                  ...
                  switch (c) {
                     ...
                     default:
                        FC_THROW_EXCEPTION(
                           parse_error_exception, "illegal character '${c}' in token", ("c", c)("i", int(c)));
                  }
               } else {
                  if (strict)
...
variant variant_from_stream(T& in, uint32_t max_depth)
{
   ...
   while (signed char c = in.peek()) {
      switch (c) {
         ...
         case '"': return json_relaxed::stringFromStream<T, strict>(in);
         case '{': return json_relaxed::objectFromStream<T, strict>(in, max_depth - 1);
         case '[': return json_relaxed::arrayFromStream<T, strict>(in, max_depth - 1);
         case '-':
         case '+':
         case '.':
         case '0':
         ...
         case '7':
         case '8':
         case '9': return json_relaxed::numberFromStream<T, strict>(in);
         ...
         case 'Z':
         case '_':
         case '/': return json_relaxed::wordFromStream<T, strict>(in);
