...
#define FC_REFLECT_VISIT_BASE(r, visitor, base) fc::reflector<base>::visit_base(visitor);
   ...
   struct reflector<ENUM> {                                                                                            \
      ...
      static const char*    to_string(ENUM elem) {                                                                     \
            switch (elem) {                                                                                            \
               ...
               default: fc::throw_bad_enum_cast(fc::to_string(int64_t(elem)).c_str(), BOOST_PP_STRINGIZE(ENUM) );      \
         }                                                                                                          \
            return nullptr;                                                                                            \
      }                                                                                                                \
...
#define FC_REFLECT_DERIVED_TEMPLATE(TEMPLATE_ARGS, TYPE, INHERITS, MEMBERS)                                            \
   ...
   struct reflector<TYPE> {                                                                                            \
      ...
      static_assert(not fc::has_reflector_init<TYPE>::value || std::is_base_of<fc::reflect_init, TYPE>::value,         \
                    "must derive from fc::reflect_init");                                                              \
      static_assert(not std::is_base_of<fc::reflect_init, TYPE>::value || fc::has_reflector_init<TYPE>::value,         \
                    "must provide reflector_init() method");                                                           \
   };                                                                                                                  \
   }

#define FC_REFLECT_DERIVED(TYPE, INHERITS, MEMBERS) FC_REFLECT_DERIVED_TEMPLATE((), TYPE, INHERITS, MEMBERS)
